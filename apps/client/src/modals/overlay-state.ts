...
const getCurrentTopbarReserveHeight = (): number => {
  if (typeof document === "undefined") {
    return 0;
  }

  const topbar = document.getElementById("game-header") || document.querySelector<HTMLElement>(".game-topbar");
  const rectHeight = Math.ceil(topbar?.getBoundingClientRect?.().height || 0);
  return Math.max(0, rectHeight || topbar?.offsetHeight || 0);
};
