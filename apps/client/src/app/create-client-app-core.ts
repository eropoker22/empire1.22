...
import { createClientStore } from "../state/client-store";
import { createInitialClientUiState } from "../state/client-ui-state";
import { createCommandDispatcher } from "../transport/command-dispatcher";
import type { ClientTransport } from "../transport/client-transport";

export interface CreateClientAppCoreOptions {
  transport: ClientTransport;
  projectRenderState(store: ReturnType<typeof createClientStore>): ClientRenderState;
  onStateRecompute?(reason: string): void;
}

export const createClientAppCore = ({
  transport,
  projectRenderState,
  onStateRecompute
}: CreateClientAppCoreOptions): ClientAppShell => {
