...
describe("production info view models", () => {
  ...
  it("builds factory info rows with exact labels", () => {
    ...
    expect(viewModel.effectsLabel).toBe("Další level: pasivní produkce +50%");
    ...
    expect(viewModel.upgrade).toEqual({ costLabel: "100$", benefitLabel: "L3 · pasivní produkce +50%" });
  ...
  it("handles missing config without crashing", () => {
    expect(getProductionBuildingEffectsLabel("unknown", 1)).toBe("Budova · základní pasivní produkce");
