...
describe("factory dashboard view model and panel", () => {
  ...
  it("adapts authoritative Factory data into the same canonical presentation as the demo", () => {
    ...
    expect(viewModel.slots.every((slot) => slot.durationAdjustmentLabel.startsWith("Zrychlení: "))).toBe(true);
    expect(viewModel.slots[0].durationAdjustmentLabel).toContain("→");
