...
export const publicBuildingDefinitions: PublicBuildingDefinition[] = [
  ...
  building("shopping_mall", "Obchodní centrum", "commercial", "Economy / market / influence / multiplier", "Obchodní centrum generuje peníze, menší dirty cash, vliv a snižuje ceny na marketu. Výlohy svítí, kasy pípají a v podzemních garážích se domlouvají dohody, které nikdy neuvidíš na účtence. Obchodní centrum není jen nákupní zóna. Je to tepna zásobování.", perMinuteStat(3700 / 60, 22, 65, 95, 1), []),
  building("restaurant", "Restaurace", "commercial", "Economy / rumors / influence / city life", "Restaurace generuje čisté peníze, trochu vlivu a městské drby. Lokální tržby, kryté schůzky a síť kontaktů z ní dělají civilní oporu komerčních districtů. Stoly u okna, zadní vchod pro kurýry a kuchyně, kde se slyší víc než na ulici.", perMinuteStat(38, 0, 57.6, 172.8, 1), [
  ...
  building("arcade", "Herna", "residential", "Economy / dirty cash / laundering / network", "Herna je pouliční cashflow. Blikající automaty, špinavé mince, zadní pokladna a dým z cigaret. Sama o sobě tě nespasí, ale síť heren dokáže krmit gang celou free session.", perMinuteStat(30, 20, 172.8, 80, 1), [
  ...
  building("casino", "Kasino", "commercial", "Laundering / high-risk", "Vzácná high-value neonová pračka peněz. Dává extrémní cashflow, dirty cash a vliv, ale rychle zvedá heat a audit risk.", perMinuteStat(4500 / 60, 2500 / 60, 150, 110, 4), [
    ...
    action({ actionId: "bribed_inspector", label: "Podplacený inspektor", description: "Zaplatí inspektora. Úspěch sníží heat a audit risk, selhání zvýší tlak.", effectSummary: "Cena 15000 clean cash, šance selhání 14 %, heat -15 při úspěchu, audit control", cooldownMs: 105 * minute, durationMs: 12 * minute, inputCost: out("cash", 15000) })
  ]),
  ...
  building("exchange", "Směnárna", "commercial", "Economy / laundering / network", "Směnárna pere menší částky bezpečněji než kasino. Jedna směnárna je služba. Síť směnáren je finanční pavouk přes celé město.", perMinuteStat(70, 95, 70, 60, 1), [
  ...
  building("clinic", "Klinika", "residential", "Economy / recovery / support", "Klinika nevyrábí zbraně ani gang. Zachraňuje to, co by jinak město sežralo.", stat(3100, 0, 85, 0, 1), [
    action({
  ...
  building("factory", "Továrna", "industrial", "Production", "Produkční budova pro Metal Parts, Tech Core a bojové moduly.", stat(0, 0, 3, 10, 14), [
  ...
  building("street_dealers", "Pouliční dealeři", "park", "Dirty cash / drug distribution / street economy", "Pouliční dealeři generují slabší dirty cash a prodávají látky z Drug Labu za špinavé peníze. Lab vyrobí produkt. Pouliční dealeři ho promění v peníze.", perMinuteStat(0, 36, 0.06 * 60 * 24, 0, 1), [
    action({ actionId: "start_drug_sale", label: "Spustit prodej", description: "Použije globální slot Pouličních dealerů k prodeji látky vyrobené v Drug Labu za dirty cash.", effectSummary: "slot Pouličních dealerů, dirty cash, heat, pouliční riziko" }),
