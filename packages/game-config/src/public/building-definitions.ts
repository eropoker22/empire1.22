...
export const publicBuildingDefinitions: PublicBuildingDefinition[] = [
  building("central_bank", "Centrální banka", "downtown", "Ultra vzácná / finance / rezervy / stabilita marketu", "Centrální banka netiskne chaos. Drží ho pod zámkem. Kdo ovládá rezervy, nemusí vyhrávat každou přestřelku. Stačí, když přežije každou krizi.", perMinuteStat(160, 0, 0.1 * 60 * 24, 0.35 * 60 * 24, 1), [
    ...
    action({ actionId: "frozen_accounts", label: "Zmrazené účty", description: "Dočasně zvýší ochranu clean cash a sníží finanční ztráty.", effectSummary: "Cena 2000 clean cash, ochrana rezerv a snížení finančních ztrát", durationMs: 8 * minute, cooldownMs: 24 * minute, heatGain: 5, inputCost: out("cash", 2000) }),
  ...
  building("stock_exchange", "Burza", "downtown", "Ultra vzácná / ekonomika / kontrola marketu / finanční síla", "Burza je jediná na mapě. Neprodává zboží. Ovlivňuje ceny a rytmus celé ekonomiky. Skleněná věž v Downtownu, kde se války nevedou noži, ale grafy.", perMinuteStat(220, 0, 0.18 * 60 * 24, 0.45 * 60 * 24, 1), [
    ...
    action({ actionId: "market_pressure", label: "Tržní tlak", description: "Na 10 minut zvýší ceny materiálového marketu.", effectSummary: "Cena 3000 clean + 15 influence, heat +8, market efekt", cooldownMs: 22 * minute, durationMs: 10 * minute, heatGain: 8, inputCost: out("cash", 3000), influenceChange: -15 }),
    action({ actionId: "insider_window", label: "Vnitřní tipy", description: "Na 16 minut zlepší tržní signály a zvýší šanci Spekulativního nákupu.", effectSummary: "Cena 750 clean, heat +4, 3 tržní signály, vyšší šance spekulace", cooldownMs: 30 * minute, durationMs: 16 * minute, heatGain: 4, inputCost: out("cash", 750) })
  ]),
  ...
  building("power_station", "Energetická stanice", "industrial", "Infrastruktura / podpora / obrana", "Energetická stanice nezavádí nový zdroj. Zvedá výkon města, drží infrastrukturu při životě a posiluje bezpečnostní systémy. Když svítí stanice, město dýchá rychleji. Kamery vidí ostřeji. Alarmy řvou dřív.", perMinuteStat(2780 / 60, 780 / 60, 115.2, 0, 1), [
    ...
    action({ actionId: "power_station_feed_production", label: "Prodat přebytek", description: "Okamžitě zpeněží přebytek energie. Rychlost výrobních linek nemění.", effectSummary: "+2000 clean cash, +500 dirty cash, heat +10", cooldownMs: 60 * minute, outputGain: resources({ cash: 2000, "dirty-cash": 500 }), heatGain: 10 }),
    action({ actionId: "power_station_reduce_heat", label: "Snížit heat", description: "Za 5000 clean cash serverově sníží HEAT hráče i districtu.", effectSummary: "Cena 5000 clean cash, heat -20, cooldown 60 minut", cooldownMs: 60 * minute, inputCost: out("cash", 5000), heatGain: -20 })
  ]),
