(function() {
  ...
  const request = async (url, init) => {
    ...
    if (!response.ok || !payload?.accepted) {
      ...
      throw new AdminApiError(
        response.status,
        apiError?.code ?? "ADMIN_INVALID_RESPONSE",
  ...
  const readAdminFrontendBuildSha = () => {
    const value = document.querySelector('meta[name="empire-build-sha"]')?.content.trim() ?? "";
  ...
  const hasFocusedAdminInput = (target) => {
    ...
    return Boolean(target?.contains(active) && (active instanceof HTMLInputElement || active instanceof HTMLSelectElement || active instanceof HTMLTextAreaElement));
  };
  ...
  const updateAdminRefreshUi = (target, status) => {
    const node = target?.querySelector("[data-admin-refresh-state]");
    if (!node) return;
    ...
    const button2 = target?.querySelector("[data-admin-refresh]");
    if (!button2) return;
  ...
  const applyAdminServerFilters = (target, filters) => {
    ...
    let visibleCount = 0;
    target?.querySelectorAll("[data-admin-server-item]").forEach((item) => {
    ...
    const output = target?.querySelector("[data-admin-server-visible-count]");
  ...
  const adminDetailStateEntries = (target) => {
    ...
    return [...target.querySelectorAll("details")].map((detail, index) => {
      const summary = detail.querySelector(":scope > summary")?.textContent ?? "";
  ...
  const publicBuildingDefinitions = [
    ...
    building("lobby_club", "Lobby Club", "downtown", "Ultra vzácná / lobby / vliv / politická podpora", "Lobby Club není úřad. Je to místnost vedle úřadu, kde se rozhodne dřív, než někdo zvedne ruku. Kdo drží Lobby Club, nevládne městu přímo. Jen šeptá lidem, kteří městem hýbou.", perMinuteStat(95, 0, 0.1 * 60 * 24, 0.65 * 60 * 24, 1), [
      action({ actionId: "backroom_pressure", label: "Zákulisní tlak", description: "Na 20 minut posílí influence produkci všech budov, zlevní influence akce a sníží negativní drby.", effectSummary: "Cena 1200 clean + 8 influence, influence +18 %, influence akce -10 %, heat +3", cooldownMs: 40 * minute, durationMs: 20 * minute, heatGain: 3, inputCost: out("cash", 1200), influenceChange: -8 }),
      action({ actionId: "quiet_negotiation", label: "Tiché vyjednávání", description: "Zkrátí jeden aktivní politický nebo společenský cooldown, sníží rizika a zlevní další influence akci.", effectSummary: "Cena 800 clean + 5 influence, cooldown -20 % zbývajícího času, heat +2", cooldownMs: 24 * minute, heatGain: 2, inputCost: out("cash", 800), influenceChange: -5 }),
      action({ actionId: "media_screen", label: "Mediální clona", description: "Na 15 minut sníží pravděpodobnost negativních drbů.", effectSummary: "Cena 800 clean, negativní drby -35 %, heat +4", cooldownMs: 30 * minute, durationMs: 15 * minute, heatGain: 4, inputCost: out("cash", 800) })
    ]),
    building("stock_exchange", "Burza", "downtown", "Ultra vzácná / ekonomika / kontrola marketu / finanční síla", "Burza je jediná na mapě. Neprodává zboží. Ovládá ceny, poplatky a rytmus celé ekonomiky. Skleněná věž v Downtownu, kde se války nevedou noži, ale grafy.", perMinuteStat(220, 0, 0.18 * 60 * 24, 0.45 * 60 * 24, 1), [
      ...
      action({ actionId: "insider_window", label: "Vnitřní tipy", description: "Na 16 minut zlepší trend hinty, sníží market poplatek a zvedne šanci Spekulativního nákupu.", effectSummary: "Cena 750 clean, heat +4, 3 trend hinty, market poplatek -8 %", cooldownMs: 30 * minute, durationMs: 16 * minute, heatGain: 4, inputCost: out("cash", 750) })
    ]),
    ...
    building("airport", "Letiště", "downtown", "Ultra vzácná / logistika / import / černý trh / mobilita", "Letiště je brána města. Co ostatní musí vyrábět, ty můžeš dovézt. Co ostatní musí vozit ulicemi, ty pošleš přes runway. Ale každý kontejner má papíry. A každý falešný papír jednou někdo zkontroluje.", perMinuteStat(180, 45, 0.2 * 60 * 24, 0.2 * 60 * 24, 1), [
      action({ actionId: "express_import", label: "Expresní dovoz", description: "Okamžitě doručí materiálovou importní zásilku do SKLADU hráče.", effectSummary: "Cena 2000 clean, heat +6, customs risk 10 %", cooldownMs: 18 * minute, durationMs: 0, heatGain: 6, inputCost: out("cash", 2e3) }),
      action({ actionId: "black_charter", label: "Černý charter", description: "Na 8 minut zlevní vybrané položky černého trhu o dalších 6 %. Celní kontrola při nákupu může přidat 10 HEAT.", effectSummary: "Cena 1000 dirty, heat +9, nabídka -6 %, celní zátah při nákupu 15 %", cooldownMs: 24 * minute, durationMs: 8 * minute, heatGain: 9, inputCost: out("dirty-cash", 1e3) }),
      action({ actionId: "evacuation_corridor", label: "Evakuační koridor", description: "Na 7 minut podpoří útoky zahájené během účinku: zlepší únik a omezí ztráty při neúspěchu.", effectSummary: "Cena 1800 clean, heat +5, escape +18 %, ztráty -10 %", cooldownMs: 26 * minute, durationMs: 7 * minute, heatGain: 5, inputCost: out("cash", 1800) })
    ]),
    building("port", "Přístav", "downtown", "Logistika", "Těžká logistika, kontejnery, materiály a dirty cash přes mořské trasy.", perMinuteStat(26, 8.5, 5, 26), [
      action({ actionId: "port_container_cut", label: "Proříznout kontejner", description: "Vybere z kontejnerů užitečné zásoby.", effectSummary: "+dirty cash, +metal parts, +heat", cooldownMs: 40 * minute, heatGain: 6, outputGain: resources({ "dirty-cash": 900, "metal-parts": 6 }), influenceChange: 2 })
    ]),
    building("parliament", "Parlament", "downtown", "Moc", "Nejvyšší politická páka s extrémním clean income a vlivem.", perMinuteStat(22, 3, 3, 40), [
      action({ actionId: "parliament_policy_window", label: "Politické okno", description: "Otevře krátké politické okno pro zisk vlivu.", effectSummary: "+vliv, +clean cash, +heat", cooldownMs: 45 * minute, heatGain: 5, outputGain: out("cash", 1600), influenceChange: 10 })
    ]),
    ...
    building("restaurant", "Restaurace", "commercial", "Ekonomika / drby / vliv / městský provoz", "Restaurace generuje čisté peníze, trochu vlivu a městské drby. Lokální tržby, kryté schůzky a síť kontaktů z ní dělají civilní oporu komerčních districtů. Stoly u okna, zadní vchod pro kurýry a kuchyně, kde se slyší víc než na ulici.", perMinuteStat(38, 0, 57.6, 172.8, 1), [
      action({ actionId: "restaurant_collect_revenue", label: "Vybrat tržby", description: "Vybere lokální tržby restaurace jako clean a dirty cash.", effectSummary: "+1800 clean cash, +900 dirty cash, heat +5", cooldownMs: 60 * minute, outputGain: resources({ cash: 1800, "dirty-cash": 900 }), heatGain: 5 }),
      ...
      action({ actionId: "restaurant_local_network", label: "Posílit lokální síť", description: "Na 30 minut posílí lokální vliv restaurace.", effectSummary: "+50 % vliv, +10 vliv, heat +5 na 30 minut", cooldownMs: 45 * minute, durationMs: 30 * minute, heatGain: 5, influenceChange: 10, effectModifiers: { influenceMultiplier: 1.5 } })
    ]),
    ...
    building("exchange", "Směnárna", "commercial", "Ekonomika / praní / síť", "Směnárna pere menší částky bezpečněji než kasino. Jedna směnárna je služba. Síť směnáren je finanční pavouk přes celé město.", perMinuteStat(70, 95, 70, 60, 1), [
      action({ actionId: "good_rate", label: "Výhodný kurz", description: "Ve dne vypere 16 % aktuálního dirty cash hráče přes síť směnáren.", effectSummary: "-dirty cash, +clean cash po 12 % poplatku, heat +12, vliv +3, +audit risk", cooldownMs: 18 * minute, heatGain: 6, influenceChange: 3 })
    ]),
    ...
    building("power_station", "Energetická stanice", "industrial", "Infrastruktura / podpora / obrana", "Energetická stanice nezavádí nový zdroj. Zvedá výkon města, drží infrastrukturu při životě a posiluje bezpečnostní systémy. Když svítí stanice, město dýchá rychleji. Kamery vidí ostřeji. Alarmy řvou dřív.", perMinuteStat(2780 / 60, 780 / 60, 115.2, 0, 1), [
      ...
      action({ actionId: "power_station_reduce_heat", label: "Snížit heat", description: "Za 5000 clean cash serverově sníží HEAT hráče i districtu.", effectSummary: "Cena 5000 clean cash, heat -20, cooldown 60 minut", cooldownMs: 60 * minute, inputCost: out("cash", 5e3), heatGain: -20 })
    ]),
    ...
    building("smuggling_tunnel", "Pašovací tunel", "park", "Dirty cash / smuggling / dealer support / risk reward", "Pašovací tunel pasivně vytváří dirty cash a podporuje rychlost i bezpečnost pouliční distribuce. Cenu prodeje neurčuje: ta zůstává pevně odvozená z výroby v Drug Labu.", perMinuteStat(0, 54, 0.07 * 60 * 24, 0, 1), [
      action({ actionId: "open_channel", label: "Otevřít kanál", description: "Na 30 minut posílí dirty cash tunelů a zrychlí prodej Pouličních dealerů. Nestackuje se.", effectSummary: "Cena 900 clean cash, heat +5, +45 % dirty tok tunelů, rychlejší prodej a vyšší riziko incidentu", cooldownMs: 60 * minute, durationMs: 30 * minute, inputCost: out("cash", 900), heatGain: 5 })
    ]),
    ...
    building("strip_club", "Strip Club", "park", "Economy / influence / rumors / social network", "Strip Club pasivně vytváří peníze, vliv a drby. Vybrání cash, VIP klienti a soukromá party jsou samostatné akce s náklady, heatem a cooldownem.", perMinuteStat(75, 65, 85, 90, 1), [
      action({ actionId: "strip_club_collect_cash", label: "Vybrat cash", description: "Okamžitě vybere noční dirty cash ze Strip Clubu.", effectSummary: "+1500 dirty cash, heat +5", cooldownMs: 40 * minute, outputGain: out("dirty-cash", 1500), heatGain: 5 }),
      ...
      action({ actionId: "private_party", label: "Soukromá party", description: "Přidá okamžitý vliv, dočasně zvýší jeho tvorbu a může přinést extra drb nebo skandál.", effectSummary: "Cena 900 clean cash, +12 influence, +70 % influence na 10 minut, heat +6, riziko skandálu 12 %", cooldownMs: 30 * minute, durationMs: 10 * minute, inputCost: out("cash", 900), heatGain: 6, influenceChange: 12, effectModifiers: { influenceMultiplier: 1.7 } })
    ]),
  ...
  const freeModeAirportConfig = {
    ...
    expressImport: {
      ...
      shipmentValueRanges: {
        materials: { min: 3e3, max: 4e3 },
        rareComponents: { min: 4200, max: 6e3 },
        weapons: { min: 3e3, max: 4200 },
        defenseItems: { min: 3e3, max: 4200 }
      }
    },
    blackCharter: {
  ...
  const STREET_SALE_PRICE_MULTIPLIER = 1.8;
  ...
  const catalog = /* @__PURE__ */ JSON.parse('[{"id":"victor_01","agentId":"victor","title":"Rozbitá dodávka","description":"Jedna dodávka zůstala viset v cizím bloku. Dojeď tam, seber bedny a zmiz dřív, než si toho někdo všimne.","difficulty":"medium","successRate":82,"durationMinutes":18,"reward":{"metal-parts":5,"cash":700,"influence":2},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_02","agentId":"victor","title":"Tvrdé vyjednávání","description":"Jeden obchodník zapomněl, komu má platit. Připomeň mu to po mém.","difficulty":"medium","successRate":78,"durationMinutes":21,"reward":{"cash":1400,"influence":4},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_03","agentId":"victor","title":"Sklad bez majitele","description":"Ve skladu leží materiál a nikdo ho zrovna nehlídá dost dobře. Udělej to rychle.","difficulty":"medium","successRate":80,"durationMinutes":20,"reward":{"metal-parts":4,"cash":500},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_04","agentId":"victor","title":"Noční výběr","description":"Po zavíračce bývá město měkký. Vytáhni z toho maximum, než se vzpamatuje.","difficulty":"medium","successRate":74,"durationMinutes":21,"reward":{"dirty-cash":1600,"influence":3},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_05","agentId":"victor","title":"Převoz pod tlakem","description":"Materiál musí projít přes horkou zónu. Když to zvládneš, lidi si tě začnou pamatovat.","difficulty":"medium","successRate":73,"durationMinutes":21,"reward":{"cash":900,"influence":3},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_06","agentId":"victor","title":"Rozkopané dveře","description":"Za těma dveřma je schovaná zásoba. Otevři je po svém, já se ptát nebudu.","difficulty":"medium","successRate":76,"durationMinutes":19,"reward":{"metal-parts":3,"influence":2},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_07","agentId":"victor","title":"Tichá lekce","description":"Jeden malej hráč moc mluví. Nemusí zmizet, stačí aby začal šeptat.","difficulty":"medium","successRate":84,"durationMinutes":17,"reward":{"influence":6,"cash":700},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_08","agentId":"victor","title":"Neonový kufr","description":"Na rohu čeká kufr, kterej nemá dlouho zůstat bez majitele. Buď rychlej.","difficulty":"medium","successRate":77,"durationMinutes":16,"reward":{"cash":600,"influence":2},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_09","agentId":"victor","title":"Smrad z lékárny","description":"Z jedný lékárny odtéká víc chemie, než je zdravý. Jdi po tom.","difficulty":"medium","successRate":83,"durationMinutes":15,"reward":{"cash":500},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_10","agentId":"victor","title":"Ochlazení konkurence","description":"Někdo vedle nás roste moc rychle. Ukaž mu, že asfalt má vždycky poslední slovo.","difficulty":"hard","successRate":70,"durationMinutes":28,"reward":{"influence":7,"dirty-cash":900},"risk":{"successHeat":4,"failureHeat":10,"failureDirtyCashLoss":400}},{"id":"victor_11","agentId":"victor","title":"Pouliční test loajality","description":"Ne každej pod tlakem drží hubu. Ověř, kdo je pevnej a kdo je hadr.","difficulty":"easy","successRate":86,"durationMinutes":14,"reward":{"influence":3,"cash":750},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"victor_12","agentId":"victor","title":"Dvě minuty po půlnoci","description":"V noci mizí kamery, svědci i zábrany. Přesně proto jdeme teď.","difficulty":"hard","successRate":68,"durationMinutes":26,"reward":{"metal-parts":2,"influence":3},"risk":{"successHeat":4,"failureHeat":10,"failureDirtyCashLoss":400}},{"id":"victor_13","agentId":"victor","title":"Balík pro špatnou adresu","description":"Někdo čeká zásilku. Jen škoda, že ji čeká marně.","difficulty":"medium","successRate":79,"durationMinutes":18,"reward":{"cash":850,"influence":2},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_14","agentId":"victor","title":"Krev na parkovišti","description":"Na parkovišti se má uzavřít obchod. Udělej z toho náš obchod.","difficulty":"hard","successRate":66,"durationMinutes":27,"reward":{"dirty-cash":2000,"metal-parts":2},"risk":{"successHeat":4,"failureHeat":10,"failureDirtyCashLoss":400}},{"id":"victor_15","agentId":"victor","title":"Kdo stojí, ten bere","description":"Některý rajóny patří těm, co v nich vydrží stát nejdýl. Dneska tam budeš stát ty.","difficulty":"hard","successRate":72,"durationMinutes":30,"reward":{"influence":8,"cash":1000},"risk":{"successHeat":4,"failureHeat":10,"failureDirtyCashLoss":400}},{"id":"victor_16","agentId":"victor","title":"Rozebraná zbrojnice","description":"Mám tip na rozebranou dílnu. Posbírej všechno, co ještě střílí nebo se dá prodat.","difficulty":"medium","successRate":75,"durationMinutes":21,"reward":{"metal-parts":5},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_17","agentId":"victor","title":"Uražená hrdost","description":"Jeden blbec se chtěl zviditelnit na cizím jménu. Teď mu vysvětli, že to byl drahej nápad.","difficulty":"medium","successRate":74,"durationMinutes":21,"reward":{"influence":6,"dirty-cash":1100},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_18","agentId":"victor","title":"Pád z nákladní rampy","description":"Na rampě stojí zboží, co má změnit majitele. Vezmi ho a nic neřeš.","difficulty":"medium","successRate":81,"durationMinutes":19,"reward":{"metal-parts":6,"cash":400},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_19","agentId":"victor","title":"Hlasitý vzkaz","description":"Někdy nestačí někoho okrást. Někdy musí celej blok vědět, kdo to udělal.","difficulty":"hard","successRate":64,"durationMinutes":24,"reward":{"influence":8,"cash":900},"risk":{"successHeat":4,"failureHeat":10,"failureDirtyCashLoss":400}},{"id":"victor_20","agentId":"victor","title":"Mokrý prachy","description":"U vody čeká malej přesun peněz. Když se zdržíš, někdo jiný si namočí ruce místo tebe.","difficulty":"hard","successRate":71,"durationMinutes":22,"reward":{"dirty-cash":2200,"influence":2},"risk":{"successHeat":4,"failureHeat":10,"failureDirtyCashLoss":400}},{"id":"victor_21","agentId":"victor","title":"Starý dluh, nová bolest","description":"Starej dluh se dnes zavře. Otázka je jen, jestli penězma nebo zubama.","difficulty":"medium","successRate":79,"durationMinutes":20,"reward":{"cash":1600,"influence":4},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_22","agentId":"victor","title":"Kontejner číslo 9","description":"V kontejneru číslo 9 leží věci, co nemají vidět ráno. Otevři ho dřív než ostatní.","difficulty":"medium","successRate":76,"durationMinutes":18,"reward":{"metal-parts":3,"cash":700},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_23","agentId":"victor","title":"Měkký cíl","description":"Slabej článek řetězu bývá nejlevnější cesta dovnitř. Využij to.","difficulty":"medium","successRate":85,"durationMinutes":16,"reward":{"influence":6,"cash":500},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_24","agentId":"victor","title":"Velvet Smoke v kufru","description":"V kufru čeká pár balení Velvet Smoke. Převezmi to, než se z toho stane cizí zisk.","difficulty":"medium","successRate":84,"durationMinutes":15,"reward":{"cash":500,"influence":2},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_25","agentId":"victor","title":"Křik v zadní uličce","description":"Zadní uličky jsou moje kancelář. Dneska tam někomu zrušíš pracovní poměr.","difficulty":"medium","successRate":73,"durationMinutes":19,"reward":{"dirty-cash":1200,"influence":5},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_26","agentId":"victor","title":"Rychlý výkup","description":"Jeden zoufalec prodává materiál hluboko pod cenou. Seber to všechno, než dostane rozum.","difficulty":"easy","successRate":88,"durationMinutes":12,"reward":{"metal-parts":4},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"victor_27","agentId":"victor","title":"Tlak na rohu","description":"Na jednom rohu se rozdává respekt zadarmo. To je chyba, kterou dnes opravíš.","difficulty":"medium","successRate":80,"durationMinutes":17,"reward":{"influence":6,"cash":800},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_28","agentId":"victor","title":"Spadlá bedna","description":"Někde spadla bedna z transportu. Kdo ji najde první, ten určuje pravidla.","difficulty":"medium","successRate":77,"durationMinutes":15,"reward":{"metal-parts":2},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_29","agentId":"victor","title":"Přepálená dávka","description":"Někdo vaří moc nahlas a moc blízko. Vlez tam, seber vzorek a zbytek nech rozpadnout.","difficulty":"hard","successRate":69,"durationMinutes":23,"reward":{"influence":3},"risk":{"successHeat":4,"failureHeat":10,"failureDirtyCashLoss":400}},{"id":"victor_30","agentId":"victor","title":"Drobní, ale naši","description":"Malí dealeři se začínají dívat jinam. Připomeň jim, kde končí každá ulice.","difficulty":"medium","successRate":78,"durationMinutes":20,"reward":{"cash":1300,"influence":6},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_31","agentId":"victor","title":"Tichá zbroj","description":"Mám kontakt na vybavení, co nechodí přes papíry. Vyber to a neotáčej se.","difficulty":"medium","successRate":82,"durationMinutes":16,"reward":{"dirty-cash":500},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_32","agentId":"victor","title":"Vyděšený účetní","description":"Jeden účetní ví, kam tečou peníze. A dneska bude chtít mluvit rychle.","difficulty":"hard","successRate":72,"durationMinutes":22,"reward":{"dirty-cash":1700,"influence":6},"risk":{"successHeat":4,"failureHeat":10,"failureDirtyCashLoss":400}},{"id":"victor_33","agentId":"victor","title":"První rána zdarma","description":"Dneska nejde o kořist. Dneska jde o to, kdo dá první ránu a kdo si ji zapamatuje.","difficulty":"hard","successRate":67,"durationMinutes":22,"reward":{"influence":8,"cash":600},"risk":{"successHeat":4,"failureHeat":10,"failureDirtyCashLoss":400}},{"id":"victor_34","agentId":"victor","title":"Rozbitý automat","description":"Někdo schovává peníze tam, kde si myslí, že vypadají nevinně. Rozbij to a vezmi obsah.","difficulty":"medium","successRate":85,"durationMinutes":15,"reward":{"cash":1200,"dirty-cash":400},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_35","agentId":"victor","title":"Krátká návštěva v docku","description":"V doku kotví něco, co tam nemá vydržet do rána. Přesuneme to dřív.","difficulty":"medium","successRate":74,"durationMinutes":21,"reward":{"cash":700},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_36","agentId":"victor","title":"Neonový nátlak","description":"Jeden klub má dneska přinést víc než hudbu. Vlez tam, zatlač a vytáhni z toho hodnotu.","difficulty":"hard","successRate":68,"durationMinutes":25,"reward":{"dirty-cash":1900,"influence":3},"risk":{"successHeat":4,"failureHeat":10,"failureDirtyCashLoss":400}},{"id":"victor_37","agentId":"victor","title":"Kov a krev","description":"Tam, kde je kov, bývá i zisk. Tam, kde je zisk, bývá i problém. Dneska si vezmeš oboje.","difficulty":"medium","successRate":80,"durationMinutes":18,"reward":{"metal-parts":6,"cash":400,"influence":2},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_38","agentId":"victor","title":"Tlačenice u zadního vstupu","description":"Zadní vstup je vždycky levnější než fronta. A mnohem výnosnější.","difficulty":"hard","successRate":71,"durationMinutes":22,"reward":{"cash":500,"influence":3},"risk":{"successHeat":4,"failureHeat":10,"failureDirtyCashLoss":400}},{"id":"victor_39","agentId":"victor","title":"Stůl pro dva, problém pro jednoho","description":"V restauraci proběhne schůzka. Ty se postaráš, aby domů neodnesli všechno, co přinesli.","difficulty":"medium","successRate":77,"durationMinutes":21,"reward":{"cash":1600,"influence":4},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_40","agentId":"victor","title":"Zkušební tlak","description":"Chci vidět, jak makáš, když tě někdo tlačí do zdi. Tahle práce je přesně na to.","difficulty":"medium","successRate":83,"durationMinutes":15,"reward":{"influence":6,"cash":400},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_41","agentId":"victor","title":"Bazuka ve stínu","description":"Někdo schoval těžší kus železa tam, kde se bojí pro něj vrátit. To není náš problém.","difficulty":"rare","successRate":58,"durationMinutes":30,"reward":{"combat-module":1,"dirty-cash":500},"risk":{"successHeat":6,"failureHeat":12,"failureDirtyCashLoss":600}},{"id":"victor_42","agentId":"victor","title":"Hluk před bouří","description":"Celý blok je nervózní. To je nejlepší chvíle sebrat to, co není přibitý.","difficulty":"medium","successRate":79,"durationMinutes":18,"reward":{"cash":1000,"metal-parts":4,"influence":2},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_43","agentId":"victor","title":"Ztracený kamerový záznam","description":"Někdo si myslí, že ho chrání záznam. Zmizí záznam, zmizí i jeho jistota.","difficulty":"medium","successRate":84,"durationMinutes":15,"reward":{"influence":2,"cash":500},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_44","agentId":"victor","title":"Převzetí směny","description":"Končí směna, začíná chaos. Přesně v tom chaosu vyděláš nejvíc.","difficulty":"medium","successRate":81,"durationMinutes":17,"reward":{"dirty-cash":1600,"influence":3},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_45","agentId":"victor","title":"Když se nikdo neptá","description":"Tohle je ten druh práce, kde nikdo nic neviděl a nikdo nic neví. Mám tyhle práce rád.","difficulty":"easy","successRate":87,"durationMinutes":13,"reward":{"cash":600,"influence":2},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"victor_46","agentId":"victor","title":"Rozpal ulici","description":"Dneska nechci čistou práci. Dneska chci, aby se o tom mluvilo ještě zítra ráno.","difficulty":"rare","successRate":60,"durationMinutes":26,"reward":{"smg":1,"dirty-cash":800},"risk":{"successHeat":6,"failureHeat":12,"failureDirtyCashLoss":600}},{"id":"victor_47","agentId":"victor","title":"Pod pultem","description":"Jeden obchod má vzadu něco lepšího než ve výloze. Jdi si pro to.","difficulty":"medium","successRate":85,"durationMinutes":15,"reward":{"cash":400},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_48","agentId":"victor","title":"Zlomený alarm","description":"Alarm se dá vypnout dvěma způsoby. Já mám radši ten hlučnější.","difficulty":"medium","successRate":73,"durationMinutes":19,"reward":{"metal-parts":3,"cash":500},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_49","agentId":"victor","title":"Síla bez omluvy","description":"Někdy je plán přeceňovanej. Vleť tam, udělej tlak a odejdi silnější než jsi přišel.","difficulty":"hard","successRate":69,"durationMinutes":22,"reward":{"influence":8,"cash":900,"pistol":1},"risk":{"successHeat":4,"failureHeat":10,"failureDirtyCashLoss":400}},{"id":"victor_50","agentId":"victor","title":"Ulice si pamatuje","description":"Tohle není jen práce. Tohle je podpis. Udělej to tak, aby si město zapamatovalo, kdo tady určuje rytmus.","difficulty":"hard","successRate":63,"durationMinutes":29,"reward":{"influence":8,"dirty-cash":1400},"risk":{"successHeat":4,"failureHeat":10,"failureDirtyCashLoss":400}},{"id":"victor_51","agentId":"victor","title":"Rozkopnutý sklad","description":"Někdo si myslí, že plechové dveře znamenají bezpečí. Dneska zjistí, že jsou to jen dražší třísky.","difficulty":"medium","successRate":79,"durationMinutes":19,"reward":{"metal-parts":6,"cash":400,"influence":3},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_52","agentId":"victor","title":"Cizí roh","description":"Na našem území si někdo staví vlastní jméno. Sejmi tu pohádku dřív, než jí někdo uvěří.","difficulty":"medium","successRate":75,"durationMinutes":21,"reward":{"influence":6,"dirty-cash":1000},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_53","agentId":"victor","title":"Závora dolů","description":"Jeden vjezd se dnes na chvíli zavře. A všechno, co zůstane uvnitř, bude naše.","difficulty":"medium","successRate":81,"durationMinutes":18,"reward":{"metal-parts":4,"cash":700},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_54","agentId":"victor","title":"Ruce na kapotu","description":"Na parkovišti stojí auto, co veze víc než plech. Otevři ho a vyber, co se hodí.","difficulty":"medium","successRate":77,"durationMinutes":17,"reward":{"cash":800,"influence":2},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_55","agentId":"victor","title":"Krátká porada","description":"Jeden chytrák potřebuje vysvětlit realitu. Ty budeš ten výukový materiál.","difficulty":"medium","successRate":84,"durationMinutes":15,"reward":{"influence":6,"cash":900},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_56","agentId":"victor","title":"Prachy v mrazáku","description":"Někteří lidi schovávají peníze vedle masa. Dneska rozmrazíš jejich jistoty.","difficulty":"medium","successRate":80,"durationMinutes":16,"reward":{"dirty-cash":1600,"influence":2},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_57","agentId":"victor","title":"Přeložená zásilka","description":"Jedna bedna má změnit adresu dřív, než změří teplotu skladu. Nezdržuj se.","difficulty":"medium","successRate":85,"durationMinutes":15,"reward":{"cash":600,"influence":2},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_58","agentId":"victor","title":"Těžká pěst","description":"Někde nestačí mluvit. Někde musíš nechat odpověď otisknutou ve zdi.","difficulty":"hard","successRate":72,"durationMinutes":22,"reward":{"influence":8,"dirty-cash":700},"risk":{"successHeat":4,"failureHeat":10,"failureDirtyCashLoss":400}},{"id":"victor_59","agentId":"victor","title":"Chybná směna","description":"Ve směnárně mají dneska špatný kurz. Pro ně. Pro nás je to výdělek.","difficulty":"medium","successRate":78,"durationMinutes":18,"reward":{"cash":1163,"dirty-cash":436},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_60","agentId":"victor","title":"Noční inventura","description":"V noci se nejlíp počítá cizí majetek. Zvlášť když si ho ráno už nikdo nespočítá.","difficulty":"medium","successRate":79,"durationMinutes":19,"reward":{"metal-parts":5,"cash":700},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_61","agentId":"victor","title":"Rozlitá krev, čistý zisk","description":"Někdo si chtěl hrát na tvrdýho. Nech mu tvrdou lekci a měkký kolena.","difficulty":"hard","successRate":68,"durationMinutes":23,"reward":{"influence":8,"cash":1000},"risk":{"successHeat":4,"failureHeat":10,"failureDirtyCashLoss":400}},{"id":"victor_62","agentId":"victor","title":"Balík pod mostem","description":"Pod mostem čeká balík bez majitele. A když ho nevezmeš ty, vezme ho někdo rychlejší.","difficulty":"easy","successRate":86,"durationMinutes":13,"reward":{"cash":500,"influence":2},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"victor_63","agentId":"victor","title":"Vymáhání po staru","description":"Ten dluh je malej jen na papíře. Udělej z něj velkej problém, dokud nebude splacenej.","difficulty":"medium","successRate":80,"durationMinutes":17,"reward":{"cash":1500,"influence":5},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_64","agentId":"victor","title":"Přístup jen pro tvrdé","description":"Za zadním vstupem leží věci pro lidi bez skrupulí. Tak tam běž jako domů.","difficulty":"medium","successRate":76,"durationMinutes":16,"reward":{"metal-parts":3},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_65","agentId":"victor","title":"Vybitý kamerový dohled","description":"Někdo se spoléhá na kamery. Dneska mu ukážeš, že kabely křičí míň než lidi.","difficulty":"medium","successRate":83,"durationMinutes":15,"reward":{"influence":3,"cash":400},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_66","agentId":"victor","title":"Tři minuty strachu","description":"Stačí tři minuty a celej blok začne šeptat. Udělej z nich dlouhý tři minuty.","difficulty":"hard","successRate":69,"durationMinutes":22,"reward":{"influence":8,"dirty-cash":900},"risk":{"successHeat":4,"failureHeat":10,"failureDirtyCashLoss":400}},{"id":"victor_67","agentId":"victor","title":"Lékárna po zavíračce","description":"Po zavíračce zůstává uvnitř víc než jen světla. Posbírej to, co má cenu.","difficulty":"medium","successRate":81,"durationMinutes":18,"reward":{"cash":500},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_68","agentId":"victor","title":"Narušený deal","description":"Dva lidi se chtějí domluvit bez nás. To je chyba, kterou je potřeba zpeněžit.","difficulty":"medium","successRate":74,"durationMinutes":20,"reward":{"dirty-cash":1600,"influence":3},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_69","agentId":"victor","title":"Otevřený kufr","description":"Kufr je otevřenej, nervy taky. Vezmi všechno, co uneseš, a zmiz dřív než cvakne zámek.","difficulty":"medium","successRate":78,"durationMinutes":15,"reward":{"cash":700,"influence":2},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_70","agentId":"victor","title":"Směna skončila","description":"Když lidi končí směnu, dělají chyby. Ty na těch chybách dneska vyděláš.","difficulty":"medium","successRate":85,"durationMinutes":15,"reward":{"cash":1000,"metal-parts":4},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_71","agentId":"victor","title":"Přetlačená ulice","description":"Na tý ulici je moc cizích ramen a málo našeho jména. Vyrovnej to.","difficulty":"medium","successRate":76,"durationMinutes":19,"reward":{"influence":6,"cash":900},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_72","agentId":"victor","title":"Náklad bez pojištění","description":"Jeden převoz nemá ochranu ani štěstí. Přesně takový věci mám rád.","difficulty":"medium","successRate":80,"durationMinutes":18,"reward":{"cash":600},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_73","agentId":"victor","title":"Páka na účetního","description":"Účetní nejsou tvrdí. Jen vypadají draze. Stlač ho a pustí víc, než čekáš.","difficulty":"medium","successRate":79,"durationMinutes":17,"reward":{"dirty-cash":1500,"influence":4},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_74","agentId":"victor","title":"Betonová lekce","description":"Dneska někdo pochopí, že beton je tvrdší než jeho ego. Ty budeš ten překlad.","difficulty":"hard","successRate":67,"durationMinutes":22,"reward":{"influence":8,"cash":800},"risk":{"successHeat":4,"failureHeat":10,"failureDirtyCashLoss":400}},{"id":"victor_75","agentId":"victor","title":"Špinavý schody","description":"Ve vchodu se schází lidi, co zapomněli platit za klid. Připomeň jim sazebník.","difficulty":"medium","successRate":82,"durationMinutes":16,"reward":{"cash":1244,"dirty-cash":355,"influence":3},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_76","agentId":"victor","title":"Nedodaná bedna","description":"Jeden zákazník dneska nic nedostane. Protože všechno skončí v tvých rukách.","difficulty":"medium","successRate":73,"durationMinutes":19,"reward":{"metal-parts":2},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_77","agentId":"victor","title":"Přesun pod tlakem","description":"Musíš dostat zásobu přes místo, kde všichni čumí. To je přesně chvíle, kdy se pozná, kdo má nervy.","difficulty":"hard","successRate":71,"durationMinutes":22,"reward":{"cash":600,"influence":3},"risk":{"successHeat":4,"failureHeat":10,"failureDirtyCashLoss":400}},{"id":"victor_78","agentId":"victor","title":"Vyrvaná jistota","description":"Jeden člověk je moc v pohodě. A pohodlí na ulici bývá dočasná věc.","difficulty":"medium","successRate":81,"durationMinutes":15,"reward":{"influence":6,"dirty-cash":1000},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_79","agentId":"victor","title":"Nabouraný převoz","description":"Nehoda se dá zařídit různě. Hlavní je, aby po ní zůstalo něco použitelného.","difficulty":"hard","successRate":72,"durationMinutes":22,"reward":{"metal-parts":6,"cash":700,"influence":2},"risk":{"successHeat":4,"failureHeat":10,"failureDirtyCashLoss":400}},{"id":"victor_80","agentId":"victor","title":"Půlnoční výběrčí","description":"Po půlnoci bývají lidi štědřejší. Hlavně když mají důvod se bát odmítnout.","difficulty":"medium","successRate":75,"durationMinutes":18,"reward":{"cash":1600,"influence":4},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_81","agentId":"victor","title":"Cizí železo","description":"V dílně zůstalo pár kusů železa bez dozoru. Tak tam nechoď pro dovolení.","difficulty":"medium","successRate":84,"durationMinutes":15,"reward":{"metal-parts":6,"cash":400},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_82","agentId":"victor","title":"Vysoký tlak, nízký hlas","description":"Někdo mluví moc nahlas o věcech, co by měly zůstat pod stolem. Ztiš ho.","difficulty":"medium","successRate":74,"durationMinutes":17,"reward":{"influence":6,"dirty-cash":800},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_83","agentId":"victor","title":"Slepý roh","description":"Na slepým rohu se dneska ztratí jedna zásilka a několik iluzí.","difficulty":"medium","successRate":80,"durationMinutes":16,"reward":{"cash":600,"influence":2},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_84","agentId":"victor","title":"Rozjetej motor","description":"Motor běží, řidič je nervózní a náklad je cennej. Stačí být rychlejší než panika.","difficulty":"medium","successRate":79,"durationMinutes":18,"reward":{"cash":1200,"influence":2},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_85","agentId":"victor","title":"Pevná ruka","description":"Někdy je rozdíl mezi chaosem a respektem jen v tom, kdo drží situaci za krk.","difficulty":"hard","successRate":66,"durationMinutes":22,"reward":{"influence":8,"cash":700},"risk":{"successHeat":4,"failureHeat":10,"failureDirtyCashLoss":400}},{"id":"victor_86","agentId":"victor","title":"Podlomený obchod","description":"Jeden podnik dneska vydělá míň, než čekal. Protože část zisku půjde domů s tebou.","difficulty":"medium","successRate":81,"durationMinutes":17,"reward":{"dirty-cash":1219,"cash":380,"influence":2},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_87","agentId":"victor","title":"Špatně zamčený box","description":"Box je zamčenej jen pro slušný lidi. Ty tam nejdeš slušně.","difficulty":"medium","successRate":77,"durationMinutes":15,"reward":{"dirty-cash":500},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_88","agentId":"victor","title":"Řeči z okna","description":"Někdo se dívá z okna a myslí si, že je mimo hru. Připomeň mu, že ulice sahá výš.","difficulty":"medium","successRate":73,"durationMinutes":18,"reward":{"influence":6,"cash":800},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_89","agentId":"victor","title":"Kyselý náklad","description":"Jedna várka chemie se má ztratit cestou. Tak jí pomoz zmizet správným směrem.","difficulty":"medium","successRate":82,"durationMinutes":16,"reward":{"cash":500,"influence":2},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_90","agentId":"victor","title":"Příliš klidný klub","description":"Ten klub je dneska až moc v klidu. Udělej tam takovej tlak, aby se začalo platit za ticho.","difficulty":"hard","successRate":70,"durationMinutes":23,"reward":{"dirty-cash":1900,"influence":4},"risk":{"successHeat":4,"failureHeat":10,"failureDirtyCashLoss":400}},{"id":"victor_91","agentId":"victor","title":"Rozbitý stůl","description":"Když se rozbije stůl, často se otevřou i kapsy. Využij obojí.","difficulty":"medium","successRate":80,"durationMinutes":16,"reward":{"cash":1400,"influence":3},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_92","agentId":"victor","title":"Přeseknutá dohoda","description":"Někdo si myslí, že může obchodovat bez povolení. Dneska zjistí, že povolení vypadá jako ty.","difficulty":"medium","successRate":75,"durationMinutes":19,"reward":{"influence":6,"dirty-cash":900},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_93","agentId":"victor","title":"Kufr plný problémů","description":"V kufru je víc problémů než oblečení. Otevři ho a změň problémy na zásoby.","difficulty":"hard","successRate":65,"durationMinutes":24,"reward":{"metal-parts":2,"influence":3},"risk":{"successHeat":4,"failureHeat":10,"failureDirtyCashLoss":400}},{"id":"victor_94","agentId":"victor","title":"Kroky ve skladu","description":"Sklad dneska nebude tichej. A po tvým odchodu nebude ani plnej.","difficulty":"medium","successRate":81,"durationMinutes":18,"reward":{"metal-parts":5,"cash":700},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_95","agentId":"victor","title":"Poslední varování","description":"Někdo už jedno varování dostal. Teď dostane takový, co se nedá přeslechnout.","difficulty":"hard","successRate":68,"durationMinutes":22,"reward":{"influence":8,"cash":900},"risk":{"successHeat":4,"failureHeat":10,"failureDirtyCashLoss":400}},{"id":"victor_96","agentId":"victor","title":"Vlhký bankovky","description":"U přístavu se dneska lepí bankovky na špatný ruce. Ty máš zařídit, aby se lepily na správný.","difficulty":"hard","successRate":72,"durationMinutes":22,"reward":{"dirty-cash":2100,"influence":3},"risk":{"successHeat":4,"failureHeat":10,"failureDirtyCashLoss":400}},{"id":"victor_97","agentId":"victor","title":"Odtržená směna","description":"Jedna parta dneska nedokončí směnu v pohodě. A ty z toho vytáhneš, co půjde.","difficulty":"medium","successRate":82,"durationMinutes":17,"reward":{"cash":1000,"metal-parts":4,"influence":2},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_98","agentId":"victor","title":"Tvrdý přepočet","description":"Když se špatně přepočítáš na ulici, někdo jiný si to spočítá za tebe. Jdi jim pomoct s matematikou.","difficulty":"medium","successRate":76,"durationMinutes":18,"reward":{"cash":1500,"influence":5},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_99","agentId":"victor","title":"Díra v plotě","description":"Každý plot má slabý místo. A za každým slabým místem bývá něco, co se dá odnést.","difficulty":"medium","successRate":84,"durationMinutes":15,"reward":{"cash":600},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"victor_100","agentId":"victor","title":"Victorův podpis","description":"Tohle není jen další práce. Tohle je připomínka všem v okolí, kdo má v ulicích poslední slovo.","difficulty":"hard","successRate":62,"durationMinutes":27,"reward":{"influence":8,"dirty-cash":1500,"metal-parts":3},"risk":{"successHeat":4,"failureHeat":10,"failureDirtyCashLoss":400}},{"id":"leon_01","agentId":"leon","title":"Kšeft z kufru","description":"Na parkovišti stojí kufr plnej věcí, co oficiálně neexistujou. Přijeď, zaplať správně a zmiz dřív, než se někdo začne ptát.","difficulty":"medium","successRate":85,"durationMinutes":15,"reward":{"metal-parts":4,"chemicals":2},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_02","agentId":"leon","title":"Levný zboží, drahý následky","description":"Mám deal, kterej smrdí už z dálky. Ale marže je krásná. Vem to, než to někdo vyžere před tebou.","difficulty":"medium","successRate":80,"durationMinutes":16,"reward":{"cash":1120,"dirty-cash":480},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_03","agentId":"leon","title":"Kontakt ze zadní uličky","description":"Jeden můj kontakt chce mluvit jen venku, mezi odpadkama a špínou. Což většinou znamená, že nabídka stojí za to.","difficulty":"medium","successRate":84,"durationMinutes":15,"reward":{"influence":4,"chemicals":4,"cash":400},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_04","agentId":"leon","title":"Přeprodej bez otázek","description":"Dostaneš zboží. Neptáš se odkud je. Jen ho otočíš rychle a draze. Přesně tak se vydělává ve městě.","difficulty":"easy","successRate":87,"durationMinutes":13,"reward":{"cash":1000,"influence":3},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"leon_05","agentId":"leon","title":"Špinavý kontakt","description":"Jeden kontakt je nervózní a chce se něčeho zbavit. Ty budeš ten, kdo mu uleví od nákladu i od peněz.","difficulty":"medium","successRate":79,"durationMinutes":17,"reward":{"cash":700,"influence":2},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_06","agentId":"leon","title":"Zboží pod pultem","description":"Ve výloze nic není. Ale pod pultem leží věci, kvůli kterým se vyplatí přijít zadním vchodem.","difficulty":"medium","successRate":78,"durationMinutes":16,"reward":{"cash":500,"influence":2},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_07","agentId":"leon","title":"Rychlá otočka","description":"Koupíš levně, prodáš rychle, zmizíš dřív, než někdo zjistí, že byl právě obranej. Klasika.","difficulty":"easy","successRate":89,"durationMinutes":12,"reward":{"cash":789,"dirty-cash":210},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"leon_08","agentId":"leon","title":"Zásilka bez jména","description":"Přijde bedna bez jména, bez papírů a bez výmluv. To bývají ty nejlepší obchody.","difficulty":"easy","successRate":86,"durationMinutes":14,"reward":{"metal-parts":3},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"leon_09","agentId":"leon","title":"Přehazovačka","description":"Dvě party si mají předat zboží. Ty zařídíš, aby po cestě změnilo majitele i cenu.","difficulty":"medium","successRate":77,"durationMinutes":18,"reward":{"dirty-cash":1600,"influence":3},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_10","agentId":"leon","title":"Sleva za ticho","description":"Jeden prodejce udělá hezkou cenu. Protože ví, že když ji neudělá, může přestat prodávat úplně.","difficulty":"medium","successRate":85,"durationMinutes":15,"reward":{"velvet-smoke":1,"influence":3},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_11","agentId":"leon","title":"Noční burza","description":"Po půlnoci se otevírá trh pro lidi, co nechtějí účtenky. Tam chodí skutečný peníze.","difficulty":"medium","successRate":81,"durationMinutes":17,"reward":{"cash":1252,"dirty-cash":347},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_12","agentId":"leon","title":"Falešný prostředník","description":"Jedna schůzka potřebuje prostředníka. Ty budeš ten prostředník. A taky ten, kdo si ukousne největší část.","difficulty":"easy","successRate":86,"durationMinutes":14,"reward":{"influence":3,"cash":1000},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"leon_13","agentId":"leon","title":"Drahá adresa","description":"Někdy neprodáváš zboží. Někdy prodáváš jen to, že víš, kam jít a na koho zatlačit.","difficulty":"medium","successRate":84,"durationMinutes":15,"reward":{"influence":6,"dirty-cash":700},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_14","agentId":"leon","title":"Kradený kov","description":"Mám partu, co tahá kov z míst, kde už ho nikdo nebude postrádat. Otoč to na trhu, než vystydne stopa.","difficulty":"medium","successRate":82,"durationMinutes":16,"reward":{"metal-parts":6,"cash":400},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_15","agentId":"leon","title":"Kapsy plný marže","description":"Dneska nejde o sílu. Dneska jde o to, kdo vytěží víc z cizí blbosti. A to jsi ty.","difficulty":"easy","successRate":90,"durationMinutes":12,"reward":{"cash":1000,"influence":3},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"leon_16","agentId":"leon","title":"Podivný léky","description":"Někdo prodává farmaceutický zásoby bokem. Kvalita pochybná, zisk krásnej. Takže to bereme.","difficulty":"medium","successRate":80,"durationMinutes":18,"reward":{"chemicals":6,"cash":40},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_17","agentId":"leon","title":"Překupník v běhu","description":"Jeden malej překupník panikaří a chce všechno střelit hned. Vezmi mu to za směšnou cenu.","difficulty":"easy","successRate":91,"durationMinutes":11,"reward":{"cash":300,"metal-parts":3,"influence":2},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"leon_18","agentId":"leon","title":"Druhá ruka, první zisk","description":"Tohle zboží už někdo vlastnil. A teď ho budeš vlastnit ty. Krátce. Než ho prodáš ještě dráž.","difficulty":"medium","successRate":78,"durationMinutes":16,"reward":{"dirty-cash":900,"influence":2},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_19","agentId":"leon","title":"Směna ve tmě","description":"Žádný světla, žádný jména, žádný potvrzení. Jen deal a rychlý ruce.","difficulty":"medium","successRate":85,"durationMinutes":15,"reward":{"velvet-smoke":1},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_20","agentId":"leon","title":"Kontakt z druhý strany města","description":"Mám tip z části města, kam normálně nechodíš. Což je přesně důvod, proč tam leží prachy.","difficulty":"medium","successRate":83,"durationMinutes":15,"reward":{"influence":5,"cash":1100},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_21","agentId":"leon","title":"Nadupaná přirážka","description":"Někdo něco zoufale potřebuje. A zoufalství je jen jiný slovo pro vyšší cenu.","difficulty":"easy","successRate":88,"durationMinutes":13,"reward":{"cash":1000,"influence":3},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"leon_22","agentId":"leon","title":"Kšeft mezi popelnicema","description":"Když se velký peníze řeší mezi popelnicema, většinou z toho něco kápne i bokem. Dneska hodně.","difficulty":"medium","successRate":79,"durationMinutes":17,"reward":{"dirty-cash":1480,"chemicals":2},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_23","agentId":"leon","title":"Dveře bez cedule","description":"Za jedněma neoznačenýma dveřma čeká nabídka, co se nebude opakovat. Buď první uvnitř.","difficulty":"medium","successRate":77,"durationMinutes":18,"reward":{"tech-core":1,"influence":2},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_24","agentId":"leon","title":"Tichý přepočet","description":"Někdo se přepočítal v náš prospěch. A ty mu teď pomůžeš tu chybu už nenapravit.","difficulty":"easy","successRate":89,"durationMinutes":12,"reward":{"cash":750,"dirty-cash":250},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"leon_25","agentId":"leon","title":"Otoč to, než to shnije","description":"Jedna várka je horká, jedna špinavá a jedna se kazí. Neřeš která je která. Prostě to otoč.","difficulty":"medium","successRate":81,"durationMinutes":16,"reward":{"chemicals":4},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_26","agentId":"leon","title":"Fix za fixem","description":"Dneska neprodáváš věc. Dneska prodáváš řešení. A řešení ve městě bývají dražší než kulky.","difficulty":"medium","successRate":85,"durationMinutes":15,"reward":{"influence":6,"cash":1200},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_27","agentId":"leon","title":"Sektorovej překup","description":"Co je levný v jednom sektoru, je drahý v druhým. A ty budeš ten most mezi chamtivostí a nouzí.","difficulty":"easy","successRate":86,"durationMinutes":14,"reward":{"cash":1000,"influence":3},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"leon_28","agentId":"leon","title":"Balíček pro nervózního klienta","description":"Klient chce diskrétnost. To znamená vyšší cenu a rychlejší nohy. Obojí máš.","difficulty":"medium","successRate":80,"durationMinutes":17,"reward":{"cash":800,"influence":2},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_29","agentId":"leon","title":"Cizí problém, náš zisk","description":"Někdo má moc zboží, málo času a nulovou páteř. Přesně z takových se žije nejlíp.","difficulty":"medium","successRate":84,"durationMinutes":15,"reward":{"metal-parts":5,"cash":700,"influence":3},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_30","agentId":"leon","title":"Pouliční licence","description":"Na tomhle bloku nikdo neprodává bez toho, aby něco neodvedl. Dneska vybíráš ty.","difficulty":"medium","successRate":82,"durationMinutes":16,"reward":{"cash":1120,"dirty-cash":480,"influence":4},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_31","agentId":"leon","title":"Zlomený řetězec","description":"Jeden dodavatelský řetězec právě praskl. A ty posbíráš, co z něj vypadne na zem.","difficulty":"medium","successRate":85,"durationMinutes":15,"reward":{"chemicals":5},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_32","agentId":"leon","title":"Přestřelená cena","description":"Někdo chce moc. Ty mu zaplatíš málo. A ještě na tom vyděláš. Tomu říkám obchod.","difficulty":"easy","successRate":90,"durationMinutes":11,"reward":{"cash":1000,"influence":3},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"leon_33","agentId":"leon","title":"Tichý runner","description":"Potřebuju, aby něco přešlo přes tři bloky a nikdo to nezastavil. Žádná sláva, jen čistý profit.","difficulty":"easy","successRate":86,"durationMinutes":13,"reward":{"cash":1000,"influence":2},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"leon_34","agentId":"leon","title":"Výprodej strachu","description":"Když začne někdo panikařit, prodává hluboko pod cenou. A my jsme přesně ti, co to umí využít.","difficulty":"easy","successRate":91,"durationMinutes":12,"reward":{"metal-parts":4},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"leon_35","agentId":"leon","title":"Spodní police","description":"To nejlepší zboží nebývá na očích. Bývá dole, za plentou, mezi věcma bez původu.","difficulty":"medium","successRate":79,"durationMinutes":16,"reward":{"velvet-smoke":1},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_36","agentId":"leon","title":"Dohoda na rohu","description":"Na rohu čeká deal. Malej stůl, špinavý ruce, velký peníze. Nezvor to.","difficulty":"medium","successRate":80,"durationMinutes":17,"reward":{"dirty-cash":1600,"influence":3},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_37","agentId":"leon","title":"Sběrač marže","description":"Dva blbci se hádají o cenu. Ty přijdeš mezi ně a odejdeš s největším kusem.","difficulty":"easy","successRate":87,"durationMinutes":13,"reward":{"cash":1000,"influence":3},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"leon_38","agentId":"leon","title":"Krabice bez původu","description":"Mám tři krabice. Jedna je legální, druhá ne a třetí je nejlepší neotvírat. Vezmi všechny.","difficulty":"medium","successRate":78,"durationMinutes":18,"reward":{"chemicals":3,"metal-parts":3,"cash":220},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_39","agentId":"leon","title":"Pouliční arbitráž","description":"Jedna strana má zboží, druhá peníze a obě mají málo mozku. Ty z toho vytěžíš nejvíc.","difficulty":"easy","successRate":88,"durationMinutes":12,"reward":{"cash":818,"dirty-cash":181},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"leon_40","agentId":"leon","title":"Tlačenice o bedny","description":"Došlo pár beden a pár lidí po nich skočí. Ty skočíš rychlejc a prodáš je s přirážkou.","difficulty":"medium","successRate":79,"durationMinutes":16,"reward":{"tech-core":1,"influence":2},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_41","agentId":"leon","title":"Klient bez nervů","description":"Můj klient se sype a chce všechno hned. Tak mu to dej. Ale draho.","difficulty":"easy","successRate":86,"durationMinutes":13,"reward":{"cash":750,"dirty-cash":250},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"leon_42","agentId":"leon","title":"Zadní schodiště","description":"Na zadním schodišti se dneska budou měnit ruce, kapsy a loajalita. Buď u toho první.","difficulty":"medium","successRate":84,"durationMinutes":15,"reward":{"influence":5,"cash":1000},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_43","agentId":"leon","title":"Přeskládání trhu","description":"Jedna várka zmizí z trhu a jiná se objeví za dvojnásobek. Krása volný ulice.","difficulty":"medium","successRate":81,"durationMinutes":17,"reward":{"cash":1600,"influence":3},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_44","agentId":"leon","title":"Vypůjčený sklad","description":"Na pár minut si půjčíš cizí sklad. Na pár hodin z něj budeš žít.","difficulty":"medium","successRate":80,"durationMinutes":18,"reward":{"metal-parts":6,"chemicals":1},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_45","agentId":"leon","title":"Nelegální přirážka","description":"Některý věci jsou drahý proto, že jsou vzácný. Jiný proto, že za ně můžeš skončit v problému. Tohle je ten druhý případ.","difficulty":"medium","successRate":74,"durationMinutes":19,"reward":{"cash":800,"influence":3},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_46","agentId":"leon","title":"Šeptaná nabídka","description":"Když někdo šeptá cenu, většinou ví, že je buď moc dobrá, nebo moc špinavá. Mně jsou sympatický obě možnosti.","difficulty":"medium","successRate":80,"durationMinutes":15,"reward":{"dirty-cash":1500,"influence":2},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_47","agentId":"leon","title":"Pouliční broker","description":"Dneska seš prostředník mezi hladovejma rukama a plným bednama. A prostředník bere vždycky první kus.","difficulty":"easy","successRate":87,"durationMinutes":13,"reward":{"cash":1000,"influence":3},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"leon_48","agentId":"leon","title":"Přepálený zájem","description":"Když někdo něco chce až moc, přestává řešit cenu. A přesně v tu chvíli přicházíš ty.","difficulty":"easy","successRate":89,"durationMinutes":12,"reward":{"cash":1000,"influence":3},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"leon_49","agentId":"leon","title":"Černý seznam kontaktů","description":"Mám seznam jmen, adres a slabin. Nechci ho celý. Stačí mi, když z něj vytěžíš maximum.","difficulty":"medium","successRate":82,"durationMinutes":16,"reward":{"influence":6,"dirty-cash":900},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_50","agentId":"leon","title":"Leonův řez","description":"Pamatuj si to. V tomhle městě nevyhrává ten, kdo něco má. Vyhrává ten, kdo si z každýho kšeftu ukousne největší kus. Dneska to budeš ty.","difficulty":"rare","successRate":65,"durationMinutes":25,"reward":{"overdrive-x":1,"dirty-cash":500},"risk":{"successHeat":6,"failureHeat":12,"failureDirtyCashLoss":600}},{"id":"leon_51","agentId":"leon","title":"Krabice od špíny","description":"Na kraji sektoru čeká pár beden, co už prošly moc rukama. Smrdí, jsou kradený a přesně proto na nich vyděláš nejvíc.","difficulty":"medium","successRate":82,"durationMinutes":16,"reward":{"metal-parts":5,"chemicals":1},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_52","agentId":"leon","title":"Obchod se strachem","description":"Jeden malej dealer se bojí, že ho někdo obere. Nabídni mu ochranu. Drahou, špinavou a povinnou.","difficulty":"medium","successRate":81,"durationMinutes":15,"reward":{"cash":1500,"influence":4},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_53","agentId":"leon","title":"Přesunutá zásilka","description":"Jedna zásilka má dojet jinam. Ty zařídíš, že skončí u nás. Bez hluku, bez výčitek, se ziskem.","difficulty":"medium","successRate":80,"durationMinutes":17,"reward":{"chemicals":5},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_54","agentId":"leon","title":"Vysátý sklad","description":"Ve skladu zůstalo víc, než měl majitel přiznat. Tak mu pomůžeme s inventurou po svým.","difficulty":"medium","successRate":76,"durationMinutes":19,"reward":{"metal-parts":6,"cash":400,"influence":2},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_55","agentId":"leon","title":"Cena za mlčení","description":"Někdo viděl víc, než měl. Nech ho pochopit, že ticho je levnější než nemocnice.","difficulty":"medium","successRate":78,"durationMinutes":16,"reward":{"dirty-cash":1400,"influence":5},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_56","agentId":"leon","title":"Přeprodej krve","description":"Po jednom špinavým střetu zůstalo na zemi vybavení. Posbírej to a otoč to, než zaschne krev.","difficulty":"medium","successRate":74,"durationMinutes":18,"reward":{"metal-parts":3,"cash":600},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_57","agentId":"leon","title":"Mokrej deal u kanálu","description":"U kanálu se mají měnit ruce, peníze a loajalita. Dohlídni, aby všechno skončilo v našich kapsách.","difficulty":"medium","successRate":80,"durationMinutes":17,"reward":{"dirty-cash":1600,"influence":3},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_58","agentId":"leon","title":"Špatná adresa, dobrý zisk","description":"Jedna zásilka půjde na špatnou adresu. A ta adresa bude naše. Někdy je logistika krásná věc.","difficulty":"easy","successRate":86,"durationMinutes":14,"reward":{"chemicals":2},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"leon_59","agentId":"leon","title":"Rozprodej paniky","description":"Když se někdo začne bát razie, prodá i vlastní boty. Kup levně všechno, co pustí z ruky.","difficulty":"easy","successRate":88,"durationMinutes":13,"reward":{"metal-parts":4},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"leon_60","agentId":"leon","title":"Špinavé procento","description":"Dva idioti chtějí udělat obchod. Ty jim ho umožníš. A ukousneš si takovej podíl, že je to bude bolet až doma.","difficulty":"easy","successRate":87,"durationMinutes":14,"reward":{"cash":1000,"influence":3},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"leon_61","agentId":"leon","title":"Bedny z rozbité dodávky","description":"Na krajnici stojí dodávka, co nedojela. Někdo brečí nad plechem, ty vyděláš na obsahu.","difficulty":"medium","successRate":83,"durationMinutes":16,"reward":{"chemicals":1,"metal-parts":5},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_62","agentId":"leon","title":"Šelma mezi překupníky","description":"Na trhu je moc hladových krys. Buď největší z nich a stáhni jim nejlepší kusy přímo před nosem.","difficulty":"medium","successRate":82,"durationMinutes":15,"reward":{"cash":1219,"dirty-cash":380,"influence":3},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_63","agentId":"leon","title":"Kontakty v bordelu","description":"Nejlepší informace neleží v kanceláři. Leží v zakouřeným bordelu mezi lidma, co mluví, když si myslí, že jsou v bezpečí.","difficulty":"medium","successRate":81,"durationMinutes":17,"reward":{"influence":6,"dirty-cash":800},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_64","agentId":"leon","title":"Levný kulky, drahá noc","description":"Někdo se chce zbavit železa, než přijde kontrola. Seber to levně a pošli dál ještě před svítáním.","difficulty":"medium","successRate":73,"durationMinutes":19,"reward":{"cash":700,"influence":2},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_65","agentId":"leon","title":"Kapsářský velkoobchod","description":"Malej zloděj ukradl víc, než zvládne prodat. Tak ho odlehči. Klidně i od iluzí.","difficulty":"easy","successRate":89,"durationMinutes":12,"reward":{"cash":300,"metal-parts":3,"influence":2},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"leon_66","agentId":"leon","title":"Přehoz přes sektor","description":"V jednom sektoru je bída, v druhým hlad. Ty propojíš jedno s druhým a zbytek shrábneš.","difficulty":"easy","successRate":86,"durationMinutes":14,"reward":{"cash":1000,"influence":3},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"leon_67","agentId":"leon","title":"Ostrá přirážka","description":"Klient chce zboží hned. To znamená jediný: zvedni cenu, usměj se a nech ho krvácet do peněženky.","difficulty":"easy","successRate":90,"durationMinutes":11,"reward":{"cash":863,"dirty-cash":136},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"leon_68","agentId":"leon","title":"Zadní pokoj","description":"V zadním pokoji se dneska budou přehazovat věci, co neměly opustit sklad. Dohlídni, aby opustily i majitele.","difficulty":"medium","successRate":80,"durationMinutes":16,"reward":{"chemicals":3},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_69","agentId":"leon","title":"Srážka zájmů","description":"Dvě party chtějí to samý zboží. Ty jim prodáš naději, chaos a nakonec to zinkasuješ celý.","difficulty":"medium","successRate":79,"durationMinutes":17,"reward":{"cash":1600,"influence":5},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_70","agentId":"leon","title":"Oškrabaná marže","description":"Na dealu už si ukousli jiní. Ty z toho seškrábneš poslední vrstvu. A ta bývá nejtučnější.","difficulty":"easy","successRate":86,"durationMinutes":13,"reward":{"dirty-cash":750,"cash":250},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"leon_71","agentId":"leon","title":"Výprodej slabosti","description":"Někdo potřebuje rychle cash a prodá všechno pod cenou. Ty potřebuješ jen přijít včas a bejt bez slitování.","difficulty":"easy","successRate":92,"durationMinutes":12,"reward":{"metal-parts":4},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"leon_72","agentId":"leon","title":"Rozsypaný lékárenský zboží","description":"Po jedný hádce zůstalo pár beden z lékárny bez dozoru. Posbírej to a prodej to dřív, než se majitel probere.","difficulty":"medium","successRate":82,"durationMinutes":16,"reward":{"chemicals":6,"cash":40},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_73","agentId":"leon","title":"Prašivej broker","description":"Dneska nebudeš obchodník. Dneska budeš hyena s kontakty. A hyeny se v tomhle městě nají nejlíp.","difficulty":"easy","successRate":88,"durationMinutes":13,"reward":{"cash":1000,"influence":3},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"leon_74","agentId":"leon","title":"Dohoda v dešti","description":"Když prší, lidi méně koukají. To je ideální chvíle poslat špinavý zboží přes půl bloku.","difficulty":"medium","successRate":85,"durationMinutes":15,"reward":{"velvet-smoke":1},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_75","agentId":"leon","title":"Výběr od zoufalců","description":"Dneska neokradeš bohatý. Dneska vytěžíš zoufalý. A zoufalí platí nejrychlejc.","difficulty":"medium","successRate":84,"durationMinutes":15,"reward":{"dirty-cash":1600,"influence":4},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_76","agentId":"leon","title":"Překup za rozbitým barem","description":"Za jedním rozbitým barem čeká týpek s věcma, co by oficiálně měly být zamčený jinde. Tak je oficiálně přesuň k nám.","difficulty":"medium","successRate":79,"durationMinutes":17,"reward":{"chemicals":2,"cash":600},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_77","agentId":"leon","title":"Křivá směnka","description":"Někdo se upsál špatným lidem. Ty od něj koupíš dluh za drobný a vybereš ho jako plnou cenu.","difficulty":"medium","successRate":81,"durationMinutes":16,"reward":{"cash":1309,"dirty-cash":290,"influence":3},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_78","agentId":"leon","title":"Špinavý přepočet beden","description":"Na papíře jich je deset. Ve skutečnosti jich může zmizet dvanáct. Takovej účetnictví já respektuju.","difficulty":"medium","successRate":80,"durationMinutes":16,"reward":{"metal-parts":5},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_79","agentId":"leon","title":"Přepálený zájemce","description":"Jeden kupec chce zboží tak moc, že už necítí pach podrazu. Přesně takový mám nejradši.","difficulty":"easy","successRate":89,"durationMinutes":11,"reward":{"cash":1000,"influence":3},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"leon_80","agentId":"leon","title":"Rozřezaná trasa","description":"Běžná přepravní trasa je dneska mrtvá. Vezmeš náklad bokem a z marže uděláš malý svinstvo.","difficulty":"easy","successRate":87,"durationMinutes":14,"reward":{"metal-parts":4},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"leon_81","agentId":"leon","title":"Sektorový pijavice","description":"Na každým sektoru visí někdo, kdo už saje moc dlouho. Dneska ho odsajeme my.","difficulty":"medium","successRate":77,"durationMinutes":18,"reward":{"dirty-cash":1600,"influence":4},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_82","agentId":"leon","title":"Levná bolest, drahý zisk","description":"Někdo prodá cennej materiál jen proto, aby přežil noc. Ty si z jeho bolesti uděláš obchodní model.","difficulty":"easy","successRate":86,"durationMinutes":14,"reward":{"influence":2},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"leon_83","agentId":"leon","title":"Kufr po mrtvým dealu","description":"Po jednom zpackaným setkání zůstal kufr bez dozoru. Otevři ho a udělej z cizího průseru náš profit.","difficulty":"medium","successRate":75,"durationMinutes":18,"reward":{"dirty-cash":900,"influence":2},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_84","agentId":"leon","title":"Prodej přes bolest","description":"Někdy nestačí nabídnout cenu. Někdy musíš nabídnout i důvod, proč ji mají přijmout bez keců.","difficulty":"medium","successRate":82,"durationMinutes":15,"reward":{"cash":1600,"influence":5},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_85","agentId":"leon","title":"Rozebranej kontejner","description":"V přístavu někdo otevřel, co otevřít neměl. Posbírej zbytky a pošli je dál, než přijdou uniformy.","difficulty":"medium","successRate":76,"durationMinutes":19,"reward":{"metal-parts":6,"chemicals":1},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_86","agentId":"leon","title":"Pobodaná nabídka","description":"Jeden obchod skončil nožem ve stole. To znamená dvě věci: méně zájemců a víc prostoru pro nás.","difficulty":"medium","successRate":74,"durationMinutes":18,"reward":{"cash":700,"dirty-cash":400},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_87","agentId":"leon","title":"Otočka přes špínu","description":"Tohle zboží je tak špinavý, že by si zasloužilo vlastní kanalizaci. Přesně proto má krásnou marži.","difficulty":"medium","successRate":83,"durationMinutes":15,"reward":{"chemicals":3},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_88","agentId":"leon","title":"Rozšlapaný kontakt","description":"Jeden kontakt dostal přes hubu a chce zmizet. Nech ho zmizet. Ale nejdřív z něj vytáhni všechno cenný.","difficulty":"medium","successRate":84,"durationMinutes":15,"reward":{"influence":6,"dirty-cash":800},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_89","agentId":"leon","title":"Přesun černé várky","description":"Várka je horká, sektor nervózní a čas krátkej. Přesuň to, než se někdo začne zajímat moc.","difficulty":"medium","successRate":82,"durationMinutes":16,"reward":{"chemicals":4,"cash":500},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_90","agentId":"leon","title":"Drahý mlčení u stolu","description":"U jednoho stolu sedí lidi, co by spolu normálně nemluvili. Ty jim pomůžeš najít společnou řeč. Za velmi nepříjemnou cenu.","difficulty":"medium","successRate":80,"durationMinutes":17,"reward":{"cash":1600,"influence":4},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_91","agentId":"leon","title":"Prohnilý deal","description":"Ten kšeft je prohnilej od základu. Ale i z prohnilýho dřeva se dá postavit pěkně hnusnej zisk.","difficulty":"easy","successRate":86,"durationMinutes":13,"reward":{"dirty-cash":800,"cash":200},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"leon_92","agentId":"leon","title":"Tahání za nitky","description":"Dneska nebudeš tahat bedny. Dneska budeš tahat lidi. A lidi se prodávají ještě líp než zboží.","difficulty":"medium","successRate":83,"durationMinutes":16,"reward":{"influence":6,"cash":1000},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_93","agentId":"leon","title":"Sklad pro krysy","description":"Jeden sklad je tak děravej, že si z něj bere každej. Dneska si z něj vezmeme nejvíc my.","difficulty":"medium","successRate":84,"durationMinutes":15,"reward":{"metal-parts":6,"cash":400,"influence":2},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_94","agentId":"leon","title":"Zuby trhu","description":"Trh není místo pro obchodníky. Je to místo pro predátory. Tak koukej kousat.","difficulty":"easy","successRate":88,"durationMinutes":13,"reward":{"cash":1000,"influence":3},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"leon_95","agentId":"leon","title":"Rozkradený papíry","description":"Některý zásilky cestují díky razítku. Dneska se postaráš, aby papíry zmizely a zboží zůstalo nám.","difficulty":"medium","successRate":81,"durationMinutes":16,"reward":{"tech-core":1,"influence":3},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_96","agentId":"leon","title":"Překupnický masakr","description":"Na jednom rohu se dneska roztrhá několik překupníků o stejnou věc. Ty to vezmeš první a prodáš jim to zpátky dráž.","difficulty":"medium","successRate":79,"durationMinutes":17,"reward":{"cash":1600,"influence":4},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_97","agentId":"leon","title":"Vydřená marže","description":"Tohle nebude hezkej obchod. Tohle bude špinavý, tvrdý a přesně tak výdělečný, jak to mám rád.","difficulty":"medium","successRate":78,"durationMinutes":18,"reward":{"dirty-cash":1600,"influence":4},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_98","agentId":"leon","title":"Dodávka z pekla","description":"Jedna dodávka veze tolik bordelu, že by ji nikdo neměl vidět. Postarej se, aby ji nikdo ani nedopočítal.","difficulty":"medium","successRate":77,"durationMinutes":19,"reward":{"chemicals":1,"metal-parts":5},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_99","agentId":"leon","title":"Řez z každý kapsy","description":"Dneska nebudeš brát jen z jednoho zdroje. Dneska si ukousneš z každý kapsy, co se v sektoru pohne.","difficulty":"medium","successRate":82,"durationMinutes":16,"reward":{"cash":1280,"dirty-cash":320,"influence":4},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"leon_100","agentId":"leon","title":"Leonova špinavá škola","description":"Zapamatuj si to. Ulice nepatří tomu, kdo má čistý ruce. Patří tomu, kdo umí z každýho svinstva udělat zisk. Dneska budeš učit ostatní.","difficulty":"rare","successRate":65,"durationMinutes":25,"reward":{"ghost-serum":1,"dirty-cash":500},"risk":{"successHeat":6,"failureHeat":12,"failureDirtyCashLoss":600}},{"id":"nyra_01","agentId":"nyra","title":"Špatně zamčený telefon","description":"Jeden idiot nechal telefon bez dozoru a bez zámku. Vezmi z něj všechno, co se dá prodat, zneužít nebo poslat správným lidem.","difficulty":"easy","successRate":86,"durationMinutes":14,"reward":{"influence":3,"dirty-cash":700},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"nyra_02","agentId":"nyra","title":"Šeptaná slabina","description":"V každém sektoru je někdo, kdo ví příliš moc a pije příliš levně. Sedni si k němu a nech ho mluvit.","difficulty":"easy","successRate":88,"durationMinutes":13,"reward":{"influence":3,"cash":800},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"nyra_03","agentId":"nyra","title":"První lež zdarma","description":"Rozšiř mezi správné uši malou lež. Když se chytne, ostatní udělají zbytek práce za tebe.","difficulty":"medium","successRate":84,"durationMinutes":15,"reward":{"influence":6,"dirty-cash":500},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_04","agentId":"nyra","title":"Fotka, která bolí","description":"Jedna fotka má větší váhu než zásobník. Získej ji a pak sleduj, jak rychle se mění loajalita za ticho.","difficulty":"medium","successRate":82,"durationMinutes":15,"reward":{"influence":6,"cash":900},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_05","agentId":"nyra","title":"Odcizený seznam","description":"Někdo si vede seznam jmen, adres a dluhů. Ten seznam dnes změní majitele. A s ním i půlku města.","difficulty":"medium","successRate":80,"durationMinutes":17,"reward":{"influence":6,"dirty-cash":700},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_06","agentId":"nyra","title":"Cizí paranoia","description":"Není třeba někoho zničit. Stačí, aby začal pochybovat o lidech kolem sebe. To už zvládne rozebrat zbytek sám.","difficulty":"medium","successRate":85,"durationMinutes":15,"reward":{"influence":6,"cash":600},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_07","agentId":"nyra","title":"Ztracený přístup","description":"Jeden přístupový kód se má ztratit. Ty se postaráš, aby se ztratil správnému člověku do kapsy.","difficulty":"medium","successRate":83,"durationMinutes":15,"reward":{"influence":4,"cash":500},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_08","agentId":"nyra","title":"Vydírání bez hlasu","description":"Někdy není potřeba říct ani slovo. Jen poslat správný důkaz na správné místo a počkat, kdo přijde platit první.","difficulty":"medium","successRate":79,"durationMinutes":16,"reward":{"dirty-cash":1200,"influence":6},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_09","agentId":"nyra","title":"Nastražená zpráva","description":"Pošli jednu zprávu tak, aby vypadala, že přišla od někoho jiného. Lidi jsou překvapivě ochotní si ničit životy sami.","difficulty":"medium","successRate":78,"durationMinutes":17,"reward":{"influence":6,"cash":700},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_10","agentId":"nyra","title":"Tichá výměna","description":"Na střeše proběhne výměna informací. Ty se neukážeš. Jen zajistíš, že jedna strana odejde chudší a druhá vyděšená.","difficulty":"medium","successRate":84,"durationMinutes":15,"reward":{"influence":5,"dirty-cash":900},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_11","agentId":"nyra","title":"Rozbitá důvěra","description":"Dva lidi si ještě pořád věří. To je chyba, kterou dnes opravíš.","difficulty":"medium","successRate":83,"durationMinutes":15,"reward":{"influence":6,"cash":800},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_12","agentId":"nyra","title":"Záznam z chodby","description":"Na jedné chodbě visí kamera, která viděla víc, než by měla. Stáhni záznam dřív, než ho smaže někdo jiný.","difficulty":"medium","successRate":85,"durationMinutes":15,"reward":{"influence":6,"cash":700,"dirty-cash":300},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_13","agentId":"nyra","title":"Toxický drb","description":"Jedna dobře vypuštěná informace dokáže otrávit celý sektor. Vypusť ji jemně a sleduj, kdo se začne dusit první.","difficulty":"medium","successRate":77,"durationMinutes":18,"reward":{"influence":6,"dirty-cash":500},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_14","agentId":"nyra","title":"Dívka u baru","description":"Některé dveře neotevře páčidlo, ale úsměv a dvě správné otázky. Dnes otevřeš právě takové.","difficulty":"easy","successRate":89,"durationMinutes":12,"reward":{"influence":3,"cash":900},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"nyra_15","agentId":"nyra","title":"Složka bez jména","description":"V jedné zásuvce leží složka, která nemá existovat. Vezmi ji a připomeň městu, že papír někdy řeže hlouběji než nůž.","difficulty":"medium","successRate":80,"durationMinutes":16,"reward":{"influence":6,"dirty-cash":800},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_16","agentId":"nyra","title":"Podvržený podpis","description":"Stačí jeden podpis na špatném místě a někdo se probudí s hodně drahým problémem.","difficulty":"medium","successRate":79,"durationMinutes":17,"reward":{"cash":1000,"influence":6},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_17","agentId":"nyra","title":"Noční odposlech","description":"Na jednu noc zapojíš uši tam, kam nepatří. To, co zachytíš, prodáš třikrát různým lidem.","difficulty":"medium","successRate":81,"durationMinutes":18,"reward":{"dirty-cash":1100,"influence":6},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_18","agentId":"nyra","title":"Jméno na seznamu","description":"Jedno jméno se objeví na špatném seznamu. A pak už jen sleduj, jak rychle začne jeho majitel panikařit.","difficulty":"medium","successRate":84,"durationMinutes":15,"reward":{"influence":6,"cash":700},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_19","agentId":"nyra","title":"Špína v archivu","description":"Nejlepší tajemství nejsou na ulici. Jsou uložená, seřazená a čekají, až je někdo použije správným způsobem.","difficulty":"medium","successRate":82,"durationMinutes":15,"reward":{"influence":6,"dirty-cash":900},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_20","agentId":"nyra","title":"Falešná stopa","description":"Naveď lovce na špatnou adresu a kořist zůstane bez dozoru. Krása manipulace je v tom, že nikdo neví, kdo začal.","difficulty":"easy","successRate":86,"durationMinutes":14,"reward":{"cash":900,"influence":3},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"nyra_21","agentId":"nyra","title":"Cizí heslo","description":"Někdo používá stejné heslo všude. Smutné. Ale výdělečné.","difficulty":"easy","successRate":90,"durationMinutes":11,"reward":{"influence":3,"cash":727,"dirty-cash":272},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"nyra_22","agentId":"nyra","title":"Přítelkyně problému","description":"Dnes se nebudeš prát. Dnes někomu nabídneš řešení, které ho udělá závislým na další schůzce s námi.","difficulty":"medium","successRate":85,"durationMinutes":15,"reward":{"influence":6,"dirty-cash":700},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_23","agentId":"nyra","title":"Šepot na schodišti","description":"Na schodišti se dnes řekne něco, co nemělo nikdy zaznít nahlas. Ty budeš stát dost blízko, aby to mělo cenu.","difficulty":"easy","successRate":88,"durationMinutes":12,"reward":{"influence":3,"cash":700},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"nyra_24","agentId":"nyra","title":"Zkažený deal","description":"Není třeba obchod zastavit. Stačí ho jen trochu pokazit, aby se obě strany začaly navzájem podezírat.","difficulty":"medium","successRate":79,"durationMinutes":16,"reward":{"influence":6,"dirty-cash":600},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_25","agentId":"nyra","title":"Sklenička navíc","description":"Lidi po třetí skleničce říkají věci, za které by ráno platili. Ty jim tu šanci dáš.","difficulty":"easy","successRate":91,"durationMinutes":11,"reward":{"cash":1000,"influence":3},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"nyra_26","agentId":"nyra","title":"Zamčená minulost","description":"Každý má minulost, kterou by nejradši utopil. Ty ji jen vytáhneš na hladinu a nabídneš ručník za správnou cenu.","difficulty":"medium","successRate":78,"durationMinutes":17,"reward":{"dirty-cash":1300,"influence":6},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_27","agentId":"nyra","title":"Slabé místo aliance","description":"Každá aliance má člena, co drží hubu jen do chvíle, než dostane správnou nabídku. Najdi ho.","difficulty":"medium","successRate":80,"durationMinutes":18,"reward":{"influence":6,"cash":900},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_28","agentId":"nyra","title":"Cizí deník","description":"Papír snese všechno. A některé papíry snesou dost na to, aby někdo začal platit pravidelně.","difficulty":"medium","successRate":84,"durationMinutes":15,"reward":{"influence":6,"dirty-cash":800},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_29","agentId":"nyra","title":"Otrávené podezření","description":"Stačí zasít malou pochybnost a sledovat, jak si ji lidi zalijí vlastní panikou.","difficulty":"medium","successRate":77,"durationMinutes":17,"reward":{"influence":6,"cash":600},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_30","agentId":"nyra","title":"Tichá výstraha","description":"Ne všichni potřebují dostat přes hubu. Některým stačí obálka bez odesílatele a špatný spánek na týden dopředu.","difficulty":"medium","successRate":85,"durationMinutes":15,"reward":{"influence":6,"dirty-cash":900},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_31","agentId":"nyra","title":"Přesměrovaná nenávist","description":"Dneska někoho nenasměruješ k cíli. Nasměruješ ho k omylu. A omyly v našem městě bývají smrtelně drahé.","difficulty":"medium","successRate":79,"durationMinutes":16,"reward":{"influence":6,"cash":800},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_32","agentId":"nyra","title":"Stará hlasová schránka","description":"Někdo zapomněl smazat hlasovky. Ty zapomeneš mít slitování.","difficulty":"easy","successRate":86,"durationMinutes":12,"reward":{"dirty-cash":1000,"influence":3},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"nyra_33","agentId":"nyra","title":"Zblízka a bez otisků","description":"Potřebuju, abys byl dost blízko na to slyšet pravdu a dost chytrej na to, abys po sobě nic nenechal.","difficulty":"medium","successRate":84,"durationMinutes":15,"reward":{"influence":6,"cash":700},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_34","agentId":"nyra","title":"Lehký dotek chaosu","description":"Nebudeme rozbíjet dveře. Jen jemně zatlačíme na správné lidi a zbytek město rozebere samo.","difficulty":"medium","successRate":78,"durationMinutes":16,"reward":{"influence":6,"dirty-cash":700},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_35","agentId":"nyra","title":"Smazaná kamera","description":"Někde chybí pár minut záznamu. Postarej se, aby chyběly přesně ty, které potřebujeme.","difficulty":"easy","successRate":87,"durationMinutes":12,"reward":{"cash":900,"influence":3},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"nyra_36","agentId":"nyra","title":"Noční návštěva","description":"Dnes někomu necháš za dveřmi důkaz, který tam neměl nikdy být. A pak počkáš, kdo začne křičet první.","difficulty":"medium","successRate":80,"durationMinutes":15,"reward":{"influence":6,"dirty-cash":800},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_37","agentId":"nyra","title":"Dva kroky od zrady","description":"Zrada nezačíná nožem do zad. Začíná jednou pochybností a správně položenou otázkou.","difficulty":"medium","successRate":76,"durationMinutes":18,"reward":{"influence":6,"cash":700},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_38","agentId":"nyra","title":"Kapesní tajemství","description":"Malé USB, velké problémy. Najdi ho a pak rozhodneme, kdo si za jeho návrat zaplatí nejvíc.","difficulty":"medium","successRate":85,"durationMinutes":15,"reward":{"dirty-cash":1200,"influence":5},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_39","agentId":"nyra","title":"Rozhovor za plentou","description":"Za jednou tenkou stěnou se dnes probere něco, co může rozpárat celý sektor. Naslouchej.","difficulty":"easy","successRate":89,"durationMinutes":12,"reward":{"influence":3,"cash":800},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"nyra_40","agentId":"nyra","title":"Podvržená účast","description":"Někdo bude vypadat, jako že byl na místě, kde nikdy nestál. A někdo jiný za to zaplatí, aby to zmizelo.","difficulty":"medium","successRate":79,"durationMinutes":17,"reward":{"dirty-cash":1100,"influence":6},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_41","agentId":"nyra","title":"Zaměněná obálka","description":"Stačí jedna obálka v nesprávných rukách a celý večer dostane nový směr.","difficulty":"easy","successRate":86,"durationMinutes":13,"reward":{"cash":900,"influence":3},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"nyra_42","agentId":"nyra","title":"Přepnutá loajalita","description":"Někteří lidé nejsou věrní. Jen ještě nedostali lepší nabídku. Dnes ji dostanou.","difficulty":"medium","successRate":81,"durationMinutes":16,"reward":{"influence":6,"dirty-cash":700},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_43","agentId":"nyra","title":"Křehká pověst","description":"Pověst je sklo. Jedna prasklina a zbytek už udělá tlak okolí. Ty uděláš tu prasklinu.","difficulty":"medium","successRate":83,"durationMinutes":15,"reward":{"influence":6,"cash":700},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_44","agentId":"nyra","title":"Vzkaz bez podpisu","description":"Pošli vzkaz, který nebude znít jako hrozba. Jen jako něco, co by si chytrý člověk neměl dovolit ignorovat.","difficulty":"easy","successRate":87,"durationMinutes":12,"reward":{"dirty-cash":1000,"influence":3},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"nyra_45","agentId":"nyra","title":"Druhé dno šuplíku","description":"Vždycky mě zajímá, co lidi schovávají pod tím, co schovávají. Tam bývá skutečná cena.","difficulty":"medium","successRate":85,"durationMinutes":15,"reward":{"influence":6,"cash":900,"dirty-cash":300},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_46","agentId":"nyra","title":"Zlá kombinace","description":"Spoj dvě pravdy s jednou lží a dostaneš příběh, který rozbije víc vztahů než pistole kolen.","difficulty":"medium","successRate":78,"durationMinutes":17,"reward":{"influence":6,"dirty-cash":600},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_47","agentId":"nyra","title":"Stín za zády","description":"Někdo musí mít pocit, že ho někdo sleduje. A ten pocit ho má stát peníze.","difficulty":"easy","successRate":86,"durationMinutes":13,"reward":{"cash":1000,"influence":3},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"nyra_48","agentId":"nyra","title":"Šepot v síti","description":"Dnes nevypustíš zprávu do ulic. Dnes ji pustíš do správných kanálů a necháš ji udělat ošklivější práci tiše.","difficulty":"medium","successRate":80,"durationMinutes":16,"reward":{"influence":6,"dirty-cash":800},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_49","agentId":"nyra","title":"Pád masky","description":"Každý někde hraje roli. Najdi místo, kde se zapomněl převléct zpátky do své lži.","difficulty":"medium","successRate":79,"durationMinutes":16,"reward":{"influence":6,"cash":800},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_50","agentId":"nyra","title":"Nyřin tah","description":"Pamatuj si to. Kulka udělá díru. Tajemství udělá prázdno. Dneska v tom prázdnu vyděláme víc než ostatní za celou noc.","difficulty":"rare","successRate":65,"durationMinutes":25,"reward":{"overdrive-x":1,"influence":8},"risk":{"successHeat":6,"failureHeat":12,"failureDirtyCashLoss":600}},{"id":"nyra_51","agentId":"nyra","title":"Druhá obálka","description":"První obálka člověka znervózní. Druhá ho připraví o spánek. Doruč tu druhou a nech ho přemýšlet, co všechno ještě víme.","difficulty":"easy","successRate":86,"durationMinutes":14,"reward":{"influence":3,"dirty-cash":900},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"nyra_52","agentId":"nyra","title":"Prasklina v hlavě","description":"Někdy není potřeba někoho zlomit. Stačí mu do hlavy zasadit jednu otázku, která tam začne hnít.","difficulty":"medium","successRate":84,"durationMinutes":15,"reward":{"influence":6,"cash":700},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_53","agentId":"nyra","title":"Cizí hlas ve tmě","description":"Jedna zpráva přehraná správným hlasem dokáže rozebrat víc než zbraň. Pošli ji a nech jejich jistoty umřít potichu.","difficulty":"medium","successRate":81,"durationMinutes":16,"reward":{"influence":6,"dirty-cash":800},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_54","agentId":"nyra","title":"Ztráta jistoty","description":"Dnes nikomu nevezmeš peníze. Dnes mu vezmeš pocit bezpečí. A ten bývá dražší.","difficulty":"medium","successRate":79,"durationMinutes":17,"reward":{"influence":6,"cash":600},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_55","agentId":"nyra","title":"Nespolehlivá vzpomínka","description":"Přesvědč někoho, že si pamatuje věc, která se nikdy nestala. Lidi si zbytek lži dopíšou sami.","difficulty":"medium","successRate":78,"durationMinutes":18,"reward":{"influence":6,"dirty-cash":700},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_56","agentId":"nyra","title":"Ztracený klid","description":"Jedna maličkost zmizí z bytu, druhá se objeví na špatném místě. A najednou začne mít někdo pocit, že už není sám.","difficulty":"medium","successRate":85,"durationMinutes":15,"reward":{"influence":6,"cash":800},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_57","agentId":"nyra","title":"Zrcadlo bez odrazu","description":"Každý má obraz sám o sobě. Ty ho dnes rozbiješ a necháš střepy, aby řezaly ještě dlouho potom.","difficulty":"medium","successRate":76,"durationMinutes":19,"reward":{"influence":6,"dirty-cash":600},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_58","agentId":"nyra","title":"Špatná hodina","description":"Vzbuď někoho uprostřed noci zprávou, která nedává smysl. Ráno už ho bude rozkládat vlastní představivost.","difficulty":"easy","successRate":90,"durationMinutes":11,"reward":{"influence":3,"cash":700},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"nyra_59","agentId":"nyra","title":"Tenká nitka loajality","description":"Důvěra není zeď. Je to nit. A dnes ji stačí jen lehce naříznout.","difficulty":"medium","successRate":80,"durationMinutes":16,"reward":{"influence":6,"dirty-cash":700},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_60","agentId":"nyra","title":"Zapomenutý klíč","description":"Někdo najde klíč, který nikdy nevlastnil. Přesně od chvíle, kdy ho vezme do ruky, začne přemýšlet, co všechno už někdo otevřel před ním.","difficulty":"easy","successRate":86,"durationMinutes":13,"reward":{"influence":3,"cash":727,"dirty-cash":272},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"nyra_61","agentId":"nyra","title":"Úsměv a jed","description":"Nejhorší rány nepřicházejí v hněvu. Přicházejí s klidem, úsměvem a přesně zvolenou větou.","difficulty":"easy","successRate":87,"durationMinutes":12,"reward":{"influence":3,"dirty-cash":800},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"nyra_62","agentId":"nyra","title":"Hlas na druhém konci","description":"Jedno anonymní zavolání. Jeden správný tón. Jeden večer, který už nikdy nebude normální.","difficulty":"medium","successRate":85,"durationMinutes":15,"reward":{"influence":6,"cash":900},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_63","agentId":"nyra","title":"Návštěva bez svědků","description":"Někdy stačí, aby někdo zahlédl stín za dveřmi a už si nikdy nebude jistý, jestli byl sám.","difficulty":"medium","successRate":81,"durationMinutes":15,"reward":{"influence":6,"dirty-cash":900},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_64","agentId":"nyra","title":"Zpožděná pravda","description":"Pravda je nejjedovatější, když přijde pozdě. Doruč ji přesně ve chvíli, kdy už nikdo nebude věřit vysvětlení.","difficulty":"medium","successRate":79,"durationMinutes":17,"reward":{"influence":6,"cash":700},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_65","agentId":"nyra","title":"Křehký dech","description":"Připomeň někomu, jak moc snadno se může zlomit jeho svět. Ne silou. Jen přesností.","difficulty":"medium","successRate":80,"durationMinutes":16,"reward":{"influence":6,"dirty-cash":700},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_66","agentId":"nyra","title":"Prázdná židle","description":"Na schůzce nech jednu židli prázdnou a jednu informaci navíc. Paranoia pak zaplní zbytek místnosti sama.","difficulty":"easy","successRate":86,"durationMinutes":13,"reward":{"influence":3,"cash":800},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"nyra_67","agentId":"nyra","title":"Kroky za zády","description":"Nech někoho slyšet kroky tam, kde nikdo není. To, co si domyslí, bude horší než skutečnost.","difficulty":"medium","successRate":77,"durationMinutes":18,"reward":{"influence":6,"dirty-cash":600},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_68","agentId":"nyra","title":"Rozladěné nervy","description":"Rozbij někomu rytmus dne. Jeden telefon ráno, jeden vzkaz večer, jedna cizí věc doma. Pak už se rozbije sám.","difficulty":"easy","successRate":88,"durationMinutes":12,"reward":{"influence":3,"cash":700},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"nyra_69","agentId":"nyra","title":"Otevřená rána","description":"Každý má místo, kam se nevrací. Ty ho tam dnes pošleš zpátky, aniž bys se ho dotkla.","difficulty":"medium","successRate":78,"durationMinutes":17,"reward":{"influence":6,"dirty-cash":800},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_70","agentId":"nyra","title":"Cizí oči","description":"Někdo musí uvěřit, že je sledovaný. Ne proto, že to je pravda. Ale protože strach platí rychleji než důkazy.","difficulty":"medium","successRate":85,"durationMinutes":15,"reward":{"influence":6,"cash":900},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_71","agentId":"nyra","title":"Jemné rozvrácení","description":"Nezničíš skupinu útokem. Zničíš ji tím, že si každý začne myslet, že ostatní něco skrývají.","difficulty":"medium","successRate":79,"durationMinutes":18,"reward":{"influence":6,"dirty-cash":700},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_72","agentId":"nyra","title":"Jed v tichu","description":"Některé věci není třeba říkat nahlas. Stačí je nechat v hlavě správného člověka dost dlouho.","difficulty":"easy","successRate":87,"durationMinutes":12,"reward":{"influence":3,"cash":800},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"nyra_73","agentId":"nyra","title":"Noc bez odpovědí","description":"Pošli sérii náznaků a pak zmiz. Nejhorší nejsou odpovědi. Nejhorší je, když žádné nepřijdou.","difficulty":"easy","successRate":86,"durationMinutes":13,"reward":{"influence":3,"dirty-cash":900},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"nyra_74","agentId":"nyra","title":"Přesná slabost","description":"Síla je hlučná. Slabost je tichá. Najdi ji, stiskni ji a sleduj, jak se celý člověk ohne kolem ní.","difficulty":"medium","successRate":80,"durationMinutes":16,"reward":{"influence":6,"cash":800},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_75","agentId":"nyra","title":"Porušený rytmus","description":"Lidé přežívají díky rutině. Znič ji a zbytek jejich jistot se začne sypat sám.","difficulty":"easy","successRate":88,"durationMinutes":12,"reward":{"influence":3,"dirty-cash":1000},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"nyra_76","agentId":"nyra","title":"Místnost bez vzduchu","description":"Zaveď někoho do rozhovoru, kde nebude moct lhát ani utéct. To bývá nejčistší forma násilí.","difficulty":"medium","successRate":81,"durationMinutes":15,"reward":{"influence":6,"cash":900},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_77","agentId":"nyra","title":"Vzkaz pod kůži","description":"Nech zprávu tam, kde ji najde jen ten správný člověk. A kde se jí nebude umět zbavit ani po přečtení.","difficulty":"easy","successRate":86,"durationMinutes":13,"reward":{"influence":3,"dirty-cash":900},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"nyra_78","agentId":"nyra","title":"Vina bez svědků","description":"Dnes nevyvoláš strach. Dnes vyvoláš vinu. A vina člověka rozloží zevnitř mnohem pomaleji a důkladněji.","difficulty":"medium","successRate":78,"durationMinutes":18,"reward":{"influence":6,"cash":700},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_79","agentId":"nyra","title":"Pocit cizí přítomnosti","description":"Uprav pár detailů a nech někoho dojít domů do prostoru, který už nebude působit jako jeho vlastní.","difficulty":"medium","successRate":82,"durationMinutes":15,"reward":{"influence":6,"dirty-cash":800},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_80","agentId":"nyra","title":"Tichý nátlak","description":"Nátlak nemusí křičet. Stačí, když se usadí vedle člověka a dýchá mu na krk celé odpoledne.","difficulty":"easy","successRate":87,"durationMinutes":12,"reward":{"influence":3,"cash":900},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"nyra_81","agentId":"nyra","title":"Narušený spánek","description":"Vyčerpaný člověk se láme snáz. Připrav ho o klidnou noc a ráno už udělá chybu sám.","difficulty":"easy","successRate":89,"durationMinutes":11,"reward":{"influence":3,"dirty-cash":700},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"nyra_82","agentId":"nyra","title":"Jméno ve špatných ústech","description":"Dnes rozšíříš jedno jméno přesně tam, kde ho nikdo nechce slyšet. Škody pak udělá sama jeho ozvěna.","difficulty":"medium","successRate":81,"durationMinutes":16,"reward":{"influence":6,"cash":800},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_83","agentId":"nyra","title":"Dům plný ticha","description":"Některá ticha nejsou klidná. Jsou nemocná. Ujisti se, že jedno takové dnes někoho doma počká.","difficulty":"medium","successRate":85,"durationMinutes":15,"reward":{"influence":6,"dirty-cash":900},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_84","agentId":"nyra","title":"Úhel pohledu","description":"Nepotřebuješ měnit fakta. Stačí změnit pořadí, ve kterém je někdo uslyší. A najednou z pravdy začne téct jed.","difficulty":"medium","successRate":80,"durationMinutes":17,"reward":{"influence":6,"cash":700},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_85","agentId":"nyra","title":"Cizí otisk","description":"Nech na místě něco, co tam nepatří. Člověk si pak zbytek scénáře dopíše sám a většinou mnohem hůř, než bychom vymysleli my.","difficulty":"easy","successRate":87,"durationMinutes":12,"reward":{"influence":3,"dirty-cash":1000},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"nyra_86","agentId":"nyra","title":"Dvě verze noci","description":"Stejný večer, dvě různé verze, tři různí svědci. Až se to začne srážet, nezůstane nikomu pevná půda pod nohama.","difficulty":"medium","successRate":78,"durationMinutes":18,"reward":{"influence":6,"cash":800},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_87","agentId":"nyra","title":"Rozklad jistoty","description":"Něčí sebevědomí stojí na jedné představě. Dnes mu ji vezmeš a necháš ho sledovat, jak se rozsype všechno okolo.","difficulty":"medium","successRate":80,"durationMinutes":16,"reward":{"influence":6,"dirty-cash":900},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_88","agentId":"nyra","title":"Přesně načasované ticho","description":"Někdy je nejkrutější neodpovědět. Dnes necháš ticho pracovat déle, než je pro někoho zdravé.","difficulty":"easy","successRate":91,"durationMinutes":11,"reward":{"influence":3,"cash":900},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"nyra_89","agentId":"nyra","title":"Neviditelná trhlina","description":"Na povrchu nebude vidět nic. Ale uvnitř už začne všechno praskat. To jsou moje oblíbené práce.","difficulty":"medium","successRate":79,"durationMinutes":17,"reward":{"influence":6,"dirty-cash":800},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_90","agentId":"nyra","title":"Přítomnost bez tváře","description":"Postarej se, aby někdo cítil něčí blízkost, aniž by kdy zahlédl tvář. Lidská představivost je levná a smrtelně účinná zbraň.","difficulty":"easy","successRate":86,"durationMinutes":13,"reward":{"influence":3,"cash":800},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"nyra_91","agentId":"nyra","title":"Sběr slabých míst","description":"Dnes neřešíš velké tajemství. Dnes posbíráš deset malých. A z těch malých se staví nejhorší klece.","difficulty":"easy","successRate":88,"durationMinutes":12,"reward":{"influence":3,"dirty-cash":1000},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"nyra_92","agentId":"nyra","title":"Pod kůží města","description":"V každém sektoru pulzuje strach, jen ho nikdo nechce pojmenovat. Dnes mu dáš tvar a cenu.","difficulty":"medium","successRate":78,"durationMinutes":18,"reward":{"influence":6,"cash":700},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_93","agentId":"nyra","title":"Slovo, které zůstane","description":"Vyber jednu větu, která se člověku usadí v hlavě jako střep. A pak ji řekni přesně jednou.","difficulty":"easy","successRate":87,"durationMinutes":12,"reward":{"influence":3,"dirty-cash":900},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"nyra_94","agentId":"nyra","title":"Cizí dotek v prostoru","description":"Přesuň pár věcí, nech pár stop a jednu nejasnost. Nic víc. To úplně stačí na dlouhou noc bez dechu.","difficulty":"easy","successRate":86,"durationMinutes":13,"reward":{"influence":3,"cash":800},"risk":{"successHeat":1,"failureHeat":3,"failureDirtyCashLoss":0}},{"id":"nyra_95","agentId":"nyra","title":"Hlad po odpovědi","description":"Dnes někomu nedáš důkaz. Dáš mu jen dost na to, aby po zbytku začal šílet toužit.","difficulty":"medium","successRate":80,"durationMinutes":16,"reward":{"influence":6,"dirty-cash":800},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_96","agentId":"nyra","title":"Jedovatá blízkost","description":"Nejhorší hrozby nejsou daleko. Jsou těsně vedle člověka, ve stejné místnosti, v obyčejném tónu hlasu.","difficulty":"medium","successRate":85,"durationMinutes":15,"reward":{"influence":6,"cash":900},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_97","agentId":"nyra","title":"Vnitřní pád","description":"Některé lidi není třeba srazit. Stačí jim odebrat poslední oporu a oni se zřítí sami.","difficulty":"medium","successRate":77,"durationMinutes":18,"reward":{"influence":6,"dirty-cash":900},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_98","agentId":"nyra","title":"Tři náznaky","description":"První náznak znejistí. Druhý rozhodí. Třetí zlomí. Doruč všechny tři ve správném pořadí.","difficulty":"medium","successRate":81,"durationMinutes":16,"reward":{"influence":6,"cash":800},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_99","agentId":"nyra","title":"Tma mezi lidmi","description":"Největší temnota není v ulicích. Je mezi lidmi, kteří si přestali věřit. Rozšiř ji.","difficulty":"medium","successRate":75,"durationMinutes":19,"reward":{"influence":6,"dirty-cash":800},"risk":{"successHeat":2,"failureHeat":6,"failureDirtyCashLoss":200}},{"id":"nyra_100","agentId":"nyra","title":"Nyřin jed","description":"Zapamatuj si to. Strach je hlasitý jen na začátku. Pak ztichne, usadí se v člověku a začne ho požírat zevnitř. Dnes ten hlad nakrmíme.","difficulty":"rare","successRate":65,"durationMinutes":25,"reward":{"overdrive-x":1,"influence":8},"risk":{"successHeat":6,"failureHeat":12,"failureDirtyCashLoss":600}}]');
  ...
  const freeModeOverride = {
    ...
    balance: {
      ...
      conflict: {
        initialAttackProtectionMs: 2 * 60 * 60 * 1e3,
        ...
        robbery: {
          ...
          poolsByZone: {
            park: {
              cash: { min: 4500, max: 7e3 },
              dirtyCash: { min: 2500, max: 4500 },
            ...
            residential: {
              cash: { min: 5e3, max: 8e3 },
              dirtyCash: { min: 3e3, max: 5e3 },
              chemicals: { min: 3, max: 5 },
              biomass: { min: 3, max: 5 },
              metalParts: { min: 2, max: 4 },
            ...
            commercial: {
              cash: { min: 6e3, max: 1e4 },
              dirtyCash: { min: 4e3, max: 7e3 },
            ...
            downtown: {
              cash: { min: 8e3, max: 14e3 },
              dirtyCash: { min: 5e3, max: 1e4 },
              chemicals: { min: 2, max: 4 },
              biomass: { min: 2, max: 4 },
              metalParts: { min: 3, max: 5 },
              stimPack: { min: 2, max: 4 },
  ...
  const rawRumorTemplates = /* @__PURE__ */ JSON.parse('[{"id":"rumor.attack_activity.confirmed.occupy_success.001","category":"attack_activity","confidence":"confirmed","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":[],"text":"{districtName} změnil vlajku tiše. Ráno už se tam ptali jiných lidí, komu patří chodník.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":1,"priority":90},{"id":"rumor.attack_activity.confirmed.occupy_success.002","category":"attack_activity","confidence":"confirmed","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":[],"text":"V {districtName} se přes noc přepsala pravidla. Nikdo nekřičel, ale všichni pochopili.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":1,"priority":90},{"id":"rumor.attack_activity.confirmed.occupy_success.003","category":"attack_activity","confidence":"confirmed","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":[],"text":"{districtName} spolkl novou posádku a vyplivl nový pořádek. Staré kontakty mlčí.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":1,"priority":90},{"id":"rumor.attack_activity.confirmed.occupy_success.004","category":"attack_activity","confidence":"confirmed","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":[],"text":"Ulice v {districtName} dostaly nové hlídky. Místní dělají, že je to normální.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":1,"priority":90},{"id":"rumor.attack_activity.confirmed.occupy_success.005","category":"attack_activity","confidence":"confirmed","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":[],"text":"{districtName} padl bez velkého divadla. O to víc z toho město znervóznělo.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":1,"priority":90},{"id":"rumor.attack_activity.confirmed.occupy_failure.001","category":"attack_activity","confidence":"confirmed","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":[],"text":"Pokus o převzetí {districtName} se rozpadl v bočních ulicích. Lidi zmizeli, district zůstal volný.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":1,"priority":90},{"id":"rumor.attack_activity.confirmed.occupy_failure.002","category":"attack_activity","confidence":"confirmed","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":[],"text":"{districtName} dnes nikoho nepustil dovnitř. Posádka se rozsypala dřív, než stihla pověsit barvy.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":1,"priority":90},{"id":"rumor.attack_activity.confirmed.occupy_failure.003","category":"attack_activity","confidence":"confirmed","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":[],"text":"V {districtName} někdo čekal moc dlouho na správný signál. Signál nepřišel, lidi ano.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":1,"priority":90},{"id":"rumor.attack_activity.confirmed.occupy_failure.004","category":"attack_activity","confidence":"confirmed","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":[],"text":"Obsazení {districtName} skončilo špatným tichem. Zůstaly jen prázdné kouty a chybějící tváře.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":1,"priority":90},{"id":"rumor.attack_activity.confirmed.occupy_failure.005","category":"attack_activity","confidence":"confirmed","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":[],"text":"{districtName} se tentokrát nepohnul. Ulice sežraly plán i lidi, co ho nesli.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":1,"priority":90},{"id":"rumor.population_movement.confirmed.001","category":"population_movement","confidence":"confirmed","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Ulice kolem {districtName} se vyprázdnily rychleji než bar po policejní razii. Někdo odvedl lidi pryč.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":1,"priority":30},{"id":"rumor.population_movement.confirmed.002","category":"population_movement","confidence":"confirmed","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Do {districtName} dorazila početná skupina. Nikdo se neusmívá a všichni mají stejný směr.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":1,"priority":30},{"id":"rumor.population_movement.confirmed.003","category":"population_movement","confidence":"confirmed","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Kamery zachytily zvýšený pohyb členů gangu {playerName}. Místní už zamykají dvěma zámky.","expiresAfterSeconds":900,"revealsPlayerName":true,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":30},{"id":"rumor.population_movement.confirmed.004","category":"population_movement","confidence":"confirmed","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"V {districtName} proběhl větší přesun lidí. Město to slyšelo dřív, než to někdo přiznal.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":1,"priority":30},{"id":"rumor.population_movement.confirmed.005","category":"population_movement","confidence":"confirmed","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Část ulic je náhle plná cizích tváří. Tohle není návštěva příbuzných.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":30},{"id":"rumor.population_movement.credible.001","category":"population_movement","confidence":"credible","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Říká se, že {districtName} právě polyká nové lidi po desítkách. Nikdo neví, koho bude zítra vyplivovat.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":1,"priority":30},{"id":"rumor.population_movement.credible.002","category":"population_movement","confidence":"credible","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"V okolí {districtName} se schází víc členů gangu než obvykle. A ne kvůli narozeninám.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":1,"priority":30},{"id":"rumor.population_movement.credible.003","category":"population_movement","confidence":"credible","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Místní řidiči vozí lidi jedním směrem a vracejí se sami. To obvykle nevěstí rodinnou oslavu.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":30},{"id":"rumor.population_movement.credible.004","category":"population_movement","confidence":"credible","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"V zadních ulicích {districtName} přibylo hlídek. Někdo čeká problém, nebo ho právě vyrábí.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":1,"priority":30},{"id":"rumor.population_movement.credible.005","category":"population_movement","confidence":"credible","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Několik bloků je náhle plných nervózních lidí. Nervozita je levná. Munice už méně.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":30},{"id":"rumor.population_movement.suspicion.001","category":"population_movement","confidence":"suspicion","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"V {districtName} je moc pohybu na obyčejnou noc a málo hudby na večírek.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":1,"priority":30},{"id":"rumor.population_movement.suspicion.002","category":"population_movement","confidence":"suspicion","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Taxikáři si stěžují, že pořád vozí stejné typy lidí do stejné části města.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":30},{"id":"rumor.population_movement.suspicion.003","category":"population_movement","confidence":"suspicion","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Někdo v {districtName} svolává svoje lidi. Otázka je, jestli k útoku, obraně, nebo pohřbu.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":1,"priority":30},{"id":"rumor.population_movement.suspicion.004","category":"population_movement","confidence":"suspicion","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Ulice zhoustly. Ne deštěm. Lidmi.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":30},{"id":"rumor.population_movement.suspicion.005","category":"population_movement","confidence":"suspicion","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Pár hospodských tvrdí, že se v okolí {districtName} něco chystá. Hospodští tvrdí ledacos, ale tentokrát zavřeli dřív.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":1,"priority":30},{"id":"rumor.population_movement.rumor.001","category":"population_movement","confidence":"rumor","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Na východě prý někdo sbírá lidi jako drobné ze stolu.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":30},{"id":"rumor.population_movement.rumor.002","category":"population_movement","confidence":"rumor","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Město šeptá, že {playerName} volá svoje krysy zpátky do jedné díry.","expiresAfterSeconds":900,"revealsPlayerName":true,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":30},{"id":"rumor.population_movement.rumor.003","category":"population_movement","confidence":"rumor","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Někdo prý vyklízí celé patro bytových bloků. Buď přijde práce, nebo smrt.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":30},{"id":"rumor.population_movement.rumor.004","category":"population_movement","confidence":"rumor","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Na zastávkách se mluví o přesunu, který má změnit náladu celé čtvrti.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":30},{"id":"rumor.population_movement.rumor.005","category":"population_movement","confidence":"rumor","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"V {districtName} je prý víc bot než dlažebních kostek. To se městu nelíbí.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":1,"priority":30},{"id":"rumor.population_movement.false_possible.001","category":"population_movement","confidence":"false_possible","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Jeden opilec tvrdí, že {districtName} je plný vojáků. Druhý opilec tvrdí, že první je policajt.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":0.5,"priority":30},{"id":"rumor.population_movement.false_possible.002","category":"population_movement","confidence":"false_possible","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Prý se stahují stovky lidí. Zdroj to počítal přes zamlžené okno a jeden z nich byl popelář.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":30},{"id":"rumor.population_movement.false_possible.003","category":"population_movement","confidence":"false_possible","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Někdo tvrdí, že celé bloky mizí do podzemí. Možná mobilizace. Možná porucha tramvají.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":30},{"id":"rumor.population_movement.false_possible.004","category":"population_movement","confidence":"false_possible","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"V {districtName} se údajně připravuje válka. Stejný člověk včera předpověděl konec světa před obědem.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":0.5,"priority":30},{"id":"rumor.population_movement.false_possible.005","category":"population_movement","confidence":"false_possible","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Říká se, že nikdo nezůstal doma. Jenže ve městě se doma nezůstává ani při požáru.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":30},{"id":"rumor.weapons_materials.confirmed.001","category":"weapons_materials","confidence":"confirmed","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club"],"text":"V okolí {districtName} výrazně vzrostla poptávka po bojovém vybavení. Prodavači počítají peníze, doktoři obvazy.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":1,"priority":50},{"id":"rumor.weapons_materials.confirmed.002","category":"weapons_materials","confidence":"confirmed","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club"],"text":"Několik dílen jede přes noc. Když stroje nespí, lidé brzy nebudou spát taky.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.weapons_materials.confirmed.003","category":"weapons_materials","confidence":"confirmed","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club"],"text":"Do oblasti dorazily těžké bedny bez označení. V tomhle městě jsou neoznačené bedny vždycky špatná zpráva.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.weapons_materials.confirmed.004","category":"weapons_materials","confidence":"confirmed","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club"],"text":"Zbrojní provozy v {districtName} hlásí zvýšenou aktivitu. Kov zpívá a ulice poslouchají.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":1,"priority":50},{"id":"rumor.weapons_materials.confirmed.005","category":"weapons_materials","confidence":"confirmed","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club"],"text":"Na černém trhu zmizela větší várka vybavení. Někdo se připravuje utratit víc než peníze.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.weapons_materials.credible.001","category":"weapons_materials","confidence":"credible","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club"],"text":"Říká se, že {playerName} nakupuje víc železa, než kolik potřebuje na opravu plotu.","expiresAfterSeconds":1800,"revealsPlayerName":true,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.weapons_materials.credible.002","category":"weapons_materials","confidence":"credible","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club"],"text":"Dílny v {districtName} jedou naplno. Zápach oleje se míchá s něčím, co připomíná strach.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":1,"priority":50},{"id":"rumor.weapons_materials.credible.003","category":"weapons_materials","confidence":"credible","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club"],"text":"Dodavatelé hlásí neobvykle velké objednávky. Nikdo se neptá proč. Ti chytří se neptají nikdy.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.weapons_materials.credible.004","category":"weapons_materials","confidence":"credible","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club"],"text":"V zadních skladech se prý hromadí výbava. A bedny jsou příliš těžké na oblečení.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.weapons_materials.credible.005","category":"weapons_materials","confidence":"credible","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club"],"text":"Místní kováři mají práci. Místní hrobníci se začínají usmívat.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.weapons_materials.suspicion.001","category":"weapons_materials","confidence":"suspicion","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club"],"text":"V {districtName} se v noci svítí v každé dílně. Buď vyrábějí zázrak, nebo problém.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":1,"priority":50},{"id":"rumor.weapons_materials.suspicion.002","category":"weapons_materials","confidence":"suspicion","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club"],"text":"Nákladní auta přijíždějí plná a odjíždějí prázdná. Papíry tvrdí, že vezou nábytek.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.weapons_materials.suspicion.003","category":"weapons_materials","confidence":"suspicion","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club"],"text":"Někdo skupuje materiál rychleji, než ho trh stíhá nacenit.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.weapons_materials.suspicion.004","category":"weapons_materials","confidence":"suspicion","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club"],"text":"V okolí skladů přibyly ozbrojené hlídky. Nikdo nehlídá prázdné regály.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.weapons_materials.suspicion.005","category":"weapons_materials","confidence":"suspicion","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club"],"text":"Zbrojovky mlčí, ale komíny mluví za ně.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.weapons_materials.rumor.001","category":"weapons_materials","confidence":"rumor","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club"],"text":"Po městě se povídá, že někdo staví dost železa na malou válku.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.weapons_materials.rumor.002","category":"weapons_materials","confidence":"rumor","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club"],"text":"Prý zmizela celá zásilka bojového vybavení. Policie hledá dodávku. Gangy hledají kupce.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.weapons_materials.rumor.003","category":"weapons_materials","confidence":"rumor","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club"],"text":"Někdo v {districtName} údajně skládá zbraně jako jiní skládají básně. Jen s horší pointou.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":1,"priority":50},{"id":"rumor.weapons_materials.rumor.004","category":"weapons_materials","confidence":"rumor","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club"],"text":"Na trhu koluje řeč o tajné objednávce. Cena byla vysoká. Mlčení ještě vyšší.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.weapons_materials.rumor.005","category":"weapons_materials","confidence":"rumor","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club"],"text":"Říká se, že příští výstřel bude dražší než poslední.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.weapons_materials.false_possible.001","category":"weapons_materials","confidence":"false_possible","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club"],"text":"Jeden překupník tvrdí, že viděl raketu v krabici od pizzy. Byl to překupník. A byla to velká pizza.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":50},{"id":"rumor.weapons_materials.false_possible.002","category":"weapons_materials","confidence":"false_possible","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club"],"text":"Prý se vyrábí nová superzbraň. Zdroj nepoznal granát od termosky.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":50},{"id":"rumor.weapons_materials.false_possible.003","category":"weapons_materials","confidence":"false_possible","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club"],"text":"Někdo údajně vyzbrojil půl města. Druhá půlka to zatím nepotvrdila.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":50},{"id":"rumor.weapons_materials.false_possible.004","category":"weapons_materials","confidence":"false_possible","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club"],"text":"Po barech se mluví o bednách plných zlata a zbraní. Obvykle jsou plné šroubů.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":50},{"id":"rumor.weapons_materials.false_possible.005","category":"weapons_materials","confidence":"false_possible","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club"],"text":"Říká se, že zbrojovka v {districtName} nikdy nezhasíná. Majitel tvrdí, že jen zapomněl vypínač.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":0.5,"priority":50},{"id":"rumor.district_defense.confirmed.001","category":"district_defense","confidence":"confirmed","allowedAudiences":["current_player","selected_district"],"allowedSourceBuildings":["restaurant","vip_lounge"],"text":"Obrana {districtName} byla nedávno oslabena. Zdi stojí, sebevědomí méně.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":1,"priority":50},{"id":"rumor.district_defense.confirmed.002","category":"district_defense","confidence":"confirmed","allowedAudiences":["current_player","selected_district"],"allowedSourceBuildings":["restaurant","vip_lounge"],"text":"Po posledním střetu zůstaly v {districtName} viditelné škody. Některé se opravují hůř než beton.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":1,"priority":50},{"id":"rumor.district_defense.confirmed.003","category":"district_defense","confidence":"confirmed","allowedAudiences":["current_player","selected_district"],"allowedSourceBuildings":["restaurant","vip_lounge"],"text":"Vstupy do districtu byly dodatečně zabezpečeny. Někdo se bojí návštěvy.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.district_defense.confirmed.004","category":"district_defense","confidence":"confirmed","allowedAudiences":["current_player","selected_district"],"allowedSourceBuildings":["restaurant","vip_lounge"],"text":"Na střechách přibyly hlídky. Město má teď víc očí a méně klidu.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.district_defense.confirmed.005","category":"district_defense","confidence":"confirmed","allowedAudiences":["current_player","selected_district"],"allowedSourceBuildings":["restaurant","vip_lounge"],"text":"Obranné pozice v {districtName} se změnily. Kdo znal starou cestu dovnitř, zná už jen cestu do márnice.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":1,"priority":50},{"id":"rumor.district_defense.credible.001","category":"district_defense","confidence":"credible","allowedAudiences":["current_player","selected_district"],"allowedSourceBuildings":["restaurant","vip_lounge"],"text":"Místní tvrdí, že {districtName} je po poslední ráně měkčí než obvykle.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":1,"priority":50},{"id":"rumor.district_defense.credible.002","category":"district_defense","confidence":"credible","allowedAudiences":["current_player","selected_district"],"allowedSourceBuildings":["restaurant","vip_lounge"],"text":"Říká se, že obránci nestíhají opravovat všechno, co jim nepřátelé rozbili.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.district_defense.credible.003","category":"district_defense","confidence":"credible","allowedAudiences":["current_player","selected_district"],"allowedSourceBuildings":["restaurant","vip_lounge"],"text":"V okolí {districtName} se shání víc betonu, kamer a nervů.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":1,"priority":50},{"id":"rumor.district_defense.credible.004","category":"district_defense","confidence":"credible","allowedAudiences":["current_player","selected_district"],"allowedSourceBuildings":["restaurant","vip_lounge"],"text":"Několik hlídek zmizelo ze svých obvyklých míst. Někdo je přesunul, nebo někdo odstranil.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.district_defense.credible.005","category":"district_defense","confidence":"credible","allowedAudiences":["current_player","selected_district"],"allowedSourceBuildings":["restaurant","vip_lounge"],"text":"District působí pevněji než včera. Město nerado vidí, když se kořist naučí kousat.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.district_defense.suspicion.001","category":"district_defense","confidence":"suspicion","allowedAudiences":["current_player","selected_district"],"allowedSourceBuildings":["restaurant","vip_lounge"],"text":"Brána se zavírá dřív a otevírá později. To bývá známka strachu, ne pořádku.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.district_defense.suspicion.002","category":"district_defense","confidence":"suspicion","allowedAudiences":["current_player","selected_district"],"allowedSourceBuildings":["restaurant","vip_lounge"],"text":"Na zdech je čerstvá barva. Pod čerstvou barvou bývají čerstvé díry.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.district_defense.suspicion.003","category":"district_defense","confidence":"suspicion","allowedAudiences":["current_player","selected_district"],"allowedSourceBuildings":["restaurant","vip_lounge"],"text":"Hlídky v {districtName} vypadají unaveně. Unavený obránce je pořád obránce.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":1,"priority":50},{"id":"rumor.district_defense.suspicion.004","category":"district_defense","confidence":"suspicion","allowedAudiences":["current_player","selected_district"],"allowedSourceBuildings":["restaurant","vip_lounge"],"text":"Některé kamery se otáčejí jinam než dřív. Buď nový plán, nebo stará chyba.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.district_defense.suspicion.005","category":"district_defense","confidence":"suspicion","allowedAudiences":["current_player","selected_district"],"allowedSourceBuildings":["restaurant","vip_lounge"],"text":"U vstupů přibyly překážky. Nikdo nestaví barikádu proti dobrým zprávám.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.district_defense.rumor.001","category":"district_defense","confidence":"rumor","allowedAudiences":["current_player","selected_district"],"allowedSourceBuildings":["restaurant","vip_lounge"],"text":"Prý je {districtName} skoro bez obrany. Tohle tvrzení už poslalo do hrobu víc lidí než kulky.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":1,"priority":50},{"id":"rumor.district_defense.rumor.002","category":"district_defense","confidence":"rumor","allowedAudiences":["current_player","selected_district"],"allowedSourceBuildings":["restaurant","vip_lounge"],"text":"Mluví se o slabém místě na severní straně. V tomhle městě má každá slabina cenu.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.district_defense.rumor.003","category":"district_defense","confidence":"rumor","allowedAudiences":["current_player","selected_district"],"allowedSourceBuildings":["restaurant","vip_lounge"],"text":"Říká se, že obránci čekají pomoc, která nepřijde.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.district_defense.rumor.004","category":"district_defense","confidence":"rumor","allowedAudiences":["current_player","selected_district"],"allowedSourceBuildings":["restaurant","vip_lounge"],"text":"Někdo slyšel, že district drží pohromadě jen dráty, nadávky a poslední směna.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.district_defense.rumor.005","category":"district_defense","confidence":"rumor","allowedAudiences":["current_player","selected_district"],"allowedSourceBuildings":["restaurant","vip_lounge"],"text":"Po městě běhá řeč, že {districtName} je připraven padnout. Řeči běhají rychleji než armády.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":1,"priority":50},{"id":"rumor.district_defense.false_possible.001","category":"district_defense","confidence":"false_possible","allowedAudiences":["current_player","selected_district"],"allowedSourceBuildings":["restaurant","vip_lounge"],"text":"Jeden bezdomovec tvrdí, že brána je otevřená. Možná myslel supermarket.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":50},{"id":"rumor.district_defense.false_possible.002","category":"district_defense","confidence":"false_possible","allowedAudiences":["current_player","selected_district"],"allowedSourceBuildings":["restaurant","vip_lounge"],"text":"Prý nejsou na střechách žádné hlídky. Zdroj se díval ze sklepa.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":50},{"id":"rumor.district_defense.false_possible.003","category":"district_defense","confidence":"false_possible","allowedAudiences":["current_player","selected_district"],"allowedSourceBuildings":["restaurant","vip_lounge"],"text":"Někdo tvrdí, že obrana zmizela. Někdo jiný tvrdí, že první někdo zmizel.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":50},{"id":"rumor.district_defense.false_possible.004","category":"district_defense","confidence":"false_possible","allowedAudiences":["current_player","selected_district"],"allowedSourceBuildings":["restaurant","vip_lounge"],"text":"Říká se, že district chrání jen jeden pes. Pes se k tomu odmítl vyjádřit.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":50},{"id":"rumor.district_defense.false_possible.005","category":"district_defense","confidence":"false_possible","allowedAudiences":["current_player","selected_district"],"allowedSourceBuildings":["restaurant","vip_lounge"],"text":"Fáma tvrdí, že všechny kamery jsou slepé. Fámy samy většinou nevidí nic.","expiresAfterSeconds":1800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":50},{"id":"rumor.attack_activity.confirmed.001","category":"attack_activity","confidence":"confirmed","allowedAudiences":["current_player","selected_district","global_city","alliance"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge"],"text":"V {districtName} proběhl ozbrojený střet. Město se ráno probudilo o něco tišší.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":1,"priority":70},{"id":"rumor.attack_activity.confirmed.002","category":"attack_activity","confidence":"confirmed","allowedAudiences":["current_player","selected_district","global_city","alliance"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge"],"text":"District změnil vlastníka. Staré vlajky hořely krátce, nové budou možná hořet déle.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":70},{"id":"rumor.attack_activity.confirmed.003","category":"attack_activity","confidence":"confirmed","allowedAudiences":["current_player","selected_district","global_city","alliance"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge"],"text":"Útok byl odražen. Útočníci nechali na ulici víc než jen sebevědomí.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":70},{"id":"rumor.attack_activity.confirmed.004","category":"attack_activity","confidence":"confirmed","allowedAudiences":["current_player","selected_district","global_city","alliance"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge"],"text":"Obrana padla, ale district zůstal neobsazený. Někdo rozbil dveře a zapomněl projít.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":70},{"id":"rumor.attack_activity.confirmed.005","category":"attack_activity","confidence":"confirmed","allowedAudiences":["current_player","selected_district","global_city","alliance"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge"],"text":"Střet skončil katastrofou. Oheň měl poslední slovo a nebylo krátké.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":70},{"id":"rumor.attack_activity.credible.001","category":"attack_activity","confidence":"credible","allowedAudiences":["current_player","selected_district","global_city","alliance"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge"],"text":"Říká se, že {playerName} připravuje něco hlučného v okolí {districtName}.","expiresAfterSeconds":3600,"revealsPlayerName":true,"revealsAllianceName":false,"revealsDistrictName":true,"weight":1,"priority":70},{"id":"rumor.attack_activity.credible.002","category":"attack_activity","confidence":"credible","allowedAudiences":["current_player","selected_district","global_city","alliance"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge"],"text":"V ulicích se objevili lidé, kteří nevypadají, že přišli vyjednávat.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":70},{"id":"rumor.attack_activity.credible.003","category":"attack_activity","confidence":"credible","allowedAudiences":["current_player","selected_district","global_city","alliance"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge"],"text":"Z blízkých districtů mizí hlídky a zásoby. Někdo je soustředí jinam.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":70},{"id":"rumor.attack_activity.credible.004","category":"attack_activity","confidence":"credible","allowedAudiences":["current_player","selected_district","global_city","alliance"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge"],"text":"Místní slyšeli krátké rozkazy a dlouhé ticho. To bývá předehra.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":70},{"id":"rumor.attack_activity.credible.005","category":"attack_activity","confidence":"credible","allowedAudiences":["current_player","selected_district","global_city","alliance"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge"],"text":"Někdo si v okolí {districtName} kreslí mapy. Ne turistické.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":1,"priority":70},{"id":"rumor.attack_activity.suspicion.001","category":"attack_activity","confidence":"suspicion","allowedAudiences":["current_player","selected_district","global_city","alliance"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge"],"text":"Na střechách je příliš mnoho pozorovatelů a příliš málo holubů.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":70},{"id":"rumor.attack_activity.suspicion.002","category":"attack_activity","confidence":"suspicion","allowedAudiences":["current_player","selected_district","global_city","alliance"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge"],"text":"Doprava v okolí {districtName} houstne. Řidiči se neptají, kdo sedí vzadu.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":1,"priority":70},{"id":"rumor.attack_activity.suspicion.003","category":"attack_activity","confidence":"suspicion","allowedAudiences":["current_player","selected_district","global_city","alliance"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge"],"text":"Rádio zachytilo krátké šifrované zprávy. Buď útok, nebo velmi špatná poezie.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":70},{"id":"rumor.attack_activity.suspicion.004","category":"attack_activity","confidence":"suspicion","allowedAudiences":["current_player","selected_district","global_city","alliance"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge"],"text":"Některé gangy náhle ztichly. Ticho je ve městě často nabitá zbraň.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":70},{"id":"rumor.attack_activity.suspicion.005","category":"attack_activity","confidence":"suspicion","allowedAudiences":["current_player","selected_district","global_city","alliance"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge"],"text":"V okolí cílových ulic se objevují cizí hlídky. Možná průzkum. Možná ztracené duše.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":70},{"id":"rumor.attack_activity.rumor.001","category":"attack_activity","confidence":"rumor","allowedAudiences":["current_player","selected_district","global_city","alliance"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge"],"text":"Mluví se o útoku, který má přijít bez varování. Což je zvláštní, když o něm mluví celé město.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":70},{"id":"rumor.attack_activity.rumor.002","category":"attack_activity","confidence":"rumor","allowedAudiences":["current_player","selected_district","global_city","alliance"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge"],"text":"Prý se chystá krev na severu. Déšť ji pak odnese na jih.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":70},{"id":"rumor.attack_activity.rumor.003","category":"attack_activity","confidence":"rumor","allowedAudiences":["current_player","selected_district","global_city","alliance"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge"],"text":"Někdo tvrdí, že {playerName} už vybral cíl. Cíl zatím neví, že je cílem.","expiresAfterSeconds":3600,"revealsPlayerName":true,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":70},{"id":"rumor.attack_activity.rumor.004","category":"attack_activity","confidence":"rumor","allowedAudiences":["current_player","selected_district","global_city","alliance"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge"],"text":"Po barech koluje mapa s jedním místem zakroužkovaným červeně.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":70},{"id":"rumor.attack_activity.rumor.005","category":"attack_activity","confidence":"rumor","allowedAudiences":["current_player","selected_district","global_city","alliance"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge"],"text":"Říká se, že zítřejší ráno bude patřit vítězům. A dnešní noc pohřebním službám.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":70},{"id":"rumor.attack_activity.false_possible.001","category":"attack_activity","confidence":"false_possible","allowedAudiences":["current_player","selected_district","global_city","alliance"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge"],"text":"Prý zaútočí všichni na všechny. Zdroj si spletl plán útoku s účtem v restauraci.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":70},{"id":"rumor.attack_activity.false_possible.002","category":"attack_activity","confidence":"false_possible","allowedAudiences":["current_player","selected_district","global_city","alliance"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge"],"text":"Někdo tvrdí, že útok začne o půlnoci. Ve městě je vždycky někde půlnoc.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":70},{"id":"rumor.attack_activity.false_possible.003","category":"attack_activity","confidence":"false_possible","allowedAudiences":["current_player","selected_district","global_city","alliance"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge"],"text":"Fáma mluví o tajné armádě v kanálech. Kanalizace hlásí jen krysy.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":70},{"id":"rumor.attack_activity.false_possible.004","category":"attack_activity","confidence":"false_possible","allowedAudiences":["current_player","selected_district","global_city","alliance"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge"],"text":"Údajný cíl útoku se mění podle toho, kdo zrovna platí rundu.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":70},{"id":"rumor.attack_activity.false_possible.005","category":"attack_activity","confidence":"false_possible","allowedAudiences":["current_player","selected_district","global_city","alliance"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge"],"text":"Prý je válka nevyhnutelná. V Empire Streets je nevyhnutelná hlavně další fáma.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":70},{"id":"rumor.espionage.confirmed.001","category":"espionage","confidence":"confirmed","allowedAudiences":["current_player","selected_district"],"allowedSourceBuildings":["convenience_store"],"text":"V {districtName} byl odhalen cizí pozorovatel. Neviděl všechno. Možná viděl dost.","expiresAfterSeconds":1200,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":1,"priority":30},{"id":"rumor.espionage.confirmed.002","category":"espionage","confidence":"confirmed","allowedAudiences":["current_player","selected_district"],"allowedSourceBuildings":["convenience_store"],"text":"Bezpečnost zachytila pokus o průnik. Někdo teď ví méně, než doufal, a víc, než by měl.","expiresAfterSeconds":1200,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":30},{"id":"rumor.espionage.confirmed.003","category":"espionage","confidence":"confirmed","allowedAudiences":["current_player","selected_district"],"allowedSourceBuildings":["convenience_store"],"text":"Podezřelá osoba byla zadržena poblíž strategického bodu. Její kapsy byly prázdné, oči ne.","expiresAfterSeconds":1200,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":30},{"id":"rumor.espionage.confirmed.004","category":"espionage","confidence":"confirmed","allowedAudiences":["current_player","selected_district"],"allowedSourceBuildings":["convenience_store"],"text":"V districtu proběhla bezpečnostní kontrola. Některé dveře se zavřely. Některé lidi taky.","expiresAfterSeconds":1200,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":30},{"id":"rumor.espionage.confirmed.005","category":"espionage","confidence":"confirmed","allowedAudiences":["current_player","selected_district"],"allowedSourceBuildings":["convenience_store"],"text":"Špehovací aktivita v oblasti vzrostla. Tajemství začínají být dražší než munice.","expiresAfterSeconds":1200,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":30},{"id":"rumor.espionage.credible.001","category":"espionage","confidence":"credible","allowedAudiences":["current_player","selected_district"],"allowedSourceBuildings":["convenience_store"],"text":"Místní si všimli tváří, které se dívají déle, než je slušné.","expiresAfterSeconds":1200,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":30},{"id":"rumor.espionage.credible.002","category":"espionage","confidence":"credible","allowedAudiences":["current_player","selected_district"],"allowedSourceBuildings":["convenience_store"],"text":"V {districtName} se někdo vyptává na směny, sklady a zadní vchody.","expiresAfterSeconds":1200,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":1,"priority":30},{"id":"rumor.espionage.credible.003","category":"espionage","confidence":"credible","allowedAudiences":["current_player","selected_district"],"allowedSourceBuildings":["convenience_store"],"text":"Několik kamer zachytilo stejného člověka na různých místech. Buď špeh, nebo velmi špatný turista.","expiresAfterSeconds":1200,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":30},{"id":"rumor.espionage.credible.004","category":"espionage","confidence":"credible","allowedAudiences":["current_player","selected_district"],"allowedSourceBuildings":["convenience_store"],"text":"Někdo mapuje okolí. Dělá to pomalu, opatrně a bez zájmu o architekturu.","expiresAfterSeconds":1200,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":30},{"id":"rumor.espionage.credible.005","category":"espionage","confidence":"credible","allowedAudiences":["current_player","selected_district"],"allowedSourceBuildings":["convenience_store"],"text":"V baru padaly otázky, za které se jinde platí penězi. Tady se platí zuby.","expiresAfterSeconds":1200,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":30},{"id":"rumor.espionage.suspicion.001","category":"espionage","confidence":"suspicion","allowedAudiences":["current_player","selected_district"],"allowedSourceBuildings":["convenience_store"],"text":"Jeden muž seděl tři hodiny nad prázdnou sklenicí a sledoval dveře.","expiresAfterSeconds":1200,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":30},{"id":"rumor.espionage.suspicion.002","category":"espionage","confidence":"suspicion","allowedAudiences":["current_player","selected_district"],"allowedSourceBuildings":["convenience_store"],"text":"Místní prodavač tvrdí, že se ho někdo ptal na věci, které zákazníky obvykle nezajímají.","expiresAfterSeconds":1200,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":30},{"id":"rumor.espionage.suspicion.003","category":"espionage","confidence":"suspicion","allowedAudiences":["current_player","selected_district"],"allowedSourceBuildings":["convenience_store"],"text":"Některé kamery na chvíli osleply. Náhoda má ve městě drahé právníky.","expiresAfterSeconds":1200,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":30},{"id":"rumor.espionage.suspicion.004","category":"espionage","confidence":"suspicion","allowedAudiences":["current_player","selected_district"],"allowedSourceBuildings":["convenience_store"],"text":"V districtu se pohybuje cizinec, který zná příliš mnoho ulic a příliš málo lidí.","expiresAfterSeconds":1200,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":30},{"id":"rumor.espionage.suspicion.005","category":"espionage","confidence":"suspicion","allowedAudiences":["current_player","selected_district"],"allowedSourceBuildings":["convenience_store"],"text":"Někdo nechal otevřený servisní vstup. Možná chyba. Možná pozvánka.","expiresAfterSeconds":1200,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":30},{"id":"rumor.espionage.rumor.001","category":"espionage","confidence":"rumor","allowedAudiences":["current_player","selected_district"],"allowedSourceBuildings":["convenience_store"],"text":"Říká se, že {playerName} má oči tam, kde jiní mají jen zdi.","expiresAfterSeconds":1200,"revealsPlayerName":true,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":30},{"id":"rumor.espionage.rumor.002","category":"espionage","confidence":"rumor","allowedAudiences":["current_player","selected_district"],"allowedSourceBuildings":["convenience_store"],"text":"Prý se v {districtName} ztratil špeh. Ve skutečnosti se špehové neztrácejí. Jen přestávají hlásit.","expiresAfterSeconds":1200,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":1,"priority":30},{"id":"rumor.espionage.rumor.003","category":"espionage","confidence":"rumor","allowedAudiences":["current_player","selected_district"],"allowedSourceBuildings":["convenience_store"],"text":"Někdo prodává informace o místních hlídkách. Cena se mění podle toho, kdo se ptá.","expiresAfterSeconds":1200,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":30},{"id":"rumor.espionage.rumor.004","category":"espionage","confidence":"rumor","allowedAudiences":["current_player","selected_district"],"allowedSourceBuildings":["convenience_store"],"text":"Po městě koluje seznam zadních vchodů. Každý seznam má jedno jméno navíc a jednu díru v hlavě.","expiresAfterSeconds":1200,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":30},{"id":"rumor.espionage.rumor.005","category":"espionage","confidence":"rumor","allowedAudiences":["current_player","selected_district"],"allowedSourceBuildings":["convenience_store"],"text":"Říká se, že někdo ví, co je uvnitř. Nikdo ale neví, kdo to ví.","expiresAfterSeconds":1200,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":30},{"id":"rumor.espionage.false_possible.001","category":"espionage","confidence":"false_possible","allowedAudiences":["current_player","selected_district"],"allowedSourceBuildings":["convenience_store"],"text":"Jeden číšník tvrdí, že každý druhý host je špeh. První host je číšník.","expiresAfterSeconds":1200,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":30},{"id":"rumor.espionage.false_possible.002","category":"espionage","confidence":"false_possible","allowedAudiences":["current_player","selected_district"],"allowedSourceBuildings":["convenience_store"],"text":"Prý kamery zachytily neviditelného muže. Kamery byly vypnuté.","expiresAfterSeconds":1200,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":30},{"id":"rumor.espionage.false_possible.003","category":"espionage","confidence":"false_possible","allowedAudiences":["current_player","selected_district"],"allowedSourceBuildings":["convenience_store"],"text":"Někdo označil pošťáka za agenta. Pošťák požaduje omluvu a lepší dýško.","expiresAfterSeconds":1200,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":30},{"id":"rumor.espionage.false_possible.004","category":"espionage","confidence":"false_possible","allowedAudiences":["current_player","selected_district"],"allowedSourceBuildings":["convenience_store"],"text":"Fáma tvrdí, že špeh bydlí přímo v {districtName}. Stejná fáma uvádí tři různé adresy.","expiresAfterSeconds":1200,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":0.5,"priority":30},{"id":"rumor.espionage.false_possible.005","category":"espionage","confidence":"false_possible","allowedAudiences":["current_player","selected_district"],"allowedSourceBuildings":["convenience_store"],"text":"Říká se, že informace unikají skrz kanalizaci. Kanalizace zatím mlčí.","expiresAfterSeconds":1200,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":30},{"id":"rumor.heist_robbery.confirmed.001","category":"heist_robbery","confidence":"confirmed","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["convenience_store"],"text":"V {districtName} proběhla loupežná operace. Peníze změnily majitele rychleji než svědomí.","expiresAfterSeconds":2400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":1,"priority":50},{"id":"rumor.heist_robbery.confirmed.002","category":"heist_robbery","confidence":"confirmed","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["convenience_store"],"text":"Bezpečnost zaznamenala násilný průnik. Trezor přežil méně než pověst majitele.","expiresAfterSeconds":2400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.heist_robbery.confirmed.003","category":"heist_robbery","confidence":"confirmed","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["convenience_store"],"text":"Z oblasti zmizela část zásob. Zloději nezanechali podpis, jen účet.","expiresAfterSeconds":2400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.heist_robbery.confirmed.004","category":"heist_robbery","confidence":"confirmed","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["convenience_store"],"text":"Noční operace skončila střelbou a prázdnými regály.","expiresAfterSeconds":2400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.heist_robbery.confirmed.005","category":"heist_robbery","confidence":"confirmed","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["convenience_store"],"text":"V districtu byl narušen provoz. Některé podniky počítají ztráty. Jiné nové zákazníky.","expiresAfterSeconds":2400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.heist_robbery.credible.001","category":"heist_robbery","confidence":"credible","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["convenience_store"],"text":"Říká se, že někdo sleduje tok peněz přes {districtName}. Peníze se nikdy necítí bezpečně.","expiresAfterSeconds":2400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":1,"priority":50},{"id":"rumor.heist_robbery.credible.002","category":"heist_robbery","confidence":"credible","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["convenience_store"],"text":"V okolí zadních vchodů přibyly hlídky. Trezor nejspíš nespí klidně.","expiresAfterSeconds":2400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.heist_robbery.credible.003","category":"heist_robbery","confidence":"credible","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["convenience_store"],"text":"Několik zaměstnanců bylo posláno domů dřív. Když zaměstnanci odcházejí, profesionálové často přicházejí.","expiresAfterSeconds":2400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.heist_robbery.credible.004","category":"heist_robbery","confidence":"credible","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["convenience_store"],"text":"Místní bezpečnost mění směny bez vysvětlení. Vysvětlení obvykle přijde s maskou.","expiresAfterSeconds":2400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.heist_robbery.credible.005","category":"heist_robbery","confidence":"credible","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["convenience_store"],"text":"Někdo shání plán budovy, rozpis hlídek a tiché auto.","expiresAfterSeconds":2400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.heist_robbery.suspicion.001","category":"heist_robbery","confidence":"suspicion","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["convenience_store"],"text":"Nad {districtName} krouží víc dronů než obvykle. Nikdo nefotí panorama.","expiresAfterSeconds":2400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":1,"priority":50},{"id":"rumor.heist_robbery.suspicion.002","category":"heist_robbery","confidence":"suspicion","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["convenience_store"],"text":"Jedna dodávka projela stejnou ulicí čtyřikrát. Řidič tvrdil, že hledá adresu.","expiresAfterSeconds":2400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.heist_robbery.suspicion.003","category":"heist_robbery","confidence":"suspicion","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["convenience_store"],"text":"V zadní uličce byly nalezeny odstřižené kabely. Krysy používají zuby, profesionálové kleště.","expiresAfterSeconds":2400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.heist_robbery.suspicion.004","category":"heist_robbery","confidence":"suspicion","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["convenience_store"],"text":"Bezpečnostní personál je nervózní. Nervózní hlídač bývá první kapitola loupeže.","expiresAfterSeconds":2400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.heist_robbery.suspicion.005","category":"heist_robbery","confidence":"suspicion","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["convenience_store"],"text":"Někdo testuje alarmy. Město to slyší jako krátké výkřiky kovu.","expiresAfterSeconds":2400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.heist_robbery.rumor.001","category":"heist_robbery","confidence":"rumor","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["convenience_store"],"text":"Prý se chystá heist, po kterém budou bankéři spát s rozsvíceným světlem.","expiresAfterSeconds":2400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.heist_robbery.rumor.002","category":"heist_robbery","confidence":"rumor","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["convenience_store"],"text":"Někdo údajně koupil tři masky a jeden velmi čistý oblek.","expiresAfterSeconds":2400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.heist_robbery.rumor.003","category":"heist_robbery","confidence":"rumor","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["convenience_store"],"text":"Po barech se šeptá o trezoru, který má do rána změnit názor na vlastnictví.","expiresAfterSeconds":2400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.heist_robbery.rumor.004","category":"heist_robbery","confidence":"rumor","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["convenience_store"],"text":"Říká se, že příští velká loupež nebude hlučná. Jen drahá.","expiresAfterSeconds":2400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.heist_robbery.rumor.005","category":"heist_robbery","confidence":"rumor","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["convenience_store"],"text":"Někdo nabízí podíl za mapu ventilace. Nikdo neví, které budovy.","expiresAfterSeconds":2400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.heist_robbery.false_possible.001","category":"heist_robbery","confidence":"false_possible","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["convenience_store"],"text":"Prý se bude krást v každé bance zároveň. Zdroj nezvládá ani dvě kapsy.","expiresAfterSeconds":2400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":50},{"id":"rumor.heist_robbery.false_possible.002","category":"heist_robbery","confidence":"false_possible","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["convenience_store"],"text":"Někdo tvrdí, že trezor už je prázdný. Majitel ho přesto hlídá jako poslední dítě.","expiresAfterSeconds":2400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":50},{"id":"rumor.heist_robbery.false_possible.003","category":"heist_robbery","confidence":"false_possible","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["convenience_store"],"text":"Fáma mluví o tunelu pod celým městem. Bagry o něm nevědí.","expiresAfterSeconds":2400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":50},{"id":"rumor.heist_robbery.false_possible.004","category":"heist_robbery","confidence":"false_possible","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["convenience_store"],"text":"Údajný lup má cenu celého districtu. Údajný zdroj dluží za večeři.","expiresAfterSeconds":2400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":50},{"id":"rumor.heist_robbery.false_possible.005","category":"heist_robbery","confidence":"false_possible","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["convenience_store"],"text":"Říká se, že heist povede duch. Duch si zatím nesehnal řidiče.","expiresAfterSeconds":2400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":50},{"id":"rumor.economy.confirmed.001","category":"economy","confidence":"confirmed","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge","central_bank","stock_exchange"],"text":"Ekonomická aktivita v {districtName} výrazně vzrostla. Peníze tečou a někdo drží kohoutek.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":1,"priority":50},{"id":"rumor.economy.confirmed.002","category":"economy","confidence":"confirmed","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge","central_bank","stock_exchange"],"text":"Místní podniky hlásí silnější provoz. Ulice jsou bohatší a nervóznější.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.economy.confirmed.003","category":"economy","confidence":"confirmed","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge","central_bank","stock_exchange"],"text":"V oblasti proběhlo několik velkých transakcí. Čísla jsou čistá. Původ méně.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.economy.confirmed.004","category":"economy","confidence":"confirmed","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge","central_bank","stock_exchange"],"text":"Směnárny v {districtName} jedou přesčas. Peníze mění barvu rychleji než neon.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":1,"priority":50},{"id":"rumor.economy.confirmed.005","category":"economy","confidence":"confirmed","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge","central_bank","stock_exchange"],"text":"Obchodní provoz se zpomalil. Některé pokladny ztichly dřív než majitelé.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.economy.credible.001","category":"economy","confidence":"credible","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge","central_bank","stock_exchange"],"text":"Říká se, že {playerName} pere peníze tak rychle, že už ani nestíhají schnout.","expiresAfterSeconds":3600,"revealsPlayerName":true,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.economy.credible.002","category":"economy","confidence":"credible","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge","central_bank","stock_exchange"],"text":"V {districtName} se poslední dobou utrácí víc, než se přiznává.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":1,"priority":50},{"id":"rumor.economy.credible.003","category":"economy","confidence":"credible","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge","central_bank","stock_exchange"],"text":"Účetní v okolí mají kruhy pod očima. To je ekonomický ukazatel, který nelže.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.economy.credible.004","category":"economy","confidence":"credible","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge","central_bank","stock_exchange"],"text":"Některé podniky kupují zásoby ve velkém. Buď čekají růst, nebo pohřeb konkurence.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.economy.credible.005","category":"economy","confidence":"credible","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge","central_bank","stock_exchange"],"text":"Místní bankéři zavírají dveře, ale otevírají trezory.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.economy.suspicion.001","category":"economy","confidence":"suspicion","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge","central_bank","stock_exchange"],"text":"Peníze v districtu obíhají podivně. Moc rychle na běžný obchod, moc tiše na legalitu.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.economy.suspicion.002","category":"economy","confidence":"suspicion","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge","central_bank","stock_exchange"],"text":"Několik firem vykázalo náhlý růst. Ve městě roste rychle jen plevel a dluh.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.economy.suspicion.003","category":"economy","confidence":"suspicion","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge","central_bank","stock_exchange"],"text":"Kurýři vozí obálky mezi podniky, které spolu oficiálně nemluví.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.economy.suspicion.004","category":"economy","confidence":"suspicion","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge","central_bank","stock_exchange"],"text":"Pokladny zvoní i po zavírací době.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.economy.suspicion.005","category":"economy","confidence":"suspicion","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge","central_bank","stock_exchange"],"text":"Některé ceny přestaly dávat smysl. To obvykle znamená, že smysl dává někomu jinému.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.economy.rumor.001","category":"economy","confidence":"rumor","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge","central_bank","stock_exchange"],"text":"Mluví se o penězích tak špinavých, že by za ně policie potřebovala rukavice.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.economy.rumor.002","category":"economy","confidence":"rumor","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge","central_bank","stock_exchange"],"text":"Prý má {districtName} nového tichého investora. Tichý investor bývá hlasitý až při výběru dluhu.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":1,"priority":50},{"id":"rumor.economy.rumor.003","category":"economy","confidence":"rumor","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge","central_bank","stock_exchange"],"text":"Někdo údajně kupuje podniky přes prostředníky, kteří nemají prostředky.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.economy.rumor.004","category":"economy","confidence":"rumor","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge","central_bank","stock_exchange"],"text":"Říká se, že v jedné kanceláři se počítají peníze celou noc. Ráno se počítají lidé.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.economy.rumor.005","category":"economy","confidence":"rumor","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge","central_bank","stock_exchange"],"text":"Po městě koluje řeč, že někdo sedí na hromadě cash. Hromady přitahují lopaty.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.economy.false_possible.001","category":"economy","confidence":"false_possible","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge","central_bank","stock_exchange"],"text":"Jeden taxikář tvrdí, že {playerName} vlastní půl města. Druhou půlku prý vlastní jeho bratranec.","expiresAfterSeconds":3600,"revealsPlayerName":true,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":50},{"id":"rumor.economy.false_possible.002","category":"economy","confidence":"false_possible","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge","central_bank","stock_exchange"],"text":"Prý jsou všechny trezory plné. Bankéři se tomu smějí až příliš hlasitě.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":50},{"id":"rumor.economy.false_possible.003","category":"economy","confidence":"false_possible","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge","central_bank","stock_exchange"],"text":"Někdo tvrdí, že peníze prší ze střech. Včera pršely jen účtenky.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":50},{"id":"rumor.economy.false_possible.004","category":"economy","confidence":"false_possible","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge","central_bank","stock_exchange"],"text":"Fáma mluví o tajném zlatém skladu. Zatím se našel jen sklad žluté barvy.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":50},{"id":"rumor.economy.false_possible.005","category":"economy","confidence":"false_possible","allowedAudiences":["current_player","selected_district","global_city"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge","central_bank","stock_exchange"],"text":"Říká se, že ekonomika districtu stojí na jednom člověku. Ten člověk o tom neví.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":50},{"id":"rumor.police_heat.confirmed.001","category":"police_heat","confidence":"confirmed","allowedAudiences":["current_player","selected_district","global_city","police"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge","lobby_club","city_hall"],"text":"Policejní přítomnost v {districtName} zesílila. Sirény zní jako špatně naladěná hymna.","expiresAfterSeconds":2400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":1,"priority":70},{"id":"rumor.police_heat.confirmed.002","category":"police_heat","confidence":"confirmed","allowedAudiences":["current_player","selected_district","global_city","police"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge","lobby_club","city_hall"],"text":"V oblasti proběhla razie. Policie odvezla důkazy, lidi a pár dveří.","expiresAfterSeconds":2400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":70},{"id":"rumor.police_heat.confirmed.003","category":"police_heat","confidence":"confirmed","allowedAudiences":["current_player","selected_district","global_city","police"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge","lobby_club","city_hall"],"text":"District je pod zvýšeným dohledem. Kamery sledují kamery.","expiresAfterSeconds":2400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":70},{"id":"rumor.police_heat.confirmed.004","category":"police_heat","confidence":"confirmed","allowedAudiences":["current_player","selected_district","global_city","police"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge","lobby_club","city_hall"],"text":"Policejní tlak po nedávné akci vzrostl. Město má dlouhou paměť a krátkou pojistku.","expiresAfterSeconds":2400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":70},{"id":"rumor.police_heat.confirmed.005","category":"police_heat","confidence":"confirmed","allowedAudiences":["current_player","selected_district","global_city","police"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge","lobby_club","city_hall"],"text":"Část provozu byla dočasně uzavřena. Policie tvrdí, že kvůli bezpečnosti. Ulice tvrdí něco jiného.","expiresAfterSeconds":2400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":70},{"id":"rumor.police_heat.credible.001","category":"police_heat","confidence":"credible","allowedAudiences":["current_player","selected_district","global_city","police"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge","lobby_club","city_hall"],"text":"Říká se, že policie má {districtName} vysoko na seznamu. Seznam je dlouhý. Trpělivost krátká.","expiresAfterSeconds":2400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":1,"priority":70},{"id":"rumor.police_heat.credible.002","category":"police_heat","confidence":"credible","allowedAudiences":["current_player","selected_district","global_city","police"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge","lobby_club","city_hall"],"text":"Hlídky se střídají častěji a zůstávají déle.","expiresAfterSeconds":2400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":70},{"id":"rumor.police_heat.credible.003","category":"police_heat","confidence":"credible","allowedAudiences":["current_player","selected_district","global_city","police"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge","lobby_club","city_hall"],"text":"Někdo z magistrátu prý začal klást otázky. Otázky od magistrátu obvykle nosí pouta.","expiresAfterSeconds":2400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":70},{"id":"rumor.police_heat.credible.004","category":"police_heat","confidence":"credible","allowedAudiences":["current_player","selected_district","global_city","police"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge","lobby_club","city_hall"],"text":"Policejní drony se vracejí do stejné oblasti. Drony nemají intuici. Jen instrukce.","expiresAfterSeconds":2400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":70},{"id":"rumor.police_heat.credible.005","category":"police_heat","confidence":"credible","allowedAudiences":["current_player","selected_district","global_city","police"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge","lobby_club","city_hall"],"text":"V okolí se objevily neoznačené vozy. Označení chybí, zájem ne.","expiresAfterSeconds":2400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":70},{"id":"rumor.police_heat.suspicion.001","category":"police_heat","confidence":"suspicion","allowedAudiences":["current_player","selected_district","global_city","police"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge","lobby_club","city_hall"],"text":"Policisté v {districtName} přestali pít kávu a začali sledovat vchody.","expiresAfterSeconds":2400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":1,"priority":70},{"id":"rumor.police_heat.suspicion.002","category":"police_heat","confidence":"suspicion","allowedAudiences":["current_player","selected_district","global_city","police"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge","lobby_club","city_hall"],"text":"Několik hlídek dostalo nové trasy. Všechny vedou nepříjemně blízko.","expiresAfterSeconds":2400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":70},{"id":"rumor.police_heat.suspicion.003","category":"police_heat","confidence":"suspicion","allowedAudiences":["current_player","selected_district","global_city","police"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge","lobby_club","city_hall"],"text":"Rádio policie je hlučnější než obvykle. Nikdo neříká proč.","expiresAfterSeconds":2400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":70},{"id":"rumor.police_heat.suspicion.004","category":"police_heat","confidence":"suspicion","allowedAudiences":["current_player","selected_district","global_city","police"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge","lobby_club","city_hall"],"text":"Místní překupníci mizí z ulic. Krysy bývají první, kdo cítí kouř.","expiresAfterSeconds":2400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":70},{"id":"rumor.police_heat.suspicion.005","category":"police_heat","confidence":"suspicion","allowedAudiences":["current_player","selected_district","global_city","police"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge","lobby_club","city_hall"],"text":"V garážích policie se tankuje přesčas.","expiresAfterSeconds":2400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":70},{"id":"rumor.police_heat.rumor.001","category":"police_heat","confidence":"rumor","allowedAudiences":["current_player","selected_district","global_city","police"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge","lobby_club","city_hall"],"text":"Prý se chystá razie, po které budou chybět nejen dveře.","expiresAfterSeconds":2400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":70},{"id":"rumor.police_heat.rumor.002","category":"police_heat","confidence":"rumor","allowedAudiences":["current_player","selected_district","global_city","police"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge","lobby_club","city_hall"],"text":"Někdo tvrdí, že policie má informátora uvnitř {districtName}. Někdo jiný tvrdí, že informátor už nemá jazyk.","expiresAfterSeconds":2400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":true,"weight":1,"priority":70},{"id":"rumor.police_heat.rumor.003","category":"police_heat","confidence":"rumor","allowedAudiences":["current_player","selected_district","global_city","police"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge","lobby_club","city_hall"],"text":"Říká se, že další zásah přijde před svítáním. Svítání to odmítlo komentovat.","expiresAfterSeconds":2400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":70},{"id":"rumor.police_heat.rumor.004","category":"police_heat","confidence":"rumor","allowedAudiences":["current_player","selected_district","global_city","police"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge","lobby_club","city_hall"],"text":"Po městě koluje seznam sledovaných districtů. Každý má jinou verzi.","expiresAfterSeconds":2400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":70},{"id":"rumor.police_heat.rumor.005","category":"police_heat","confidence":"rumor","allowedAudiences":["current_player","selected_district","global_city","police"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge","lobby_club","city_hall"],"text":"Prý někdo nahoře rozhodl, že {playerName} už měl příliš klidnou noc.","expiresAfterSeconds":2400,"revealsPlayerName":true,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":70},{"id":"rumor.police_heat.false_possible.001","category":"police_heat","confidence":"false_possible","allowedAudiences":["current_player","selected_district","global_city","police"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge","lobby_club","city_hall"],"text":"Jeden opilec tvrdí, že policie uzavře celé město. Druhý tvrdí, že první je starosta.","expiresAfterSeconds":2400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":70},{"id":"rumor.police_heat.false_possible.002","category":"police_heat","confidence":"false_possible","allowedAudiences":["current_player","selected_district","global_city","police"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge","lobby_club","city_hall"],"text":"Prý je každý neoznačený vůz policejní. To by z policie dělalo největší taxislužbu ve městě.","expiresAfterSeconds":2400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":70},{"id":"rumor.police_heat.false_possible.003","category":"police_heat","confidence":"false_possible","allowedAudiences":["current_player","selected_district","global_city","police"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge","lobby_club","city_hall"],"text":"Někdo tvrdí, že razie proběhne přesně za hodinu. Zdroj nevlastní hodinky.","expiresAfterSeconds":2400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":70},{"id":"rumor.police_heat.false_possible.004","category":"police_heat","confidence":"false_possible","allowedAudiences":["current_player","selected_district","global_city","police"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge","lobby_club","city_hall"],"text":"Fáma mluví o tajné policejní armádě. Zatím se našli čtyři unavení strážníci.","expiresAfterSeconds":2400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":70},{"id":"rumor.police_heat.false_possible.005","category":"police_heat","confidence":"false_possible","allowedAudiences":["current_player","selected_district","global_city","police"],"allowedSourceBuildings":["restaurant","convenience_store","strip_club","vip_lounge","lobby_club","city_hall"],"text":"Říká se, že Heat už dosáhl maxima. Teploměr se rozbil.","expiresAfterSeconds":2400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":70},{"id":"rumor.alliance_activity.confirmed.001","category":"alliance_activity","confidence":"confirmed","allowedAudiences":["alliance","global_city","current_player"],"allowedSourceBuildings":["strip_club","vip_lounge","lobby_club"],"text":"Členové aliance {allianceName} byli aktivní ve stejné části města. Náhoda už požádala o právníka.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":true,"revealsDistrictName":false,"weight":1,"priority":70},{"id":"rumor.alliance_activity.confirmed.002","category":"alliance_activity","confidence":"confirmed","allowedAudiences":["alliance","global_city","current_player"],"allowedSourceBuildings":["strip_club","vip_lounge","lobby_club"],"text":"Několik districtů aliance změnilo obranné rozložení ve stejnou dobu.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":70},{"id":"rumor.alliance_activity.confirmed.003","category":"alliance_activity","confidence":"confirmed","allowedAudiences":["alliance","global_city","current_player"],"allowedSourceBuildings":["strip_club","vip_lounge","lobby_club"],"text":"Aliance {allianceName} koordinovala přesun lidí a zásob. Město vidí vzorec.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":true,"revealsDistrictName":false,"weight":1,"priority":70},{"id":"rumor.alliance_activity.confirmed.004","category":"alliance_activity","confidence":"confirmed","allowedAudiences":["alliance","global_city","current_player"],"allowedSourceBuildings":["strip_club","vip_lounge","lobby_club"],"text":"Společná operace aliance skončila otevřeným konfliktem.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":70},{"id":"rumor.alliance_activity.confirmed.005","category":"alliance_activity","confidence":"confirmed","allowedAudiences":["alliance","global_city","current_player"],"allowedSourceBuildings":["strip_club","vip_lounge","lobby_club"],"text":"Aliance posílila přítomnost v okolí {zoneName}. Mapa začíná vypadat těsněji.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":70},{"id":"rumor.alliance_activity.credible.001","category":"alliance_activity","confidence":"credible","allowedAudiences":["alliance","global_city","current_player"],"allowedSourceBuildings":["strip_club","vip_lounge","lobby_club"],"text":"Říká se, že {allianceName} dýchá jedním tempem. To je špatná zpráva pro každého poblíž.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":true,"revealsDistrictName":false,"weight":1,"priority":70},{"id":"rumor.alliance_activity.credible.002","category":"alliance_activity","confidence":"credible","allowedAudiences":["alliance","global_city","current_player"],"allowedSourceBuildings":["strip_club","vip_lounge","lobby_club"],"text":"Několik členů aliance nakupuje stejné věci a dívá se stejným směrem.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":70},{"id":"rumor.alliance_activity.credible.003","category":"alliance_activity","confidence":"credible","allowedAudiences":["alliance","global_city","current_player"],"allowedSourceBuildings":["strip_club","vip_lounge","lobby_club"],"text":"Kurýři mezi členy aliance mají plné ruce a prázdné výrazy.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":70},{"id":"rumor.alliance_activity.credible.004","category":"alliance_activity","confidence":"credible","allowedAudiences":["alliance","global_city","current_player"],"allowedSourceBuildings":["strip_club","vip_lounge","lobby_club"],"text":"V {zoneName} se schází příliš mnoho lidí z jedné aliance.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":70},{"id":"rumor.alliance_activity.credible.005","category":"alliance_activity","confidence":"credible","allowedAudiences":["alliance","global_city","current_player"],"allowedSourceBuildings":["strip_club","vip_lounge","lobby_club"],"text":"Někdo rozděluje úkoly. Někdo jiný už nejspíš rozdělil cíle.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":70},{"id":"rumor.alliance_activity.suspicion.001","category":"alliance_activity","confidence":"suspicion","allowedAudiences":["alliance","global_city","current_player"],"allowedSourceBuildings":["strip_club","vip_lounge","lobby_club"],"text":"Členové {allianceName} přestali útočit jednotlivě. Ticho mezi ranami je podezřele přesné.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":true,"revealsDistrictName":false,"weight":1,"priority":70},{"id":"rumor.alliance_activity.suspicion.002","category":"alliance_activity","confidence":"suspicion","allowedAudiences":["alliance","global_city","current_player"],"allowedSourceBuildings":["strip_club","vip_lounge","lobby_club"],"text":"Stejné symboly se objevily na několika místech během jedné noci.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":70},{"id":"rumor.alliance_activity.suspicion.003","category":"alliance_activity","confidence":"suspicion","allowedAudiences":["alliance","global_city","current_player"],"allowedSourceBuildings":["strip_club","vip_lounge","lobby_club"],"text":"Vysílačky aliance jsou aktivní. Obsah není známý. Tón stačí.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":70},{"id":"rumor.alliance_activity.suspicion.004","category":"alliance_activity","confidence":"suspicion","allowedAudiences":["alliance","global_city","current_player"],"allowedSourceBuildings":["strip_club","vip_lounge","lobby_club"],"text":"Několik districtů reagovalo na stejný signál.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":70},{"id":"rumor.alliance_activity.suspicion.005","category":"alliance_activity","confidence":"suspicion","allowedAudiences":["alliance","global_city","current_player"],"allowedSourceBuildings":["strip_club","vip_lounge","lobby_club"],"text":"Aliance se pohybuje, jako by někdo držel mapu a ostatní nože.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":70},{"id":"rumor.alliance_activity.rumor.001","category":"alliance_activity","confidence":"rumor","allowedAudiences":["alliance","global_city","current_player"],"allowedSourceBuildings":["strip_club","vip_lounge","lobby_club"],"text":"Prý {allianceName} plánuje velký tah. Velké tahy dělají velké stíny.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":true,"revealsDistrictName":false,"weight":1,"priority":70},{"id":"rumor.alliance_activity.rumor.002","category":"alliance_activity","confidence":"rumor","allowedAudiences":["alliance","global_city","current_player"],"allowedSourceBuildings":["strip_club","vip_lounge","lobby_club"],"text":"Mluví se o tajné dohodě, která má změnit rovnováhu města.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":70},{"id":"rumor.alliance_activity.rumor.003","category":"alliance_activity","confidence":"rumor","allowedAudiences":["alliance","global_city","current_player"],"allowedSourceBuildings":["strip_club","vip_lounge","lobby_club"],"text":"Někdo tvrdí, že aliance už rozdělila cizí území mezi své členy.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":70},{"id":"rumor.alliance_activity.rumor.004","category":"alliance_activity","confidence":"rumor","allowedAudiences":["alliance","global_city","current_player"],"allowedSourceBuildings":["strip_club","vip_lounge","lobby_club"],"text":"Po městě koluje řeč, že příští útok přijde ze čtyř stran.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":70},{"id":"rumor.alliance_activity.rumor.005","category":"alliance_activity","confidence":"rumor","allowedAudiences":["alliance","global_city","current_player"],"allowedSourceBuildings":["strip_club","vip_lounge","lobby_club"],"text":"Říká se, že {allianceName} už ví, kdo bude první na řadě.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":true,"revealsDistrictName":false,"weight":1,"priority":70},{"id":"rumor.alliance_activity.false_possible.001","category":"alliance_activity","confidence":"false_possible","allowedAudiences":["alliance","global_city","current_player"],"allowedSourceBuildings":["strip_club","vip_lounge","lobby_club"],"text":"Prý se dvě aliance spojily. Obě to popírají a obě mají stejnou tiskovou zprávu.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":70},{"id":"rumor.alliance_activity.false_possible.002","category":"alliance_activity","confidence":"false_possible","allowedAudiences":["alliance","global_city","current_player"],"allowedSourceBuildings":["strip_club","vip_lounge","lobby_club"],"text":"Jeden barman tvrdí, že zná celý plán aliance. Nezná ani vlastní směnu.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":70},{"id":"rumor.alliance_activity.false_possible.003","category":"alliance_activity","confidence":"false_possible","allowedAudiences":["alliance","global_city","current_player"],"allowedSourceBuildings":["strip_club","vip_lounge","lobby_club"],"text":"Fáma říká, že členové aliance sdílejí jednu mysl. Zatím nesdílejí ani účet.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":70},{"id":"rumor.alliance_activity.false_possible.004","category":"alliance_activity","confidence":"false_possible","allowedAudiences":["alliance","global_city","current_player"],"allowedSourceBuildings":["strip_club","vip_lounge","lobby_club"],"text":"Někdo tvrdí, že útok přijde ze všech stran. Geografie to zatím nepotvrdila.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":70},{"id":"rumor.alliance_activity.false_possible.005","category":"alliance_activity","confidence":"false_possible","allowedAudiences":["alliance","global_city","current_player"],"allowedSourceBuildings":["strip_club","vip_lounge","lobby_club"],"text":"Říká se, že aliance má tajného pátého člena. Možná je to účetní.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":70},{"id":"rumor.market.confirmed.001","category":"market","confidence":"confirmed","allowedAudiences":["current_player","global_city"],"allowedSourceBuildings":["stock_exchange"],"text":"Poptávka po {marketCategory} vzrostla. Trh ucítil krev a přidal přirážku.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.market.confirmed.002","category":"market","confidence":"confirmed","allowedAudiences":["current_player","global_city"],"allowedSourceBuildings":["stock_exchange"],"text":"Cena {marketCategory} výrazně klesla. Někdo vykládá zásoby rychleji než svědomí.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.market.confirmed.003","category":"market","confidence":"confirmed","allowedAudiences":["current_player","global_city"],"allowedSourceBuildings":["stock_exchange"],"text":"Na trhu se objevil neobvykle velký objem zboží.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.market.confirmed.004","category":"market","confidence":"confirmed","allowedAudiences":["current_player","global_city"],"allowedSourceBuildings":["stock_exchange"],"text":"Několik obchodníků stáhlo nabídky. Buď čekají růst, nebo návštěvu policie.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.market.confirmed.005","category":"market","confidence":"confirmed","allowedAudiences":["current_player","global_city"],"allowedSourceBuildings":["stock_exchange"],"text":"Černý trh reagoval na nedávné události. Ceny jsou nervózní.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.market.credible.001","category":"market","confidence":"credible","allowedAudiences":["current_player","global_city"],"allowedSourceBuildings":["stock_exchange"],"text":"Říká se, že někdo skupuje {marketCategory} dřív, než si ostatní všimli.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.market.credible.002","category":"market","confidence":"credible","allowedAudiences":["current_player","global_city"],"allowedSourceBuildings":["stock_exchange"],"text":"Obchodníci si šeptají o nedostatku. Nahlas říkají, že je vše v pořádku.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.market.credible.003","category":"market","confidence":"credible","allowedAudiences":["current_player","global_city"],"allowedSourceBuildings":["stock_exchange"],"text":"Několik velkých zákazníků se ptalo na stejné zboží.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.market.credible.004","category":"market","confidence":"credible","allowedAudiences":["current_player","global_city"],"allowedSourceBuildings":["stock_exchange"],"text":"Dodavatelé začali požadovat platbu předem. To je jejich způsob, jak říct: bude zle.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.market.credible.005","category":"market","confidence":"credible","allowedAudiences":["current_player","global_city"],"allowedSourceBuildings":["stock_exchange"],"text":"Trh se naklání jedním směrem. Někdo pod stolem tlačí.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.market.suspicion.001","category":"market","confidence":"suspicion","allowedAudiences":["current_player","global_city"],"allowedSourceBuildings":["stock_exchange"],"text":"Ceny skáčou bez zjevného důvodu. Ve městě je vždycky důvod. Jen nebývá zjevný.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.market.suspicion.002","category":"market","confidence":"suspicion","allowedAudiences":["current_player","global_city"],"allowedSourceBuildings":["stock_exchange"],"text":"Některé položky mizí ze seznamů dřív, než se objeví kupci.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.market.suspicion.003","category":"market","confidence":"suspicion","allowedAudiences":["current_player","global_city"],"allowedSourceBuildings":["stock_exchange"],"text":"Překupníci odmítají prodávat obvyklé množství.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.market.suspicion.004","category":"market","confidence":"suspicion","allowedAudiences":["current_player","global_city"],"allowedSourceBuildings":["stock_exchange"],"text":"Na trhu koluje víc hotovosti a méně zboží.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.market.suspicion.005","category":"market","confidence":"suspicion","allowedAudiences":["current_player","global_city"],"allowedSourceBuildings":["stock_exchange"],"text":"Několik cen se změnilo během jedné hodiny. To není ekonomika. To je panika v obleku.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.market.rumor.001","category":"market","confidence":"rumor","allowedAudiences":["current_player","global_city"],"allowedSourceBuildings":["stock_exchange"],"text":"Prý někdo chystá umělý nedostatek.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.market.rumor.002","category":"market","confidence":"rumor","allowedAudiences":["current_player","global_city"],"allowedSourceBuildings":["stock_exchange"],"text":"Říká se, že další dodávka nepřijde.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.market.rumor.003","category":"market","confidence":"rumor","allowedAudiences":["current_player","global_city"],"allowedSourceBuildings":["stock_exchange"],"text":"Někdo údajně koupil všechno, co mělo kov, kabel nebo spoušť.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.market.rumor.004","category":"market","confidence":"rumor","allowedAudiences":["current_player","global_city"],"allowedSourceBuildings":["stock_exchange"],"text":"Po městě se šeptá, že zítřejší ceny budou bolet víc než dnešní.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.market.rumor.005","category":"market","confidence":"rumor","allowedAudiences":["current_player","global_city"],"allowedSourceBuildings":["stock_exchange"],"text":"Prý se na trhu chystá finanční nástrah. Ne ta v districtu. Ta finanční.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.market.false_possible.001","category":"market","confidence":"false_possible","allowedAudiences":["current_player","global_city"],"allowedSourceBuildings":["stock_exchange"],"text":"Jeden obchodník tvrdí, že ceny zítra klesnou na polovinu. Dnes zdvojnásobil svoje.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":50},{"id":"rumor.market.false_possible.002","category":"market","confidence":"false_possible","allowedAudiences":["current_player","global_city"],"allowedSourceBuildings":["stock_exchange"],"text":"Prý už není žádné zboží. Sklady se tomu smějí.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":50},{"id":"rumor.market.false_possible.003","category":"market","confidence":"false_possible","allowedAudiences":["current_player","global_city"],"allowedSourceBuildings":["stock_exchange"],"text":"Někdo tvrdí, že trh ovládá jediný člověk. Ten člověk momentálně hledá drobné.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":50},{"id":"rumor.market.false_possible.004","category":"market","confidence":"false_possible","allowedAudiences":["current_player","global_city"],"allowedSourceBuildings":["stock_exchange"],"text":"Fáma mluví o kolapsu. Trh mezitím prodává normálně, jen dráž.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":50},{"id":"rumor.market.false_possible.005","category":"market","confidence":"false_possible","allowedAudiences":["current_player","global_city"],"allowedSourceBuildings":["stock_exchange"],"text":"Říká se, že všechny ceny jsou podvod. V tomhle městě je to spíš filozofie než zpráva.","expiresAfterSeconds":3600,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":50},{"id":"rumor.downtown_power.confirmed.001","category":"downtown_power","confidence":"confirmed","allowedAudiences":["current_player","global_city","alliance"],"allowedSourceBuildings":["strip_club","vip_lounge","lobby_club","city_hall","central_bank"],"text":"V Downtownu proběhla neveřejná schůzka několika vlivných lidí. Veřejnost nebyla pozvána. Důsledky ano.","expiresAfterSeconds":5400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.downtown_power.confirmed.002","category":"downtown_power","confidence":"confirmed","allowedAudiences":["current_player","global_city","alliance"],"allowedSourceBuildings":["strip_club","vip_lounge","lobby_club","city_hall","central_bank"],"text":"Magistrát změnil přístup k jedné z městských oblastí.","expiresAfterSeconds":5400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.downtown_power.confirmed.003","category":"downtown_power","confidence":"confirmed","allowedAudiences":["current_player","global_city","alliance"],"allowedSourceBuildings":["strip_club","vip_lounge","lobby_club","city_hall","central_bank"],"text":"V soudních kancelářích přibylo případů spojených s gangy.","expiresAfterSeconds":5400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.downtown_power.confirmed.004","category":"downtown_power","confidence":"confirmed","allowedAudiences":["current_player","global_city","alliance"],"allowedSourceBuildings":["strip_club","vip_lounge","lobby_club","city_hall","central_bank"],"text":"Centrální banka zaznamenala neobvyklé finanční pohyby.","expiresAfterSeconds":5400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.downtown_power.confirmed.005","category":"downtown_power","confidence":"confirmed","allowedAudiences":["current_player","global_city","alliance"],"allowedSourceBuildings":["strip_club","vip_lounge","lobby_club","city_hall","central_bank"],"text":"VIP podniky v Downtownu hlásí zvýšenou návštěvnost významných osob.","expiresAfterSeconds":5400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.downtown_power.credible.001","category":"downtown_power","confidence":"credible","allowedAudiences":["current_player","global_city","alliance"],"allowedSourceBuildings":["strip_club","vip_lounge","lobby_club","city_hall","central_bank"],"text":"Říká se, že někdo kupuje vliv rychleji, než ho město stíhá prodávat.","expiresAfterSeconds":5400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.downtown_power.credible.002","category":"downtown_power","confidence":"credible","allowedAudiences":["current_player","global_city","alliance"],"allowedSourceBuildings":["strip_club","vip_lounge","lobby_club","city_hall","central_bank"],"text":"V lobby klubu se mluvilo šeptem a platilo v hotovosti.","expiresAfterSeconds":5400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.downtown_power.credible.003","category":"downtown_power","confidence":"credible","allowedAudiences":["current_player","global_city","alliance"],"allowedSourceBuildings":["strip_club","vip_lounge","lobby_club","city_hall","central_bank"],"text":"Několik úředníků změnilo názor během jedné večeře.","expiresAfterSeconds":5400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.downtown_power.credible.004","category":"downtown_power","confidence":"credible","allowedAudiences":["current_player","global_city","alliance"],"allowedSourceBuildings":["strip_club","vip_lounge","lobby_club","city_hall","central_bank"],"text":"Soudci dnes chodí s ochrankou. To je buď opatrnost, nebo přiznání.","expiresAfterSeconds":5400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.downtown_power.credible.005","category":"downtown_power","confidence":"credible","allowedAudiences":["current_player","global_city","alliance"],"allowedSourceBuildings":["strip_club","vip_lounge","lobby_club","city_hall","central_bank"],"text":"V bance se otevřely dveře, které běžně nemají kliku.","expiresAfterSeconds":5400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.downtown_power.suspicion.001","category":"downtown_power","confidence":"suspicion","allowedAudiences":["current_player","global_city","alliance"],"allowedSourceBuildings":["strip_club","vip_lounge","lobby_club","city_hall","central_bank"],"text":"V Downtownu se pohybují lidé, kteří nikdy nečekají ve frontě.","expiresAfterSeconds":5400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.downtown_power.suspicion.002","category":"downtown_power","confidence":"suspicion","allowedAudiences":["current_player","global_city","alliance"],"allowedSourceBuildings":["strip_club","vip_lounge","lobby_club","city_hall","central_bank"],"text":"Několik politiků odvolalo program. Náhradní program nebyl zveřejněn.","expiresAfterSeconds":5400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.downtown_power.suspicion.003","category":"downtown_power","confidence":"suspicion","allowedAudiences":["current_player","global_city","alliance"],"allowedSourceBuildings":["strip_club","vip_lounge","lobby_club","city_hall","central_bank"],"text":"VIP salonek zavřel celé patro pro soukromou společnost.","expiresAfterSeconds":5400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.downtown_power.suspicion.004","category":"downtown_power","confidence":"suspicion","allowedAudiences":["current_player","global_city","alliance"],"allowedSourceBuildings":["strip_club","vip_lounge","lobby_club","city_hall","central_bank"],"text":"Úřední auta parkují na místech, kde obvykle parkují jen problémy.","expiresAfterSeconds":5400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.downtown_power.suspicion.005","category":"downtown_power","confidence":"suspicion","allowedAudiences":["current_player","global_city","alliance"],"allowedSourceBuildings":["strip_club","vip_lounge","lobby_club","city_hall","central_bank"],"text":"V soudní budově se svítí dlouho do noci.","expiresAfterSeconds":5400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.downtown_power.rumor.001","category":"downtown_power","confidence":"rumor","allowedAudiences":["current_player","global_city","alliance"],"allowedSourceBuildings":["strip_club","vip_lounge","lobby_club","city_hall","central_bank"],"text":"Prý někdo koupil rozhodnutí dřív, než bylo napsáno.","expiresAfterSeconds":5400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.downtown_power.rumor.002","category":"downtown_power","confidence":"rumor","allowedAudiences":["current_player","global_city","alliance"],"allowedSourceBuildings":["strip_club","vip_lounge","lobby_club","city_hall","central_bank"],"text":"Říká se, že magistrát už ví, kdo bude příští obětní beránek.","expiresAfterSeconds":5400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.downtown_power.rumor.003","category":"downtown_power","confidence":"rumor","allowedAudiences":["current_player","global_city","alliance"],"allowedSourceBuildings":["strip_club","vip_lounge","lobby_club","city_hall","central_bank"],"text":"Někdo tvrdí, že jeden podpis může zastavit razii. Podpis stojí víc než district.","expiresAfterSeconds":5400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.downtown_power.rumor.004","category":"downtown_power","confidence":"rumor","allowedAudiences":["current_player","global_city","alliance"],"allowedSourceBuildings":["strip_club","vip_lounge","lobby_club","city_hall","central_bank"],"text":"Po Downtownu koluje řeč o nové dohodě mezi penězi a mocí.","expiresAfterSeconds":5400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.downtown_power.rumor.005","category":"downtown_power","confidence":"rumor","allowedAudiences":["current_player","global_city","alliance"],"allowedSourceBuildings":["strip_club","vip_lounge","lobby_club","city_hall","central_bank"],"text":"Prý někdo vlastní polovinu soudců. Druhou polovinu si jen pronajímá.","expiresAfterSeconds":5400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":50},{"id":"rumor.downtown_power.false_possible.001","category":"downtown_power","confidence":"false_possible","allowedAudiences":["current_player","global_city","alliance"],"allowedSourceBuildings":["strip_club","vip_lounge","lobby_club","city_hall","central_bank"],"text":"Jeden číšník tvrdí, že slyšel státní tajemství. Stál u špatného stolu.","expiresAfterSeconds":5400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":50},{"id":"rumor.downtown_power.false_possible.002","category":"downtown_power","confidence":"false_possible","allowedAudiences":["current_player","global_city","alliance"],"allowedSourceBuildings":["strip_club","vip_lounge","lobby_club","city_hall","central_bank"],"text":"Prý banka tiskne peníze pro gangy. Tiskárna momentálně tiskne formuláře.","expiresAfterSeconds":5400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":50},{"id":"rumor.downtown_power.false_possible.003","category":"downtown_power","confidence":"false_possible","allowedAudiences":["current_player","global_city","alliance"],"allowedSourceBuildings":["strip_club","vip_lounge","lobby_club","city_hall","central_bank"],"text":"Někdo tvrdí, že magistrát řídí umělá inteligence. Úřední tempo tomu neodpovídá.","expiresAfterSeconds":5400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":50},{"id":"rumor.downtown_power.false_possible.004","category":"downtown_power","confidence":"false_possible","allowedAudiences":["current_player","global_city","alliance"],"allowedSourceBuildings":["strip_club","vip_lounge","lobby_club","city_hall","central_bank"],"text":"Fáma říká, že soudce zmizel. Soudce byl na obědě.","expiresAfterSeconds":5400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":50},{"id":"rumor.downtown_power.false_possible.005","category":"downtown_power","confidence":"false_possible","allowedAudiences":["current_player","global_city","alliance"],"allowedSourceBuildings":["strip_club","vip_lounge","lobby_club","city_hall","central_bank"],"text":"Říká se, že Downtown ovládá jediný muž. Ve skutečnosti se o to hádá příliš mnoho mužů.","expiresAfterSeconds":5400,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":50},{"id":"rumor.final_lockdown.confirmed.001","category":"final_lockdown","confidence":"confirmed","allowedAudiences":["global_city","current_player","alliance","police"],"allowedSourceBuildings":["city_hall"],"text":"Final Lockdown byl zahájen. Město zavřelo dveře a nechalo uvnitř všechny chyby.","expiresAfterSeconds":10800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":100},{"id":"rumor.final_lockdown.confirmed.002","category":"final_lockdown","confidence":"confirmed","allowedAudiences":["global_city","current_player","alliance","police"],"allowedSourceBuildings":["city_hall"],"text":"Bezpečnostní režim města se zpřísnil. Každý pohyb teď vrhá delší stín.","expiresAfterSeconds":10800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":100},{"id":"rumor.final_lockdown.confirmed.003","category":"final_lockdown","confidence":"confirmed","allowedAudiences":["global_city","current_player","alliance","police"],"allowedSourceBuildings":["city_hall"],"text":"Zbývající mocnosti byly oficiálně označeny jako hlavní soupeři.","expiresAfterSeconds":10800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":100},{"id":"rumor.final_lockdown.confirmed.004","category":"final_lockdown","confidence":"confirmed","allowedAudiences":["global_city","current_player","alliance","police"],"allowedSourceBuildings":["city_hall"],"text":"Některé oblasti byly uzavřeny. Město se zmenšuje. Nenávist ne.","expiresAfterSeconds":10800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":100},{"id":"rumor.final_lockdown.confirmed.005","category":"final_lockdown","confidence":"confirmed","allowedAudiences":["global_city","current_player","alliance","police"],"allowedSourceBuildings":["city_hall"],"text":"Finální střet se přiblížil. Na mapě ubývá barev a přibývá jmen mrtvých.","expiresAfterSeconds":10800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":100},{"id":"rumor.final_lockdown.credible.001","category":"final_lockdown","confidence":"credible","allowedAudiences":["global_city","current_player","alliance","police"],"allowedSourceBuildings":["city_hall"],"text":"Říká se, že město už vybralo svoje poslední predátory.","expiresAfterSeconds":10800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":100},{"id":"rumor.final_lockdown.credible.002","category":"final_lockdown","confidence":"credible","allowedAudiences":["global_city","current_player","alliance","police"],"allowedSourceBuildings":["city_hall"],"text":"Hlídky se stahují k rozhodujícím bodům.","expiresAfterSeconds":10800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":100},{"id":"rumor.final_lockdown.credible.003","category":"final_lockdown","confidence":"credible","allowedAudiences":["global_city","current_player","alliance","police"],"allowedSourceBuildings":["city_hall"],"text":"Aliance přestávají skrývat své skutečné hranice.","expiresAfterSeconds":10800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":100},{"id":"rumor.final_lockdown.credible.004","category":"final_lockdown","confidence":"credible","allowedAudiences":["global_city","current_player","alliance","police"],"allowedSourceBuildings":["city_hall"],"text":"Některé distrikty se připravují na poslední noc.","expiresAfterSeconds":10800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":100},{"id":"rumor.final_lockdown.credible.005","category":"final_lockdown","confidence":"credible","allowedAudiences":["global_city","current_player","alliance","police"],"allowedSourceBuildings":["city_hall"],"text":"V ulicích je cítit konec. Má kovovou chuť.","expiresAfterSeconds":10800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":100},{"id":"rumor.final_lockdown.suspicion.001","category":"final_lockdown","confidence":"suspicion","allowedAudiences":["global_city","current_player","alliance","police"],"allowedSourceBuildings":["city_hall"],"text":"V centru se instalují nové zátarasy.","expiresAfterSeconds":10800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":100},{"id":"rumor.final_lockdown.suspicion.002","category":"final_lockdown","confidence":"suspicion","allowedAudiences":["global_city","current_player","alliance","police"],"allowedSourceBuildings":["city_hall"],"text":"Komunikační kanály jsou přetížené.","expiresAfterSeconds":10800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":100},{"id":"rumor.final_lockdown.suspicion.003","category":"final_lockdown","confidence":"suspicion","allowedAudiences":["global_city","current_player","alliance","police"],"allowedSourceBuildings":["city_hall"],"text":"Silní hráči stahují zdroje blíž k jádru svého území.","expiresAfterSeconds":10800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":100},{"id":"rumor.final_lockdown.suspicion.004","category":"final_lockdown","confidence":"suspicion","allowedAudiences":["global_city","current_player","alliance","police"],"allowedSourceBuildings":["city_hall"],"text":"Menší gangy mizí z mapy nebo pod cizími barvami.","expiresAfterSeconds":10800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":100},{"id":"rumor.final_lockdown.suspicion.005","category":"final_lockdown","confidence":"suspicion","allowedAudiences":["global_city","current_player","alliance","police"],"allowedSourceBuildings":["city_hall"],"text":"Město zní tišeji. To není klid. To je nádech.","expiresAfterSeconds":10800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":100},{"id":"rumor.final_lockdown.rumor.001","category":"final_lockdown","confidence":"rumor","allowedAudiences":["global_city","current_player","alliance","police"],"allowedSourceBuildings":["city_hall"],"text":"Prý zbývá poslední velký tah.","expiresAfterSeconds":10800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":100},{"id":"rumor.final_lockdown.rumor.002","category":"final_lockdown","confidence":"rumor","allowedAudiences":["global_city","current_player","alliance","police"],"allowedSourceBuildings":["city_hall"],"text":"Říká se, že vítěz už byl vybrán. Vítěz o tom zatím neví.","expiresAfterSeconds":10800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":100},{"id":"rumor.final_lockdown.rumor.003","category":"final_lockdown","confidence":"rumor","allowedAudiences":["global_city","current_player","alliance","police"],"allowedSourceBuildings":["city_hall"],"text":"Někdo tvrdí, že poslední noc bude kratší než všechny předchozí.","expiresAfterSeconds":10800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":100},{"id":"rumor.final_lockdown.rumor.004","category":"final_lockdown","confidence":"rumor","allowedAudiences":["global_city","current_player","alliance","police"],"allowedSourceBuildings":["city_hall"],"text":"Po městě koluje řeč, že některé aliance se rozpadnou ještě před bojem.","expiresAfterSeconds":10800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":100},{"id":"rumor.final_lockdown.rumor.005","category":"final_lockdown","confidence":"rumor","allowedAudiences":["global_city","current_player","alliance","police"],"allowedSourceBuildings":["city_hall"],"text":"Prý se brzy ukáže, kdo stavěl impérium a kdo jen světelnou reklamu.","expiresAfterSeconds":10800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":100},{"id":"rumor.final_lockdown.false_possible.001","category":"final_lockdown","confidence":"false_possible","allowedAudiences":["global_city","current_player","alliance","police"],"allowedSourceBuildings":["city_hall"],"text":"Jeden prorok tvrdí, že město padne o půlnoci. Prorok si nepamatuje datum.","expiresAfterSeconds":10800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":100},{"id":"rumor.final_lockdown.false_possible.002","category":"final_lockdown","confidence":"false_possible","allowedAudiences":["global_city","current_player","alliance","police"],"allowedSourceBuildings":["city_hall"],"text":"Prý se všechny distrikty zamknou najednou. Zámečníci protestují.","expiresAfterSeconds":10800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":100},{"id":"rumor.final_lockdown.false_possible.003","category":"final_lockdown","confidence":"false_possible","allowedAudiences":["global_city","current_player","alliance","police"],"allowedSourceBuildings":["city_hall"],"text":"Někdo tvrdí, že vítěz dostane celé město bez odporu. Město se hlasitě zasmálo.","expiresAfterSeconds":10800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":100},{"id":"rumor.final_lockdown.false_possible.004","category":"final_lockdown","confidence":"false_possible","allowedAudiences":["global_city","current_player","alliance","police"],"allowedSourceBuildings":["city_hall"],"text":"Fáma říká, že policie odejde. Policie objednala další munici.","expiresAfterSeconds":10800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":100},{"id":"rumor.final_lockdown.false_possible.005","category":"final_lockdown","confidence":"false_possible","allowedAudiences":["global_city","current_player","alliance","police"],"allowedSourceBuildings":["city_hall"],"text":"Říká se, že konec bude spravedlivý. To je ze všech drbů ten nejméně uvěřitelný.","expiresAfterSeconds":10800,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":0.5,"priority":100},{"id":"rumor.atmosphere.rumor.001","category":"atmosphere","confidence":"rumor","allowedAudiences":["global_city","selected_district","current_player"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Ve městě dnes prší. Déšť smývá krev, ne dluhy.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":1},{"id":"rumor.atmosphere.rumor.002","category":"atmosphere","confidence":"rumor","allowedAudiences":["global_city","selected_district","current_player"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Neon svítí nejjasněji nad místy, kde lidé mizí.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":1},{"id":"rumor.atmosphere.rumor.003","category":"atmosphere","confidence":"rumor","allowedAudiences":["global_city","selected_district","current_player"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Někdo dnes vydělal jmění. Někdo jiný se stal jeho účetní položkou.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":1},{"id":"rumor.atmosphere.rumor.004","category":"atmosphere","confidence":"rumor","allowedAudiences":["global_city","selected_district","current_player"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Sirény křičí. Ulice dělají, že neslyší.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":1},{"id":"rumor.atmosphere.rumor.005","category":"atmosphere","confidence":"rumor","allowedAudiences":["global_city","selected_district","current_player"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Každý má plán, dokud se neotevřou dveře.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":1},{"id":"rumor.atmosphere.rumor.006","category":"atmosphere","confidence":"rumor","allowedAudiences":["global_city","selected_district","current_player"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Město nikdy nespí. Jen občas předstírá mrtvého.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":1},{"id":"rumor.atmosphere.rumor.007","category":"atmosphere","confidence":"rumor","allowedAudiences":["global_city","selected_district","current_player"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Dnešní pravda bude zítra levná fáma.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":1},{"id":"rumor.atmosphere.rumor.008","category":"atmosphere","confidence":"rumor","allowedAudiences":["global_city","selected_district","current_player"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Když zhasnou kamery, začínají příběhy.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":1},{"id":"rumor.atmosphere.rumor.009","category":"atmosphere","confidence":"rumor","allowedAudiences":["global_city","selected_district","current_player"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Některé dluhy se neplatí penězi.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":1},{"id":"rumor.atmosphere.rumor.010","category":"atmosphere","confidence":"rumor","allowedAudiences":["global_city","selected_district","current_player"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"V Empire Streets má i ticho sériové číslo.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":1},{"id":"rumor.atmosphere.rumor.011","category":"atmosphere","confidence":"rumor","allowedAudiences":["global_city","selected_district","current_player"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Peníze mluví. Zbraně přerušují.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":1},{"id":"rumor.atmosphere.rumor.012","category":"atmosphere","confidence":"rumor","allowedAudiences":["global_city","selected_district","current_player"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Každý district má srdce. Některá jsou neprůstřelná.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":1},{"id":"rumor.atmosphere.rumor.013","category":"atmosphere","confidence":"rumor","allowedAudiences":["global_city","selected_district","current_player"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Město má dlouhou paměť a krátký seznam přátel.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":1},{"id":"rumor.atmosphere.rumor.014","category":"atmosphere","confidence":"rumor","allowedAudiences":["global_city","selected_district","current_player"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Někdo dnes otevřel nový podnik. Někdo jiný nový hrob.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":1},{"id":"rumor.atmosphere.rumor.015","category":"atmosphere","confidence":"rumor","allowedAudiences":["global_city","selected_district","current_player"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Kdo tvrdí, že se nebojí, obvykle jen mluví příliš nahlas.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":1},{"id":"rumor.atmosphere.rumor.016","category":"atmosphere","confidence":"rumor","allowedAudiences":["global_city","selected_district","current_player"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Noc je mladá. Někteří lidé už ne.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":1},{"id":"rumor.atmosphere.rumor.017","category":"atmosphere","confidence":"rumor","allowedAudiences":["global_city","selected_district","current_player"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Všichni chtějí korunu. Málokdo kontroluje, jestli není z ostnatého drátu.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":1},{"id":"rumor.atmosphere.rumor.018","category":"atmosphere","confidence":"rumor","allowedAudiences":["global_city","selected_district","current_player"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Zítra bude mapa vypadat jinak. Mapa s tím nesouhlasí.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":1},{"id":"rumor.atmosphere.rumor.019","category":"atmosphere","confidence":"rumor","allowedAudiences":["global_city","selected_district","current_player"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Ve městě není nouze o hrdiny. Jen o živé hrdiny.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":1},{"id":"rumor.atmosphere.rumor.020","category":"atmosphere","confidence":"rumor","allowedAudiences":["global_city","selected_district","current_player"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Každá aliance začíná podáním ruky a končí kontrolou zásobníku.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":1},{"id":"rumor.atmosphere.rumor.021","category":"atmosphere","confidence":"rumor","allowedAudiences":["global_city","selected_district","current_player"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Důvěra je měna, kterou nikdo nepřijímá.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":1},{"id":"rumor.atmosphere.rumor.022","category":"atmosphere","confidence":"rumor","allowedAudiences":["global_city","selected_district","current_player"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Někde právě někdo lže. Statisticky pravděpodobně všude.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":1},{"id":"rumor.atmosphere.rumor.023","category":"atmosphere","confidence":"rumor","allowedAudiences":["global_city","selected_district","current_player"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Černý trh zavírá pozdě. Hřbitovy nikdy.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":1},{"id":"rumor.atmosphere.rumor.024","category":"atmosphere","confidence":"rumor","allowedAudiences":["global_city","selected_district","current_player"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Když gang mlčí, poslouchej jeho auta.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":1},{"id":"rumor.atmosphere.rumor.025","category":"atmosphere","confidence":"rumor","allowedAudiences":["global_city","selected_district","current_player"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Město je plné dveří. Většina vede k problémům.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":1},{"id":"rumor.atmosphere.rumor.026","category":"atmosphere","confidence":"rumor","allowedAudiences":["global_city","selected_district","current_player"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Každá barva na mapě jednou ztmavne.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":1},{"id":"rumor.atmosphere.rumor.027","category":"atmosphere","confidence":"rumor","allowedAudiences":["global_city","selected_district","current_player"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Ulice nevolí vítěze. Jen uklízejí poražené.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":1},{"id":"rumor.atmosphere.rumor.028","category":"atmosphere","confidence":"rumor","allowedAudiences":["global_city","selected_district","current_player"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Nikdo nevlastní district navždy. Někteří ho jen drží déle.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":1},{"id":"rumor.atmosphere.rumor.029","category":"atmosphere","confidence":"rumor","allowedAudiences":["global_city","selected_district","current_player"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Ve sklepích se počítají zásoby. Na střechách cíle.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":1},{"id":"rumor.atmosphere.rumor.030","category":"atmosphere","confidence":"rumor","allowedAudiences":["global_city","selected_district","current_player"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Dnes je dobrý den na obchod. Špatný den na důvěru.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":1},{"id":"rumor.atmosphere.rumor.031","category":"atmosphere","confidence":"rumor","allowedAudiences":["global_city","selected_district","current_player"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Někdo zvedl Heat. Policie zvedla obočí.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":1},{"id":"rumor.atmosphere.rumor.032","category":"atmosphere","confidence":"rumor","allowedAudiences":["global_city","selected_district","current_player"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Město miluje ambice. Jejich pohřby jsou velkolepé.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":1},{"id":"rumor.atmosphere.rumor.033","category":"atmosphere","confidence":"rumor","allowedAudiences":["global_city","selected_district","current_player"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Vliv otevírá dveře. Strach je nechává otevřené.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":1},{"id":"rumor.atmosphere.rumor.034","category":"atmosphere","confidence":"rumor","allowedAudiences":["global_city","selected_district","current_player"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Každý má cenu. Někteří jen nemají cenovku.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":1},{"id":"rumor.atmosphere.rumor.035","category":"atmosphere","confidence":"rumor","allowedAudiences":["global_city","selected_district","current_player"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Kdo ovládá informace, vybírá místo střelby.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":1},{"id":"rumor.atmosphere.rumor.036","category":"atmosphere","confidence":"rumor","allowedAudiences":["global_city","selected_district","current_player"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Nejdražší chyba ve městě je pocit bezpečí.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":1},{"id":"rumor.atmosphere.rumor.037","category":"atmosphere","confidence":"rumor","allowedAudiences":["global_city","selected_district","current_player"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"V noci jsou všechny dohody černobílé. Krev dodá barvu.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":1},{"id":"rumor.atmosphere.rumor.038","category":"atmosphere","confidence":"rumor","allowedAudiences":["global_city","selected_district","current_player"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Někdo právě vyhrál spor. Druhá strana zmizela.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":1},{"id":"rumor.atmosphere.rumor.039","category":"atmosphere","confidence":"rumor","allowedAudiences":["global_city","selected_district","current_player"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Ulice dnes působí klidně. To je vždycky podezřelé.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":1},{"id":"rumor.atmosphere.rumor.040","category":"atmosphere","confidence":"rumor","allowedAudiences":["global_city","selected_district","current_player"],"allowedSourceBuildings":["restaurant","convenience_store"],"text":"Empire Streets odpouští všechno kromě slabosti.","expiresAfterSeconds":900,"revealsPlayerName":false,"revealsAllianceName":false,"revealsDistrictName":false,"weight":1,"priority":1}]');
  ...
  const validateStreetDealersConfig = (config) => {
    ...
    if (!dealers) return;
    const values2 = createReplacementValueResolver(config);
    ...
    for (const [index, resourceKey] of EXPECTED_SLOT_RESOURCES.entries()) {
      ...
      const fullCost = values2.resolve(resourceKey);
  ...
  const validateConflictConfig = (conflict) => {
  ...
  const width = 1600;
  const height = 980;
  const districts = /* @__PURE__ */ JSON.parse('[{"id":"district:1","legacyId":1,"rowIndex":0,"columnIndex":0,"name":"District 1","zone":"commercial","polygon":[{"x":0,"y":48},{"x":15.732,"y":48},{"x":31.463,"y":54.656},{"x":47.195,"y":48},{"x":62.926,"y":48},{"x":78.658,"y":48},{"x":80.933,"y":69.92},{"x":79.08,"y":92.168},{"x":76.895,"y":114.441},{"x":90.595,"y":135.457},{"x":87.357,"y":157.814},{"x":78.815,"y":165.734},{"x":66.837,"y":164.294},{"x":58.06,"y":171.573},{"x":48.411,"y":176.477},{"x":36.91,"y":176.338},{"x":29.684,"y":174.17},{"x":21.68,"y":178.144},{"x":14.63,"y":174.593},{"x":7.504,"y":171.636},{"x":0,"y":171.661},{"x":1.907,"y":146.929},{"x":0,"y":122.197},{"x":0,"y":97.465},{"x":1.498,"y":72.732}],"neighborIds":["district:2","district:24","district:25"],"spawnZones":["west"],"isSpawnCandidate":true},{"id":"district:2","legacyId":2,"rowIndex":0,"columnIndex":1,"name":"District 2","zone":"park","polygon":[{"x":78.658,"y":48},{"x":95.936,"y":50.187},{"x":113.214,"y":48},{"x":130.493,"y":56.138},{"x":147.771,"y":50.597},{"x":165.049,"y":48},{"x":157.664,"y":71.356},{"x":147.601,"y":94.237},{"x":158.207,"y":120.788},{"x":149.313,"y":143.877},{"x":143.817,"y":167.569},{"x":142.284,"y":167.4},{"x":140.849,"y":167.54},{"x":139.66,"y":168.445},{"x":138.541,"y":169.57},{"x":137.108,"y":169.716},{"x":127.83,"y":164.526},{"x":116.75,"y":166.867},{"x":105.956,"y":168.012},{"x":98.067,"y":157.018},{"x":87.357,"y":157.814},{"x":90.595,"y":135.457},{"x":76.895,"y":114.441},{"x":79.08,"y":92.168},{"x":80.933,"y":69.92}],"neighborIds":["district:1","district:3","district:25","district:26"],"spawnZones":["west"],"isSpawnCandidate":true},{"id":"district:3","legacyId":3,"rowIndex":0,"columnIndex":2,"name":"District 3","zone":"industrial","polygon":[{"x":165.049,"y":48},{"x":176.416,"y":48},{"x":187.782,"y":53.859},{"x":199.149,"y":49.922},{"x":210.516,"y":48},{"x":221.883,"y":48},{"x":
.938},{"x":1408.429,"y":846.172},{"x":1411.3,"y":846.566},{"x":1413.863,"y":844.868},{"x":1416.473,"y":843.487},{"x":1419.294,"y":843.541},{"x":1422.197,"y":844.144},{"x":1425.18,"y":845.656},{"x":1429.305,"y":843.636},{"x":1432.089,"y":845.766},{"x":1434.651,"y":848.586},{"x":1437.918,"y":849.221},{"x":1455.576,"y":874.367},{"x":1469.246,"y":901.954},{"x":1481.896,"y":930.164},{"x":1497.375,"y":956.645}],"neighborIds":["district:136","district:137","district:158","district:160"],"spawnZones":["east","south"],"isSpawnCandidate":true},{"id":"district:160","legacyId":160,"rowIndex":6,"columnIndex":21,"name":"District 160","zone":"residential","polygon":[{"x":1552.604,"y":835.318},{"x":1546.931,"y":864.161},{"x":1537.582,"y":892.35},{"x":1528.558,"y":920.597},{"x":1535.874,"y":951.751},{"x":1526.863,"y":980},{"x":1525.082,"y":980},{"x":1523.301,"y":980},{"x":1521.52,"y":979.857},{"x":1519.739,"y":979.197},{"x":1517.958,"y":980},{"x":1497.375,"y":956.645},{"x":1481.896,"y":930.164},{"x":1469.246,"y":901.954},{"x":1455.576,"y":874.367},{"x":1437.918,"y":849.221},{"x":1457.79,"y":839.801},{"x":1480.643,"y":847.445},{"x":1500.846,"y":839.92},{"x":1520.502,"y":829.262},{"x":1542.321,"y":830.985},{"x":1544.773,"y":830.913},{"x":1546.767,"y":831.928},{"x":1548.516,"y":833.524},{"x":1550.374,"y":834.864}],"neighborIds":["district:137","district:138","district:159","district:161"],"spawnZones":["east","south"],"isSpawnCandidate":true},{"id":"district:161","legacyId":161,"rowIndex":6,"columnIndex":22,"name":"District 161","zone":"industrial","polygon":[{"x":1600,"y":828.781},{"x":1597.583,"y":859.024},{"x":1600,"y":889.268},{"x":1600,"y":919.512},{"x":1600,"y":949.756},{"x":1600,"y":980},{"x":1585.373,"y":979.291},{"x":1570.745,"y":971.642},{"x":1556.118,"y":980},{"x":1541.491,"y":979.7},{"x":1526.863,"y":980},{"x":1535.874,"y":951.751},{"x":1528.558,"y":920.597},{"x":1537.582,"y":892.35},{"x":1546.931,"y":864.161},{"x":1552.604,"y":835.318},{"x":1562.542,"y":837.335},{"x":1571.456,"y":831.931},{"x":1580.472,"y":827.265},{"x":1591.028,"y":833.762}],"neighborIds":["district:138","district:160"],"spawnZones":["east","south"],"isSpawnCandidate":true}]');
  const rawManifest = {
    id,
    version,
    width,
  ...
  const createAdminCreateController = (options) => {
    const bind = () => {
      ...
      target?.querySelector("[data-admin-create-open]")?.addEventListener("click", () => {
        options.updateState({
          wizardOpen: true,
          wizardStep: 1,
        ...
        options.render();
        queueMicrotask(() => options.target()?.querySelector("[data-admin-create-form] input:not([type=hidden])")?.focus());
      });
      const close = () => {
        options.updateState({ wizardOpen: false, wizardStep: 1, idempotencyKey: null });
        options.render();
        queueMicrotask(() => options.target()?.querySelector("[data-admin-create-open]")?.focus());
      };
      target?.querySelector("[data-admin-create-cancel]")?.addEventListener("click", close);
      target?.querySelectorAll("[data-admin-wizard-next]").forEach((button2) => button2.addEventListener("click", () => {
      ...
      target?.querySelectorAll("[data-admin-wizard-back]").forEach((button2) => button2.addEventListener("click", () => {
      ...
      target?.querySelector("[role=dialog]")?.addEventListener("keydown", (event) => {
        if (event.key === "Escape") {
    ...
    const bindMapTotal = () => {
      const form = options.target()?.querySelector("[data-admin-create-form]");
      const output = form?.querySelector("[data-admin-map-total]");
    ...
    const bindTemplatePolicy = () => {
      const form = options.target()?.querySelector("[data-admin-create-form]");
      const template = form?.querySelector("[data-admin-server-template]");
      const capacity = form?.querySelector("[data-admin-server-capacity]");
    ...
    const applyStep = () => {
      const target = options.target();
      target?.querySelectorAll("[data-admin-wizard-panel]").forEach((panel) => {
        panel.hidden = Number(panel.dataset.adminWizardPanel) !== options.state().wizardStep;
      });
      const form = target?.querySelector("[data-admin-create-form]");
      if (form) {
        updateWizardReview(form);
        form.querySelector(`[data-admin-wizard-panel="${options.state().wizardStep}"] input:not([type=hidden]),
  ...
  const createAdminLifecycleActionController = (options) => {
    ...
    const bind = () => {
      options.target()?.querySelectorAll("[data-admin-lifecycle]").forEach((button2) => button2.addEventListener("click", () => openDialog(button2)));
    };
    const openDialog = (sourceButton) => {
      ...
      bindDialog();
      queueMicrotask(() => options.target()?.querySelector("[data-admin-action-reason]")?.focus());
    };
    const bindDialog = () => {
      const target = options.target();
      const backdrop = target?.querySelector("[data-admin-lifecycle-backdrop]");
      const dialog = backdrop?.querySelector("[data-admin-lifecycle-dialog]");
      if (!backdrop || !dialog) return;
      const close = () => {
        const action2 = dialog.dataset.adminLifecycleAction;
        dialog.closest("[data-admin-lifecycle-backdrop]")?.remove();
        options.render();
        if (action2) queueMicrotask(() => options.target()?.querySelector(`[data-admin-lifecycle="${action2}"]`)?.focus());
      };
      backdrop.addEventListener("click", (event) => {
        if (event.target === event.currentTarget) close();
      });
      dialog.querySelectorAll("[data-admin-lifecycle-cancel]").forEach((button2) => button2.addEventListener("click", close));
      dialog.querySelector("[data-admin-action-reason]")?.addEventListener("input", (event) => {
      ...
      dialog.querySelector("[data-admin-lifecycle-confirm]")?.addEventListener("click", (event) => void submit(dialog, event.currentTarget));
    ...
    const submit = async (dialog, button2) => {
      ...
      const action2 = dialog.dataset.adminLifecycleAction;
      const hosted = options.controlPlane()?.servers.find((entry) => entry.serverInstanceId === instanceId);
      const reason = dialog.querySelector("[data-admin-action-reason]")?.value.trim() ?? "";
      ...
      if (reason.length < 3) {
        ...
        dialog.querySelector("[data-admin-action-reason]")?.focus();
        return;
      }
      if (action2 === "delete") {
        const confirmation = dialog.querySelector("[data-admin-delete-confirmation]")?.value.trim() ?? "";
        if (confirmation !== hosted.displayName) {
          if (message) message.textContent = "Pro smazání zadejte přesný název serveru.";
          dialog.querySelector("[data-admin-delete-confirmation]")?.focus();
          return;
        }
      }
      submitting = true;
  ...
  const createAdminRegistrationController = (options) => {
    ...
    const bind = () => {
      const target = options.target();
      target?.querySelectorAll("[data-admin-registration-action]").forEach((button2) => button2.addEventListener("click", () => openDialog(button2)));
      target?.querySelector("[data-admin-registration-opens-at]")?.addEventListener("input", (event) => {
    ...
    const schedulePayload = (instanceId) => {
      const input = options.target()?.querySelector("[data-admin-registration-opens-at]");
    ...
    const openDialog = (sourceButton) => {
      ...
      if (action2 === "schedule-registration" && !schedule) {
        const input = target?.querySelector("[data-admin-registration-opens-at]");
      ...
      bindDialog();
      queueMicrotask(() => target?.querySelector("[data-admin-registration-reason]")?.focus());
    };
    const bindDialog = () => {
      const target = options.target();
      const backdrop = target?.querySelector("[data-admin-registration-backdrop]");
      const dialog = backdrop?.querySelector("[data-admin-registration-dialog]");
      if (!backdrop || !dialog) return;
      const close = () => {
        dialog.closest("[data-admin-registration-backdrop]")?.remove();
        options.render();
        queueMicrotask(() => options.target()?.querySelector(`[data-admin-registration-action="${dialog.dataset.adminRegistrationAction}"]`)?.focus());
      };
      backdrop.addEventListener("click", (event) => {
      ...
      dialog.querySelector("[data-admin-registration-reason]")?.addEventListener("input", (event) => {
        const instanceId = dialog.dataset.adminServerId;
        if (instanceId && event.currentTarget instanceof HTMLTextAreaElement) {
          options.actionReasons.set(instanceId, event.currentTarget.value);
        }
      });
      dialog.querySelector("[data-admin-registration-confirm]")?.addEventListener("click", (event) => void submit(dialog, event.currentTarget));
      dialog.addEventListener("keydown", (event) => {
        if (event.key === "Escape" && !submitting) {
          event.preventDefault();
          close();
    ...
    const submit = async (dialog, button2) => {
      ...
      const hosted = options.controlPlane()?.servers.find((entry) => entry.serverInstanceId === instanceId);
      const reason = dialog.querySelector("[data-admin-registration-reason]")?.value.trim() ?? "";
      const message = dialog.querySelector("[data-admin-registration-error]");
      if (!instanceId || !action2 || !hosted) return;
      options.actionReasons.set(instanceId, reason);
      if (reason.length < 3) {
        if (message) message.textContent = "Uveďte důvod akce alespoň třemi znaky.";
        dialog.querySelector("[data-admin-registration-reason]")?.focus();
        return;
      }
      const payload = {
        action: action2,
      ...
      try {
        ...
        options.onActionAccepted?.(instanceId, action2);
        dialog.closest("[data-admin-registration-backdrop]")?.remove();
        await options.refresh();
      } catch (error) {
    ...
    const updateCountdowns = () => {
      ...
      options.target()?.querySelectorAll("[data-admin-registration-countdown]").forEach((node) => {
  ...
  const createAdminDashboardBindings = (options) => ({
    bind: () => {
      const target = options.target();
      target?.querySelectorAll("[data-admin-instance], [data-admin-mobile-instance]").forEach((link) => link.addEventListener("click", (event) => {
        event.preventDefault();
      ...
      target?.querySelector("[data-admin-refresh]")?.addEventListener("click", (event) => {
        const button2 = event.currentTarget;
        if (!button2.disabled) void options.refresh();
      });
      target?.querySelector("[data-admin-audit-refresh]")?.addEventListener("click", (event) => {
        event.currentTarget.disabled = true;
        void options.refresh(true);
      });
      target?.querySelector("[data-admin-logout]")?.addEventListener("click", () => void options.logout());
      target?.querySelector("[data-admin-notice-dismiss]")?.addEventListener("click", options.dismissNotice);
      const search = target?.querySelector("[data-admin-search]");
      search?.addEventListener("input", () => {
        options.updateServerFilters({ query: search.value });
        applyAdminServerFilters(target, options.serverFilters());
      });
      target?.querySelectorAll("[data-admin-server-filter]").forEach((select) => select.addEventListener("change", () => {
        const key = select.dataset.adminServerFilter;
        options.updateServerFilters({ [key]: select.value });
        applyAdminServerFilters(target, options.serverFilters());
      }));
      target?.querySelectorAll("[data-admin-server-scope]").forEach((button2) => button2.addEventListener("click", () => {
      ...
      target?.querySelector("[data-admin-filter-reset]")?.addEventListener("click", () => {
      ...
      const navItems = [...target?.querySelectorAll(".admin-nav__item") ?? []];
      ...
      navItems.forEach((item) => item.addEventListener("click", () => {
        ...
        options.setMobileNavOpen(false);
        const nav2 = target?.querySelector("#admin-primary-nav");
        const toggle = target?.querySelector("[data-admin-nav-toggle]");
        if (nav2) nav2.dataset.open = "false";
        if (toggle) toggle.setAttribute("aria-expanded", "false");
        const destination = item.hash ? target?.querySelector(item.hash) : null;
        if (destination instanceof HTMLDetailsElement) destination.open = true;
      }));
      target?.querySelector("[data-admin-nav-toggle]")?.addEventListener("click", (event) => {
      ...
      target?.querySelectorAll("[data-admin-preserve-input]").forEach((input) => input.addEventListener("blur", () => queueMicrotask(options.flushPendingRender)));
      applyAdminServerFilters(target, options.serverFilters());
    }
  });
  ...
  const createAdminLoginController = (options) => ({
    bind: () => {
      const target = options.target();
      const form = target?.querySelector("[data-admin-login]");
      const usernameInput = target?.querySelector("[data-admin-username]");
      const passwordInput = target?.querySelector("[data-admin-password]");
      ...
      form.addEventListener("submit", async (event) => {
        ...
        try {
          ...
          const message = options.target()?.querySelector("[data-admin-login-error]");
  ...
  const renderAdminAudit = (input) => {
    ...
    const status = input.error ? badge("AUDIT NEDOSTUPNÝ", "warning") : badge(`${input.entries?.length ?? 0} ZÁZNAMŮ`, "info");
  ...
  const renderAdminCommandCenter = (input) => {
    ...
    <div class="admin-command-center__primary">
      ...
      input.controlPlane ? "success" : "danger"
        ...
        ${service(
      "Databáze",
      input.controlPlane?.databaseAvailable ? "Online" : "Nedostupná",
      input.controlPlane?.databaseAvailable ? "success" : "danger"
    )}
        ${service(
      "Hosted worker",
      input.controlPlane?.workerStatus ?? "Ověřuji",
      input.controlPlane?.workerStatus === "online" ? "success" : input.controlPlane?.workerStatus === "stale" ? "warning" : "danger"
    )}
        ${service(
      "DB schéma",
      input.controlPlane?.migrationsCurrent ? "Aktuální" : "Vyžaduje migraci",
      input.controlPlane?.migrationsCurrent ? "success" : "danger"
        ...
        ${service(
      "Registrace",
      input.controlPlane?.registrationEnabled ? "Zapnutá" : "Vypnutá",
    ...
    <div class="admin-command-metrics">
      ...
      ${metric(
      "Worker",
      input.controlPlane?.workerStatus ?? "Neznámý",
      input.controlPlane?.workerStatus === "online" ? "success" : "warning",
      "Hosted runtime"
    )}
      ${metric(
      "Registrace",
  ...
  const buildTone = (control) => control?.buildCompatibility === "current" ? "success" : control?.buildCompatibility === "mismatch" ? "danger" : "warning";
  ...
  const createAdminApp = (options = {}) => {
    ...
    const handleError = (error) => {
      ...
      if (session && overview) renderOrDefer();
        ...
        target.querySelector("[data-admin-refresh]")?.addEventListener("click", () => void refresh());
      }
    };
