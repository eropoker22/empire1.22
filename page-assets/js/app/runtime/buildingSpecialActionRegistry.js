...
const SERVER_ACTIONS = new Map([
  ...
  ["burza::vnitrni tipy", ["insider_window", "stock_exchange"]],
  ...
  ["pristav::proriznout kontejner", ["port_container_cut", "port"]],
  ["parlament::policy window", ["parliament_policy_window", "parliament"]],
  ["parlament::politicke okno", ["parliament_policy_window", "parliament"]],
...
function formatRewardSummary(actionProfile = {}) {
  ...
  if (Number(actionProfile.apartmentPopulationBoostPct || 0) > 0) rewards.push(`Bytové bloky +${formatNumberValue(actionProfile.apartmentPopulationBoostPct)}% nábor členů`);
  if (Number(actionProfile.populationBoostPct || 0) > 0) rewards.push(`Populace +${formatNumberValue(actionProfile.populationBoostPct)}%`);
