...
| `EMPIRE_CLOSED_ALPHA_REGISTRATION_ENABLED` | Netlify API | Yes | Yes | No | Builds and Functions | No | true by default for owner releases; false only by an explicit owner disable | No | Keep true for staging and production public registration |
| `EMPIRE_CLOSED_ALPHA_REGISTRATION_EXPIRES_AT` | Netlify API | Optional | Optional | No | Builds and Functions | No | Empty means permanently open; a future ISO value retains the optional bounded mode | No | Keep unset for permanent public registration |
