...
| Zbrojovka | Výroba útočné a obranné výbavy. | Deset nezávislých linek spotřebovává Metal Parts, Tech Core a u high-tier receptů Combat Module. | None in building detail card. | Separate production flow, no special action rows. | Card must not render special-action buttons; production flow remains tested separately. | Do not reintroduce instant card rewards or duplicate recipe balance. |
