...
| --- | --- | --- | --- | --- | --- | --- |
| Sklad | Kapacity zásob, usage, warnings, clean/heat income. | Storage capacity by resource, warehouse network income/storage/heat multipliers. | Žádné. | Legacy-ready/passive. | Viewmodel no runtime error; capacity rows; upgrade benefit storage delta; no action rows. | Fake upgrade text, stale capacity labels, storage overflow copy. |
| Energetická stanice | Infrastruktura, clean/dirty/heat, tři akce s cooldownem. | Production/support multipliers and outage/heat support from config/runtime. | `backup_grid_switch`, `power_station_feed_production`, `power_station_reduce_outages`. | Mixed: `backup_grid_switch` has server id; legacy runtime handlers cover current profile outputs. | ActionId mapping, cooldown badge, confirm summaries, no missing handler. | Generic legacy output may not match final server behavior; verify before alpha. |
| Recyklační centrum | Salvage pool stav, clean/heat, `Vytěžit ztráty`. | Extracts non-population item losses from salvage/recovery pool. | `extract_losses`. | Legacy-ready with pool disabled condition. | Empty pool disabled, fresh item pool enabled, success starts cooldown and street news. | Must never recover population/gang members; no permanent disabled bug. |
...
| Továrna | Production/craft role, stored outputs, action buttons for production boosts. | Metal parts, tech core, combat modules according to production/craft config. | `factory_combat_module_run`, `factory_rapid_assembly`, `factory_industrial_overdrive`. | Legacy-ready; production/craft flow separate. | Action/profile mapping, confirm modal, no localStorage-authoritative mutation. | Do not break craft flow; avoid showing production action as instant reward if server disagrees. |
| Zbrojovka | Weapon/defense crafting role and action buttons. | Attack loadout, defense kit, fortify district outputs. | `armory_attack_loadout`, `armory_defense_kit`, `armory_fortify_district`. | Legacy-ready; production/craft flow separate. | Action/profile mapping, resource labels, cooldown, disabled for missing inputs if server requires. | Must not grant weapons via UI fallback if server command rejects. |
...
| Budova | Co má karta ukazovat | Reálné efekty | Special actions | Readiness | Testy | Rizika |
| --- | --- | --- | --- | --- | --- | --- |
| Pašovací tunel | Dirty flow, dealer supply, channel status/cost/cooldown. | Dirty cash flow, dealer sale price/speed/reward support, heat/street risk. | `open_channel`. | Legacy-ready/server id mapped. | Dirty cash cost disabled, active channel disabled, cooldown countdown. | Dirty cost must not be bypassed; channel state must persist across render. |
| Pouliční dealeři | Distribution from lab products, all three action rows visible/aligned. | Start sale boost, hot cash collection, stash transfer. | `start_drug_sale`, `street_dealers_collect_hot_cash`, `street_dealers_move_stash`. | Legacy-ready after profile/UI alignment. | Count UI actions equals profiles; no dead hidden action; stable actionId. | Product inventory and dirty rewards must stay server-safe before alpha. |
| Strip club | Night cash, VIP clients, kompromat/influence, heat risk. | Dirty cash, income/influence boost, influence for heat. | `strip_club_collect_cash`, `vip_lounge`, `private_party`. | Legacy-ready; `private_party` maps to current kompromat action. | Stable actionId, confirm summaries with heat/influence/cooldown. | Duplicate action id labels (`vip_lounge` building vs action) need careful UI labeling. |
