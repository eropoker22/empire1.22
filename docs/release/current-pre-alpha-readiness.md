...
6. invariant checker, archivaci disposable serverů a ověření trvale otevřené
   finální registrační policy (`registrationEnabled=true`, `mode=open`,
   `expiresAt=null`). Manuální owner deploy může aktuální `origin/main` vydat
   i bez acceptance evidence, ale stále vytváří backup, aplikuje migrace a
   zachovává rollback pointers.
